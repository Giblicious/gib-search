...
if (!builtMain.includes('semanticQueryMap(query, files)') || !builtMain.includes('semanticStarfield(query, files = null)') || !builtMain.includes('semanticNeighbors(file')) throw new Error('Local semantic map data is missing');
...
if (!mainSource.includes('collisionDistance = .035') || !mainSource.includes('edge.strength * .005')) throw new Error('Size-aware collision or similarity springs are missing');
...
if (!runtimeSource.includes('nearby.slice(0, 3)') || !mainSource.includes('charge = -Math.min')) throw new Error('Sparse semantic attraction or local repulsion is missing');
...
if (!runtimeSource.includes('residualScore: dot(residuals.get(edge.source)') || !mainSource.includes('edge.residual') || !mainSource.includes('const desired = .09 + (1 - node.relevance)')) throw new Error('Three-layer query, file, and residual physics is incomplete');
...
const emptyStarfield = await mapRuntime.semanticStarfield('', ['A.md', 'B.md', 'C.md']); if (emptyStarfield.nodes.length !== 3 || !emptyStarfield.edges.length || !emptyStarfield.edges.every(edge => edge.residualScore === 0)) throw new Error('Vault starfield is not available before a query');
