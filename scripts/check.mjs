...
const mainSource = fs.readFileSync(path.join(root, 'src', 'main.js'), 'utf8'), workerSource = fs.readFileSync(path.join(root, 'src', 'desktop-embed-worker.js'), 'utf8'); const modalSource = mainSource.slice(mainSource.indexOf('class SemanticSearchModal'), mainSource.indexOf('class SemanticInNoteSearch')), physicsSource = mainSource.slice(mainSource.indexOf('physicsStep(alpha)'), mainSource.indexOf('startSimulation(', mainSource.indexOf('physicsStep(alpha)')));
...
if (!builtMain.includes('semanticQueryMap(query, files)') || !builtMain.includes('semanticStarfield(query, files = null, focusFiles = [])') || !builtMain.includes('semanticNeighbors(file')) throw new Error('Local semantic map data is missing');
...
if (!mainSource.includes('this.startSimulation(previous.size ? .82 : 1)') || !mainSource.includes("id: '__query__'") || !mainSource.includes('(query.layoutX - query.x) * .012')) throw new Error('Persistent animated query target is missing');
if (/buildSemanticHeatField|drawContours|contourSegments|triangulateTerrain|quadraticCurveTo/.test(mainSource)) throw new Error('Terrain, contour, or line-field rendering remains in the semantic map');
if (!mainSource.includes('collisionDistance = .035') || !mainSource.includes('(node.layoutX - node.x) * .018')) throw new Error('Calculated-layout relaxation or size-aware collision is missing');
...
if (!runtimeSource.includes('louvainCommunities(ids, edges)') || !runtimeSource.includes('const reverse = directed[second].get(first)') || !runtimeSource.includes('consolidateCommunities(entries') || !runtimeSource.includes('weights.community * community ** 2')) throw new Error('Mutual-neighbor communities are not represented in the combined distance model');
if (!mainSource.includes('distance <= .92') || !mainSource.includes('(distance - .92) * .045') || physicsSource.includes('for (const edge of this.activeEdges') || physicsSource.includes('edge.residual')) throw new Error('Physics is not limited to target relaxation, collision cleanup, and emergency corralling');
if (!modalSource.includes('results = this.lastResults.slice(0, 80)') || !modalSource.includes('semanticStarfield(query, query ? activeFiles : paths, activeFiles)') || !modalSource.includes('.08 + rankedRelevance * .92')) throw new Error('Query map is not scoped to visible results or using the full relevance range');
...
if (mainSource.includes('ctx.filter') || mainSource.includes('parallaxX')) throw new Error('Distracting background blur or parallax remains in the search map');
...
if (!runtimeSource.includes('multiRelationalLayout(query, nodes') || !runtimeSource.includes('classicalDistanceLayout(ids, distances, conditioned ? conceptTopics : topics)') || !runtimeSource.includes('weights.semantic * semantic ** 2') || !runtimeSource.includes('relationDistance ** 2')) throw new Error('Query-conditioned multi-relational dimensional reduction is incomplete');
...
if (!runtimeSource.includes('async conceptFacets(query, files, evidence') || !runtimeSource.includes('contextScores(files)') || !runtimeSource.includes('argumentCandidateEdges(files)') || !modalSource.includes("lens === 'arguments'")) throw new Error('Concept, context, or argument lens analysis is missing');
...
if (!mainSource.includes("class NeighborhoodView") || !mainSource.includes("'aria-label': 'Local note lens'") || !mainSource.includes('centerFile: file.path') || !mainSource.includes('conceptFacetsFromFile(file.path')) throw new Error('Local-note lenses are not conditioned on the active note');
