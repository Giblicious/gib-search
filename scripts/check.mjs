...
if (!runtimeSource.includes('multiRelationalLayout(query, nodes') || !runtimeSource.includes('classicalDistanceLayout(ids, distances, topics)') || !runtimeSource.includes('weights.semantic * semantic ** 2') || !runtimeSource.includes('relationDistance ** 2')) throw new Error('Query-conditioned multi-relational dimensional reduction is incomplete');
if (!mainSource.includes("defaultSearchLens: 'relevance'") || !modalSource.includes("['relevance', 'concepts', 'arguments', 'context']") && !mainSource.includes('SEARCH_LENSES') || !modalSource.includes('applyLens()') || !modalSource.includes('gib-search-lens-select')) throw new Error('Coordinated search lenses or the Relevance default are missing');
