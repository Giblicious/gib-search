...
if (!runtimeSource.includes("RELATION_MODEL_ID = 'Xenova/mobilebert-uncased-mnli'") || !runtimeSource.includes('graphRelationships(edges, budget') || !modalSource.includes("result.argumentGroup = relation?.type === 'support'") || !mainSource.includes('graphRelationshipBudgetMobile')) throw new Error('Budgeted local relationship intelligence is missing');
...
if (!runtimeSource.includes('async conceptFacets(query, files)') || !runtimeSource.includes('contextScores(files)') || !modalSource.includes("lens === 'arguments'")) throw new Error('Concept, context, or argument lens analysis is missing');
...
const facets = await mapRuntime.conceptFacets('A', ['A.md', 'B.md', 'C.md']); if (facets.size !== 3 || ![...facets.values()].every(value => Number.isInteger(value.facet) && value.label)) throw new Error('Concept lens did not produce complete labeled facets'); const context = mapRuntime.contextScores(['A.md', 'B.md', 'C.md']); if (context.size !== 3 || ![...context.values()].every(value => Number.isFinite(value) && value >= 0 && value <= 1)) throw new Error('Context lens did not produce bounded vault-context scores'); for (const lens of ['relevance', 'concepts', 'arguments', 'context']) { const lensNodes = reducedNodes.map((node, index) => ({ ...node, facet: index > 1 ? 1 : 0, argumentGroup: index === 0 ? 'anchor' : index === 1 ? 'support' : 'tension' })), layout = await mapRuntime.multiRelationalLayout('A', lensNodes, new Map([[['B.md', 'C.md'].sort().join('\0'), { type: 'contrast', confidence: .9 }]]), { lens }); if (layout.size !== 3 || ![...layout.values()].every(point => Number.isFinite(point.x) && Number.isFinite(point.y))) throw new Error(`${lens} lens did not produce a complete finite layout`); }
