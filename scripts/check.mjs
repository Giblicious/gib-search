...
if (!builtMain.includes('semanticQueryMap(query, files)') || !builtMain.includes('semanticNeighbors(file')) throw new Error('Local semantic map data is missing');
...
if (!mainSource.includes('collisionDistance = .085') || !mainSource.includes('relationship.overall * .0052')) throw new Error('Size-aware collision or similarity springs are missing');
if (!mainSource.includes('relationship = focused && !queryFocused') || !mainSource.includes('.3 + related * .55')) throw new Error('Line-free semantic hover emphasis is missing');
