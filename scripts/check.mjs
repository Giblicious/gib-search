...
const emptyStarfield = await mapRuntime.semanticStarfield('', ['A.md', 'B.md', 'C.md']); if (emptyStarfield.nodes.length !== 3 || !emptyStarfield.nodes.every(node => Number.isFinite(node.x) && Number.isFinite(node.y)) || !emptyStarfield.edges.length || !emptyStarfield.edges.every(edge => edge.residualScore === 0)) throw new Error('Clustered vault starfield is not available before a query');
