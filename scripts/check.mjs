...
if (!builtMain.includes('LivingSemanticMapCanvas') || !builtMain.includes('buildSemanticHeatField') || !builtMain.includes('terrain.edges || []')) throw new Error('Living semantic heat field or note gravity is missing');
...
if (!mainSource.includes('buildSemanticHeatField(this.queryNode, this.nodes, this.activeEdges, 220)')) throw new Error('High-resolution adaptive semantic heat field is missing');
if (!mainSource.includes('localSpacing = median(distances.slice(0, 3))') || !mainSource.includes('coherence * .9')) throw new Error('Heat spread and direction are not adaptive to semantic neighborhoods');
if (!mainSource.includes('(positive ? .2 : -.16)') || !mainSource.includes('edge.strength * Math.abs(edge.residual)')) throw new Error('Signed residual relationships do not form ridges and valleys');
if (!mainSource.includes('prepareHeatInfluence') || !mainSource.includes('inverseAlong')) throw new Error('Heat rendering recalculates kernel geometry inside the raster loop');
if (!mainSource.includes('levels = [.08, .21, .35, .5, .67, .84]')) throw new Error('Heat map must use six normalized elevation contours');
...
mapRuntime.queryVector = async () => mapRuntime.vectors[0]; const terrain = await mapRuntime.semanticTerrain('A', ['A.md', 'B.md', 'C.md']); if (terrain.edges.length !== 3 || !terrain.nodes.every(node => Number.isFinite(node.semanticScore)) || !terrain.edges.every(edge => Number.isFinite(edge.score) && Number.isFinite(edge.residualScore))) throw new Error('Semantic terrain does not expose complete three-layer relationship data');
