...
const mainSource = fs.readFileSync(path.join(root, 'src', 'main.js'), 'utf8'), workerSource = fs.readFileSync(path.join(root, 'src', 'desktop-embed-worker.js'), 'utf8'); const modalSource = mainSource.slice(mainSource.indexOf('class SemanticSearchModal'), mainSource.indexOf('class SemanticInNoteSearch'));
...
if (!mainSource.includes('this.startSimulation(previous.size ? .82 : 1)') || !mainSource.includes("id: '__query__'") || !mainSource.includes('centerX = bodies.reduce')) throw new Error('Persistent unpinned query physics or whole-system recentering is missing');
...
if (!mainSource.includes('collisionDistance = .035') || !mainSource.includes('edge.strength * .017')) throw new Error('High-contrast size-aware collision or similarity springs are missing');
...
if (!runtimeSource.includes('louvainCommunities(ids, edges)') || !runtimeSource.includes('const reverse = directed[second].get(first)') || !runtimeSource.includes('clusteredSemanticPositions(entries, communities)') || !runtimeSource.includes('consolidateCommunities(entries') || !mainSource.includes('crossCommunity ? baseStrength * .08') || !mainSource.includes('charge = -Math.min')) throw new Error('Mutual-neighbor community clustering, separation, or repulsion is missing');
if (!mainSource.includes('const corralBodies =') || !mainSource.includes('(distance - radius) * .055') || !mainSource.includes('(1 - edge.overall) * .34') || mainSource.includes('group.x / group.count - node.x')) throw new Error('Soft corral, force contrast, or removal of artificial community-centroid gravity is incomplete');
...
if (!runtimeSource.includes('semanticTopicBasis(entries)') || !runtimeSource.includes('topicCoordinates(entries, queryVector, basis.axes)') || !mainSource.includes('node.topicAngle') || !mainSource.includes('node.topicHue')) throw new Error('Stable topic compass coordinates, physics, or semantic hues are missing');
...
if (!runtimeSource.includes("RELATION_MODEL_ID = 'Xenova/mobilebert-uncased-mnli'") || !runtimeSource.includes('graphRelationships(edges, budget') || !mainSource.includes('applyRelationships(details)') || !mainSource.includes('graphRelationshipBudgetMobile')) throw new Error('Budgeted local relationship intelligence is missing');
...
if (!mainSource.includes('(!a.matched || !b.matched)') || !mainSource.includes('this.pendingQuery') || !mainSource.includes('const force = Math.min(.012') || !mainSource.includes('this.cameraZoom')) throw new Error('Pending-query repulsion, foreground isolation, or zoom is missing');
...
if (!runtimeSource.includes('residualScore: dot(residuals.get(edge.source)') || !mainSource.includes('edge.residual') || !mainSource.includes('const desired = .065 + (1 - node.relevance)')) throw new Error('Three-layer query, file, and residual physics is incomplete');
