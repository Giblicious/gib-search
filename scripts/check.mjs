...
if (!mainSource.includes('collisionDistance = .035') || !mainSource.includes('edge.strength * .013')) throw new Error('High-contrast size-aware collision or similarity springs are missing');
...
if (!runtimeSource.includes('louvainCommunities(ids, edges)') || !runtimeSource.includes('const reverse = directed[second].get(first)') || !runtimeSource.includes('clusteredSemanticPositions(entries, communities)') || !runtimeSource.includes('consolidateCommunities(entries') || !mainSource.includes('crossCommunity ? baseStrength * .06') || !mainSource.includes('charge = -Math.min')) throw new Error('Mutual-neighbor community clustering, separation, or repulsion is missing');
if (!mainSource.includes('const corralBodies =') || !mainSource.includes('(distance - radius) * .055') || !mainSource.includes('(1 - edge.overall) * .46') || mainSource.includes('group.x / group.count - node.x')) throw new Error('Soft corral, force contrast, or removal of artificial community-centroid gravity is incomplete');
...
if (!runtimeSource.includes('residualScore: dot(residuals.get(edge.source)') || !mainSource.includes('edge.residual') || !mainSource.includes('const desired = .07 + (1 - node.relevance)')) throw new Error('Three-layer query, file, and residual physics is incomplete');
