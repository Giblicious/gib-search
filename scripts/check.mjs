...
if (!runtimeSource.includes("RELATION_MODEL_ID = 'Xenova/mobilebert-uncased-mnli'") || !runtimeSource.includes('graphRelationships(edges, budget') || !mainSource.includes('applyCalculatedLayout(enrichedLayout, details)') || !mainSource.includes('graphRelationshipBudgetMobile')) throw new Error('Budgeted local relationship intelligence is missing');
...
if (workerSource.includes("message.type === 'relations') embedQueue") || !modalSource.includes('window.setTimeout(resolve, 320)')) throw new Error('Relationship intelligence can block or outrun live search input');
...
if (!runtimeSource.includes('multiRelationalLayout(query, nodes') || !runtimeSource.includes('classicalDistanceLayout(ids, distances, topics)') || !runtimeSource.includes('.38 * semantic ** 2 + .32 * residual ** 2') || !runtimeSource.includes('relationDistance ** 2')) throw new Error('Query-conditioned multi-relational dimensional reduction is incomplete');
...
const reducedNodes = queryStarfield.nodes.map((node, index) => ({ ...node, relevance: 1 - index * .35 })), reducedLayout = await mapRuntime.multiRelationalLayout('A', reducedNodes); if (reducedLayout.size !== 3 || ![...reducedLayout.values()].every(point => Number.isFinite(point.x) && Number.isFinite(point.y))) throw new Error('Query-conditioned multi-relational reduction did not produce a complete finite layout'); const enrichedLayout = await mapRuntime.multiRelationalLayout('A', reducedNodes, new Map([[['B.md', 'C.md'].sort().join('\0'), { type: 'contrast', confidence: .9 }]])); if ([...reducedLayout].every(([id, point]) => Math.hypot(point.x - enrichedLayout.get(id).x, point.y - enrichedLayout.get(id).y) < 1e-5)) throw new Error('Relationship evidence did not update the combined distance layout');
