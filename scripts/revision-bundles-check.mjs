...
const mainSource = fs.readFileSync(path.join(root, 'src', 'main.js'), 'utf8'), stylesSource = fs.readFileSync(path.join(root, 'styles.css'), 'utf8');
...
if (!stylesSource.includes('.gib-similar-version-group{') || !stylesSource.includes('.gib-similar-version-link{') || !stylesSource.includes('font-size:var(--font-ui-small)')) throw new Error('Compact version history lacks the flat accessible Similar Notes styling');
