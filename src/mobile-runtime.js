...
export class MobileSearchRuntime {
  ...
  vaultContentProfiles(files = null, maximum = 8) {
    const signature = `${this.meta.length}:${this.vectors.length}:${this.meta.reduce((sum, item) => sum + Number(item.mtime || 0), 0)}`; if (!this.contentProfileCache || this.contentProfileCache.signature !== signature) { const center = Float32Array.from(this.topicBasis().center), centerNorm = Math.sqrt(dot(center, center)) || 1; for (let dimension = 0; dimension < center.length; dimension++) center[dimension] /= centerNorm; const fileResiduals = new Map([...this.fileVectors()].map(([file, entry]) => [file, residualVector(entry.vector, center)])), passages = this.meta.map((item, index) => this.vectors[index] ? { file: item.file, vector: residualVector(this.vectors[index], center), commonness: 0, weight: 1 } : null).filter(Boolean); for (const passage of passages) { const scores = [...fileResiduals].filter(([file]) => file !== passage.file).map(([, vector]) => dot(passage.vector, vector)).sort((a, b) => b - a).slice(0, 8); passage.commonness = scores.reduce((sum, value) => sum + value, 0) / Math.max(1, scores.length); } const ranked = passages.slice().sort((a, b) => a.commonness - b.commonness), divisor = Math.max(1, ranked.length - 1); ranked.forEach((passage, index) => { passage.weight = .25 + (1 - index / divisor) * .75; }); const grouped = new Map(); for (const passage of passages) { const values = grouped.get(passage.file) || []; values.push(passage); grouped.set(passage.file, values); } const profiles = new Map(); for (const [file, values] of grouped) { const remaining = values.slice(), selected = []; while (remaining.length && selected.length < maximum) { remaining.sort((a, b) => { const diversity = item => selected.length ? 1 - Math.max(...selected.map(other => dot(item.vector, other.vector))) : 1, first = a.weight * .72 + diversity(a) * .28, second = b.weight * .72 + diversity(b) * .28; return second - first; }); selected.push(remaining.shift()); } profiles.set(file, selected.map(({ vector, weight }) => ({ vector, weight }))); } this.contentProfileCache = { signature, profiles }; }
