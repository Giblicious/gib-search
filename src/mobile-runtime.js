...
export class MobileSearchRuntime {
  ...
  conceptFacetsFromVector(queryVector, files, evidence = new Map(), query = '') {
    ...
    const communities = consolidateCommunities(entries, louvainCommunities(entries.map(entry => entry.id), edges)), entitySets = this.fileEntities(ordered), entityFrequency = new Map(); for (const entities of entitySets.values()) for (const entity of entities) entityFrequency.set(entity, (entityFrequency.get(entity) || 0) + 1); const groups = new Map(); for (const entry of entries) { const id = communities.get(entry.id) ?? 0, group = groups.get(id) || []; group.push(entry.id); groups.set(id, group); }
    const queryWords = new Set(tokens(query)), candidateSets = new Map(), documentFrequency = new Map(); for (const file of ordered) { const source = `${evidence.get(file) || ''} ${basename(file)}`, words = (source.toLowerCase().match(/[\p{L}\p{N}][\p{L}\p{N}'-]*/gu) || []).filter(word => word.length > 2 && !STOP_WORDS.has(word) && !GENERIC_CONCEPTS.has(word) && !queryWords.has(word) && !/^\d+$/.test(word)), candidates = new Set([...(entitySets.get(file) || [])].filter(value => !queryWords.has(value) && !GENERIC_CONCEPTS.has(value))); for (const word of words) candidates.add(word); for (let index = 0; index < words.length - 1; index++) candidates.add(`${words[index]} ${words[index + 1]}`); candidateSets.set(file, candidates); for (const candidate of candidates) documentFrequency.set(candidate, (documentFrequency.get(candidate) || 0) + 1); }
    const centroids = new Map(), labels = new Map(); for (const [community, members] of groups) { const centroid = new Float32Array(DIMENSION); for (const file of members) { const vector = entries.find(entry => entry.id === file)?.vector; if (vector) for (let dimension = 0; dimension < DIMENSION; dimension++) centroid[dimension] += vector[dimension]; } const norm = Math.sqrt(dot(centroid, centroid)) || 1; for (let dimension = 0; dimension < DIMENSION; dimension++) centroid[dimension] /= norm; centroids.set(community, centroid); const counts = new Map(); for (const file of members) for (const candidate of candidateSets.get(file) || []) counts.set(candidate, (counts.get(candidate) || 0) + 1); const ranked = [...counts].map(([value, count]) => ({ value, score: count * Math.log(1 + entries.length / Math.max(1, documentFrequency.get(value) || 1)) * (value.includes(' ') ? 1.18 : 1) })).sort((a, b) => b.score - a.score || a.value.localeCompare(b.value)); const best = ranked[0], runnerUp = ranked[1], labelConfidence = best ? Math.min(1, .34 + best.score / Math.max(.001, best.score + Number(runnerUp?.score || 0)) * .5 + Math.min(.16, members.length / entries.length * .3)) : 0; labels.set(community, { label: best ? String(best.value).replace(/\b\p{L}/gu, letter => letter.toUpperCase()) : '', confidence: labelConfidence }); }
