...
function locallyDistinctAffinityMatrix(scores) {
  const count = scores.length, directed = Array.from({ length: count }, () => new Float64Array(count)); if (count < 2) return directed; const neighborhood = Math.min(count - 1, Math.max(8, Math.round(Math.sqrt(count) * 1.5)));
  for (let first = 0; first < count; first++) { const ranked = [...scores[first]].filter((_, second) => second !== first).sort((a, b) => b - a), local = ranked.slice(0, neighborhood), baseline = Number(local[Math.min(local.length - 1, Math.floor(local.length * .58))] || 0), peak = local.slice(0, Math.min(3, local.length)).reduce((sum, value) => sum + value, 0) / Math.max(1, Math.min(3, local.length)), scale = Math.max(.025, peak - baseline); for (let second = 0; second < count; second++) if (second !== first) { const normalized = Math.max(0, Math.min(1, (scores[first][second] - baseline) / scale)), shaped = normalized * normalized * (3 - 2 * normalized); directed[first][second] = shaped; } }
  return Array.from({ length: count }, (_, first) => Float64Array.from({ length: count }, (_, second) => first === second ? 1 : Math.max(.001, Math.min(1, ((scores[first][second] + 1) / 2) * .2 + Math.sqrt(directed[first][second] * directed[second][first]) * .8))));
}
function specificTermSimilarity(first, second) { if (!first?.weights || !second?.weights) return 0; const [small, large] = first.weights.size <= second.weights.size ? [first, second] : [second, first]; let evidence = 0; for (const [term, weight] of small.weights) if (large.weights.has(term)) evidence += Math.min(weight, large.weights.get(term)); return Math.max(0, Math.min(1, 1 - Math.exp(-evidence / 11))); }
...
export class MobileSearchRuntime {
  ...
  specificTermProfiles(files = null) {
    const signature = `${this.meta.length}:${this.meta.reduce((sum, item) => sum + Number(item.mtime || 0), 0)}`; if (!this.termProfileCache || this.termProfileCache.signature !== signature) { const counts = new Map(), add = (file, source, multiplier = 1) => { const values = counts.get(file) || new Map(); for (const term of tokens(source)) { if (STOP_WORDS.has(term) || GENERIC_CONCEPTS.has(term) || STRUCTURAL_TOPIC_LABELS.has(term) || VAGUE_LABEL_WORDS.has(term) || /^\d+$/.test(term)) continue; values.set(term, (values.get(term) || 0) + multiplier); } counts.set(file, values); }; for (const item of this.meta) { add(item.file, item.text, 1); add(item.file, item.heading, 2); } for (const file of counts.keys()) add(file, basename(file), 3); const frequency = new Map(); for (const values of counts.values()) for (const term of values.keys()) frequency.set(term, (frequency.get(term) || 0) + 1); const total = Math.max(1, counts.size), profiles = new Map(); for (const [file, values] of counts) { const ranked = [...values].map(([term, count]) => { const documentFrequency = frequency.get(term) || 1, idf = Math.log(1 + total / documentFrequency), weight = (1 + Math.log(Math.max(1, count))) * idf; return { term, weight, documentFrequency }; }).filter(item => item.documentFrequency / total <= .42).sort((a, b) => b.weight - a.weight).slice(0, 96), weights = new Map(ranked.map(item => [item.term, item.weight])); profiles.set(file, { weights }); } this.termProfileCache = { signature, profiles }; }
    const requested = files ? new Set(files) : null; return requested ? new Map([...this.termProfileCache.profiles].filter(([file]) => requested.has(file))) : this.termProfileCache.profiles;
  }
  ...
  async semanticStarfield(query, files = null, focusFiles = []) {
    ...
    if (!this.starfieldCache || this.starfieldCache.signature !== signature) {
      ...
      const rawScores = Array.from({ length: entries.length }, (_, index) => Float64Array.from({ length: entries.length }, (_, other) => index === other ? 1 : 0)); for (let first = 0; first < entries.length; first++) { for (let second = first + 1; second < entries.length; second++) { const semanticScore = vaultMode ? contentProfileSimilarity(profiles.get(entries[first].id), profiles.get(entries[second].id), tuning.passageCoverage) : dot(entries[first].vector, entries[second].vector), lexicalEvidence = vaultMode ? specificTermSimilarity(termProfiles.get(entries[first].id), termProfiles.get(entries[second].id)) : 0, score = Math.max(-1, Math.min(1, semanticScore + lexicalEvidence * .1)); rawScores[first][second] = score; rawScores[second][first] = score; } if (first % 6 === 5) await yieldToUi(); } const distinctAffinities = locallyDistinctAffinityMatrix(rawScores), neighbors = []; for (let first = 0; first < entries.length; first++) { const entry = entries[first], nearby = entries.map((other, second) => first === second ? null : { index: second, id: other.id, score: rawScores[first][second], distinctiveness: distinctAffinities[first][second], entityScore: entityAffinity(entry.id, other.id) }).filter(Boolean).sort((a, b) => b.distinctiveness + b.entityScore * .08 - a.distinctiveness - a.entityScore * .08).slice(0, 8); neighbors.push(nearby); } const directed = neighbors.map(list => new Map(list.map(item => [item.index, { ...item, affinity: item.distinctiveness * .82 + item.entityScore * .18 }]))), edges = [], seen = new Set(), incident = new Set();
      ...
      const sensitivity = Math.max(.5, Math.min(2, Number(tuning.communitySensitivity) || 1)), detected = louvainCommunities(entries.map(entry => entry.id), edges, sensitivity), baseMaximum = Math.max(10, Math.min(18, Math.round(Math.sqrt(entries.length)))), communities = vaultMode ? mergeConnectedCommunities(entries.map(entry => entry.id), edges, detected, Math.max(4, Math.round(baseMaximum * sensitivity))) : consolidateCommunities(entries, detected), positions = clusteredSemanticPositions(entries, communities), communityIds = [...new Set(communities.values())], topicAffinities = new Map();
      ...
      const communitySizes = new Map(); for (const community of communities.values()) communitySizes.set(community, (communitySizes.get(community) || 0) + 1); const communityAnalysis = this.topicCommunityAnalysis(entries, communities, tuning.communityLabelSensitivity, tuning.communityMinSize); let communityLabels = communityAnalysis.labels; if (vaultMode) communityLabels = await this.generatedTopicCommunityLabels(entries, communities, communityAnalysis, tuning.communityMinSize); this.starfieldCache = { signature, entries, edges, positions, communities, topicAffinities, communitySizes, communityLabels, entitySets, uniqueness };
    }
    const { entries, edges, positions, communities, topicAffinities, communitySizes, communityLabels, entitySets, uniqueness } = this.starfieldCache, basis = this.topicBasis(), communityFields = entry => { const community = communities.get(entry.id) ?? 0, affinities = topicAffinities.get(entry.id) || {}; return { community, topicAffinities: affinities, communityMembership: Number(affinities[community] || 0), communitySize: Number(communitySizes.get(community) || 1), communityLabel: communityLabels.get(community)?.label || '', communityFallbackLabel: communityLabels.get(community)?.fallbackLabel || '', communityLabelConfidence: Number(communityLabels.get(community)?.confidence || 0) }; };
