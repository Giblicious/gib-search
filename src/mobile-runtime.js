...
export class MobileSearchRuntime {
  ...
  async multiRelationalLayout(query, nodes, relationships = new Map(), options = {}) {
    ...
    if (conditioned) {
      const primaryEntries = entries.filter(value => Number(nodeById.get(value.id)?.generation || 1) === 1), positioned = new Map(); for (const entry of primaryEntries) { const topic = layoutTopics.get(entry.id), point = layout.get(entry.id), conceptAngle = topicMode || Number(topic?.topicStrength || 0) > .015 ? Number(topic?.topicAngle || 0) : 0, angle = lens === 'relevance' ? conceptAngle : point && Math.hypot(point.x, point.y) > .001 ? Math.atan2(point.y, point.x) : conceptAngle, radius = .08 + (1 - relevance.get(entry.id)) * .72; positioned.set(entry.id, (vaultCentered || linkMode) && point ? { x: point.x, y: point.y } : { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius }); }
