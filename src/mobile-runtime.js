...
export class MobileSearchRuntime {
  ...
  async search(query, topK, minScore, options = {}) {
    ...
    for (let i = 0; i < this.vectors.length; i++) {
      if (i % 192 === 191) { await yieldToUi(); if (this.livePending) throw staleSearchError(); } if (options.file && this.meta[i].file !== options.file) continue; const semantic = dotPacked(queryVector, this.packedVectors, i * DIMENSION); if (semantic < minScore) continue;
  ...
  async semanticStarfield(query, files = null, focusFiles = []) {
    ...
    if (!this.starfieldCache || this.starfieldCache.signature !== signature) {
      ...
      const communitySizes = new Map(); for (const community of communities.values()) communitySizes.set(community, (communitySizes.get(community) || 0) + 1); const communityAnalysis = this.topicCommunityAnalysis(entries, communities, tuning.communityLabelSensitivity, tuning.communityMinSize); let communityLabels = communityAnalysis.labels; if (vaultMode) communityLabels = await this.generatedTopicCommunityLabels(entries, communities, communityAnalysis, tuning.communityMinSize); this.starfieldCache = { signature, entries, edges, positions, communities, parentCommunities, hierarchyAnalysis: hierarchy.analysis, topicAffinities, communitySizes, communityLabels, entitySets, uniqueness };
    }
    const { entries, edges, positions, communities, parentCommunities, hierarchyAnalysis, topicAffinities, communitySizes, communityLabels, entitySets, uniqueness } = this.starfieldCache, basis = this.topicBasis(), communityFields = entry => { const community = communities.get(entry.id) ?? 0, parentCommunity = parentCommunities?.get(entry.id) ?? community, parentAnalysis = hierarchyAnalysis?.get(parentCommunity), affinities = topicAffinities.get(entry.id) || {}; return { community, parentCommunity, neighborhoodDepth: community === parentCommunity ? 1 : 2, neighborhoodTemperature: Number(parentAnalysis?.temperature || 0), neighborhoodStability: Number(parentAnalysis?.stability || 0), topicAffinities: affinities, communityMembership: Number(affinities[community] || 0), communitySize: Number(communitySizes.get(community) || 1), communityLabel: communityLabels.get(community)?.label || '', communityFallbackLabel: communityLabels.get(community)?.fallbackLabel || '', communityLabelConfidence: Number(communityLabels.get(community)?.confidence || 0) }; };
