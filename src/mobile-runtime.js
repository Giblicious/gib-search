...
const STRUCTURAL_TOPIC_LABELS = new Set(['ai', 'assistant', 'chat', 'chatbot', 'conversation', 'discussion', 'journal', 'notes', 'transcript']);
...
function cleanGeneratedTopicLabel(source) { const value = cleanText(source).replace(/^(?:the )?(?:shared )?(?:topic|subject|label)(?: is)?\s*[:\-–—]?\s*/i, '').replace(/^["'“”‘’]+|["'“”‘’.,;:!?]+$/g, '').replace(/\s+/g, ' ').trim(), words = value.match(/[\p{L}\p{N}'’&-]+/gu) || [], normalized = words.map(word => word.toLowerCase()); if (words.length < 1 || words.length > 5 || value.length > 54 || isTemporalConceptLabel(value) || normalized.every(word => STOP_WORDS.has(word) || GENERIC_CONCEPTS.has(word) || STRUCTURAL_TOPIC_LABELS.has(word)) || normalized.some(word => STRUCTURAL_TOPIC_LABELS.has(word)) && words.length <= 2) return ''; return words.join(' ').replace(/\b\p{L}/gu, letter => letter.toUpperCase()); }
...
export class MobileSearchRuntime {
  ...
  topicCommunityLabels(entries, communities, sensitivity = .58, minimumSize = 3) {
    const groups = new Map(); for (const entry of entries) { const id = communities.get(entry.id) ?? 0, members = groups.get(id) || []; members.push(entry); groups.set(id, members); } const centroids = new Map(); for (const [id, members] of groups) { const vector = new Float32Array(DIMENSION); for (const member of members) for (let dimension = 0; dimension < DIMENSION; dimension++) vector[dimension] += member.vector[dimension]; const norm = Math.sqrt(dot(vector, vector)) || 1; for (let dimension = 0; dimension < DIMENSION; dimension++) vector[dimension] /= norm; centroids.set(id, vector); }
    const labels = new Map(), threshold = Math.max(.35, Math.min(.85, Number(sensitivity) || .58)); for (const [id, members] of groups) { if (members.length < Math.max(2, Number(minimumSize) || 3)) { labels.set(id, { label: '', fallbackLabel: '', confidence: 0 }); continue; } const centroid = centroids.get(id), memberIds = members.map(member => member.id), candidates = new Map(); for (const [file, values] of this.indexedConceptCandidates(centroid, memberIds)) for (const [key, candidate] of values) { const value = candidates.get(key) || { ...candidate, files: new Set() }; value.files.add(file); candidates.set(key, value); } const otherCentroids = [...centroids].filter(([other]) => other !== id).map(([, vector]) => vector), ranked = [...candidates.values()].map(candidate => { const centrality = Math.max(0, Math.min(1, (dotHighlight(centroid, candidate.vector) + 1) / 2)), other = otherCentroids.length ? Math.max(...otherCentroids.map(vector => (dotHighlight(vector, candidate.vector) + 1) / 2)) : centrality - .08, distinction = Math.max(0, Math.min(1, .5 + (centrality - other) * 2.4)), coverage = Math.min(1, candidate.files.size / Math.max(1, members.length)), quality = Math.min(1, Number(candidate.quality || 0) / .18), score = centrality * .5 + distinction * .27 + coverage * .13 + quality * .1; return { phrase: candidate.phrase, score, files: candidate.files }; }).sort((a, b) => b.score - a.score || a.phrase.localeCompare(b.phrase)), best = ranked[0], margin = Number(best?.score || 0) - Number(ranked[1]?.score || 0), supported = Number(best?.files?.size || 0) >= Math.min(2, members.length), confidence = best && supported ? Math.max(0, Math.min(1, best.score * .82 + margin * 1.8)) : 0, fallbackLabel = best && supported && best.score >= .42 && !isTemporalConceptLabel(best.phrase) && String(best.phrase).split(/\s+/).length <= 4 ? String(best.phrase).replace(/\b\p{L}/gu, letter => letter.toUpperCase()) : ''; labels.set(id, { label: confidence >= threshold ? fallbackLabel : '', fallbackLabel, confidence }); } return labels;
  }
  async generatedTopicCommunityLabels(entries, communities, fallbackLabels, minimumSize = 3) {
    ...
    const groups = new Map(); for (const entry of entries) { const id = communities.get(entry.id) ?? 0, members = groups.get(id) || []; members.push(entry); groups.set(id, members); }
    const centroids = new Map(); for (const [id, members] of groups) { const vector = new Float32Array(DIMENSION); for (const member of members) for (let dimension = 0; dimension < DIMENSION; dimension++) vector[dimension] += member.vector[dimension]; const norm = Math.sqrt(dot(vector, vector)) || 1; for (let dimension = 0; dimension < DIMENSION; dimension++) vector[dimension] /= norm; centroids.set(id, vector); }
    const requests = []; for (const [id, members] of groups) {
      if (members.length < Math.max(2, Number(minimumSize) || 3)) continue; const memberIds = new Set(members.map(member => member.id)), fingerprint = `${TOPIC_LABEL_MODEL_ID}:v2:${members.map(member => `${member.id}:${this.fileFingerprint(member.id)}`).sort().join('|')}`; if (this.topicLabelCache.has(fingerprint)) continue;
      const centroid = centroids.get(id), passages = this.meta.map((item, index) => memberIds.has(item.file) && this.vectors[index] ? { item, score: dot(centroid, this.vectors[index]) } : null).filter(Boolean).sort((a, b) => b.score - a.score), chosen = [], chosenFiles = new Set(); for (const passage of passages) { if (chosenFiles.has(passage.item.file)) continue; const text = cleanText(`${passage.item.heading || ''}. ${passage.item.text || ''}`).slice(0, 430); if (text.length < 24) continue; chosen.push(text); chosenFiles.add(passage.item.file); if (chosen.length >= 4) break; }
      if (chosen.length < 2) continue; const prompt = `What is the main topic shared by these passages? Answer with a concise noun phrase.\n\n${chosen.map((text, index) => `Passage ${index + 1}: ${text}`).join('\n\n')}`.slice(0, 2200); requests.push({ id, fingerprint, prompt });
    }
    try {
      if (requests.length) { this.topicLabelActivity(`Naming ${requests.length} topic communit${requests.length === 1 ? 'y' : 'ies'}`); const generated = await this.generateTopicLabelTexts(requests.map(request => request.prompt)); requests.forEach((request, index) => this.topicLabelCache.set(request.fingerprint, cleanGeneratedTopicLabel(generated[index]))); await this.saveTopicLabelCache(); }
      const output = new Map(fallbackLabels), used = new Map(); for (const [id, members] of groups) { const fingerprint = `${TOPIC_LABEL_MODEL_ID}:v2:${members.map(member => `${member.id}:${this.fileFingerprint(member.id)}`).sort().join('|')}`, label = this.topicLabelCache.get(fingerprint) || ''; if (!label) continue; const vector = await this.embed(label, false), centroid = centroids.get(id), centrality = Math.max(0, Math.min(1, (dot(centroid, vector) + 1) / 2)), other = [...centroids].filter(([otherId]) => otherId !== id).map(([, value]) => Math.max(0, Math.min(1, (dot(value, vector) + 1) / 2))), distinction = centrality - (other.length ? Math.max(...other) : centrality - .08), confidence = Math.max(0, Math.min(1, centrality * .72 + Math.max(0, distinction) * 1.4)); if (centrality < .56 || distinction < -.025) continue; const key = label.toLowerCase(), previous = used.get(key); if (!previous || confidence > previous.confidence) { if (previous) output.set(previous.id, fallbackLabels.get(previous.id) || { label: '', fallbackLabel: '', confidence: 0 }); used.set(key, { id, confidence }); output.set(id, { label, fallbackLabel: fallbackLabels.get(id)?.fallbackLabel || '', confidence }); } }
      this.topicLabelActivity(requests.length ? 'Topic labels ready' : 'Using cached topic labels'); return output;
    } catch (error) { this.topicLabelUnavailable = true; this.topicLabelActivity('Generated labels unavailable; using statistical labels'); this.plugin.logDiagnostic?.(`Topic labeler unavailable: ${error?.message || error}`); return fallbackLabels; }
  }
  async semanticStarfield(query, files = null, focusFiles = []) {
    ...
    if (!this.starfieldCache || this.starfieldCache.signature !== signature) {
      ...
      const communitySizes = new Map(); for (const community of communities.values()) communitySizes.set(community, (communitySizes.get(community) || 0) + 1); let communityLabels = this.topicCommunityLabels(entries, communities, tuning.communityLabelSensitivity, tuning.communityMinSize); if (vaultMode) communityLabels = await this.generatedTopicCommunityLabels(entries, communities, communityLabels, tuning.communityMinSize); this.starfieldCache = { signature, entries, edges, positions, communities, topicAffinities, communitySizes, communityLabels, entitySets, uniqueness };
    }
