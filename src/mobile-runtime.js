...
function vaultThresholdLayout(nodes) { const values = nodes.filter(node => Number.isFinite(node.x) && Number.isFinite(node.y)); if (values.length !== nodes.length) return null; const meanX = values.reduce((sum, node) => sum + node.x, 0) / values.length, meanY = values.reduce((sum, node) => sum + node.y, 0) / values.length, extent = Math.max(.001, ...values.map(node => Math.hypot(node.x - meanX, node.y - meanY))), scale = .84 / extent; return new Map(values.map(node => [node.id, { x: (node.x - meanX) * scale, y: (node.y - meanY) * scale }])); }
