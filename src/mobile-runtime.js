...
export class MobileSearchRuntime {
  ...
  conceptFacetsFromVector(queryVector, files) {
    ...
    const labels = new Map(); for (const [community, members] of groups) { const counts = new Map(); for (const file of members) for (const entity of entitySets.get(file) || []) counts.set(entity, (counts.get(entity) || 0) + 1); const ranked = [...counts].map(([value, count]) => ({ value, score: count * Math.log(1 + entries.length / Math.max(1, entityFrequency.get(value) || 1)) })).sort((a, b) => b.score - a.score || a.value.localeCompare(b.value)); let label = ranked[0]?.value; if (!label) { const words = new Map(); for (const file of members) for (const token of tokens(basename(file))) if (!STOP_WORDS.has(token) && !GENERIC_CONCEPTS.has(token) && !/^\d+$/.test(token)) words.set(token, (words.get(token) || 0) + 1); label = [...words].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0]?.[0] || `Facet ${community + 1}`; } labels.set(community, String(label).replace(/\b\p{L}/gu, letter => letter.toUpperCase())); }
    return new Map(entries.map(entry => { const facet = communities.get(entry.id) ?? 0; return [entry.id, { facet, label: labels.get(facet) || `Facet ${facet + 1}` }]; }));
  }
  contextScores(files) {
