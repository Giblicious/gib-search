...
export class MobileSearchRuntime {
  ...
  async conceptFacets(query, files, evidence = new Map()) {
  ...
  conceptFacetsFromVector(queryVector, files, evidence = new Map(), query = '') {
    ...
    const candidateSets = new Map(), documentFrequency = new Map(); for (const file of ordered) { const candidates = new Map([...conceptLabelCandidates(evidence.get(file) || '', query), ...conceptLabelCandidates(basename(file), query, 'filename')].map(candidate => [candidate.key, candidate])); candidateSets.set(file, candidates); for (const key of candidates.keys()) documentFrequency.set(key, (documentFrequency.get(key) || 0) + 1); }
    const centroids = new Map(), labels = new Map(); for (const [community, members] of groups) { const centroid = new Float32Array(DIMENSION); for (const file of members) { const vector = entries.find(entry => entry.id === file)?.vector; if (vector) for (let dimension = 0; dimension < DIMENSION; dimension++) centroid[dimension] += vector[dimension]; } const norm = Math.sqrt(dot(centroid, centroid)) || 1; for (let dimension = 0; dimension < DIMENSION; dimension++) centroid[dimension] /= norm; centroids.set(community, centroid); const counts = new Map(), displays = new Map(); for (const file of members) for (const [key, candidate] of candidateSets.get(file) || []) { const value = counts.get(key) || { count: 0, quality: 0, novelty: 0 }; value.count++; value.quality += candidate.quality; value.novelty += candidate.novelty; counts.set(key, value); if (!displays.has(key)) displays.set(key, candidate.phrase); } const ranked = [...counts].map(([key, value]) => ({ value: displays.get(key), score: value.count * Math.log(1 + entries.length / Math.max(1, documentFrequency.get(key) || 1)) * (1 + value.quality / value.count * 2.2) * value.novelty / value.count })).sort((a, b) => b.score - a.score || a.value.localeCompare(b.value)); const best = ranked[0], runnerUp = ranked[1], labelConfidence = best ? Math.min(1, .34 + best.score / Math.max(.001, best.score + Number(runnerUp?.score || 0)) * .5 + Math.min(.16, members.length / entries.length * .3)) : 0; labels.set(community, { label: best ? String(best.value).replace(/\b\p{L}/gu, letter => letter.toUpperCase()) : '', confidence: labelConfidence }); }
