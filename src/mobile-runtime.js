...
export class MobileSearchRuntime {
  ...
  async semanticStarfield(query, files = null, focusFiles = []) {
    ...
    const tuning = Object.assign({ passageCoverage: .72, commonnessSuppression: .75, passageDiversity: .28 }, this.plugin.settings?.mapTuning || {}), signature = `${this.meta.length}:${this.vectors.length}:${vaultMode ? 'chunks' : 'files'}:${tuning.passageCoverage}:${tuning.commonnessSuppression}:${tuning.passageDiversity}:${requested.join('\0')}`;
    if (!this.starfieldCache || this.starfieldCache.signature !== signature) {
      ...
      this.starfieldCache = { signature, entries, edges, positions, communities, topicAffinities, entitySets, uniqueness };
    }
    ...
    if (!trimmed) { const topics = topicCoordinates(entries, basis.center, basis.axes); return { nodes: entries.map(entry => ({ id: entry.id, label: basename(entry.id), semanticScore: 0, uniqueness: uniqueness.get(entry.id) ?? .5, community: communities.get(entry.id) ?? 0, topicAffinities: topicAffinities.get(entry.id), entities: [...(entitySets.get(entry.id) || [])].slice(0, 8), ...(positions.get(entry.id) || {}), ...(topics.get(entry.id) || {}) })), edges: edges.map(edge => ({ ...edge, residualScore: 0 })) }; }
