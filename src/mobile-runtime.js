...
export class MobileSearchRuntime {
  ...
  async semanticStarfield(query, files = null, focusFiles = []) {
    ...
    if (!this.starfieldCache || this.starfieldCache.signature !== signature) {
      ...
      const neighbors = entries.map((entry, first) => entries.map((other, second) => first === second ? null : { index: second, id: other.id, score: vaultMode ? contentProfileSimilarity(profiles.get(entry.id), profiles.get(other.id), tuning.passageCoverage) : dot(entry.vector, other.vector), entityScore: entityAffinity(entry.id, other.id) }).filter(Boolean).sort((a, b) => b.score + b.entityScore * .08 - a.score - a.entityScore * .08).slice(0, 8)), directed = neighbors.map(list => { const floor = Number(list.at(-1)?.score || 0), ceiling = Number(list[0]?.score || 1), spread = Math.max(.02, ceiling - floor); return new Map(list.map(item => [item.index, { ...item, affinity: Math.max(.001, (item.score - floor) / spread) * .82 + item.entityScore * .18 }])); }), edges = [], seen = new Set(), incident = new Set();
