...
function isStructuralConceptLabel(source) { const words = cleanText(source).toLowerCase().match(/[\p{L}\p{N}]+/gu) || []; return !words.length || words.some(word => STRUCTURAL_TOPIC_LABELS.has(word) || /^(?:yyyy|yy|mm|dd)$/.test(word)) || words.length <= 2 && VAGUE_LABEL_WORDS.has(words[0]) || words.every(word => STOP_WORDS.has(word) || GENERIC_CONCEPTS.has(word) || VAGUE_LABEL_WORDS.has(word)); }
...
export class MobileSearchRuntime {
  ...
  conceptFacetsFromVector(queryVector, files, query = '') {
    ...
    const candidateSets = this.indexedConceptCandidates(queryVector, ordered, query), semanticCentroids = new Map(), centroids = new Map(); for (const [community, members] of groups) { const conceptual = new Float32Array(DIMENSION), semantic = new Float32Array(DIMENSION); for (const file of members) { const conceptVector = entries.find(entry => entry.id === file)?.vector, semanticVector = vectors.get(file)?.vector; if (conceptVector) for (let dimension = 0; dimension < DIMENSION; dimension++) conceptual[dimension] += conceptVector[dimension]; if (semanticVector) for (let dimension = 0; dimension < DIMENSION; dimension++) semantic[dimension] += semanticVector[dimension]; } const conceptNorm = Math.sqrt(dot(conceptual, conceptual)) || 1, semanticNorm = Math.sqrt(dot(semantic, semantic)) || 1; for (let dimension = 0; dimension < DIMENSION; dimension++) { conceptual[dimension] /= conceptNorm; semantic[dimension] /= semanticNorm; } centroids.set(community, conceptual); semanticCentroids.set(community, semantic); }
    const labels = new Map(); for (const [community, members] of groups) { const candidates = new Map(); for (const file of members) for (const [key, candidate] of candidateSets.get(file) || []) { const value = candidates.get(key) || { ...candidate, files: new Set() }; value.files.add(file); if (candidate.quality > value.quality) { value.phrase = candidate.phrase; value.quality = candidate.quality; value.vector = candidate.vector; value.novelty = candidate.novelty; } candidates.set(key, value); } const centroid = semanticCentroids.get(community), otherCentroids = [...semanticCentroids].filter(([id]) => id !== community).map(([, vector]) => vector), ranked = [...candidates.values()].map(candidate => { const centrality = Math.max(0, Math.min(1, (dotHighlight(centroid, candidate.vector) + 1) / 2)), memberCoverage = members.reduce((sum, file) => sum + Math.max(0, Math.min(1, (dotHighlight(vectors.get(file).vector, candidate.vector) + 1) / 2)), 0) / Math.max(1, members.length), other = otherCentroids.length ? Math.max(...otherCentroids.map(vector => (dotHighlight(vector, candidate.vector) + 1) / 2)) : centrality - .08, distinction = Math.max(0, Math.min(1, .5 + (centrality - other) * 2.5)), grounded = Math.sqrt(candidate.files.size / Math.max(1, members.length)), quality = Math.min(1, candidate.quality / .18), score = candidate.novelty * (centrality * .42 + memberCoverage * .25 + distinction * .2 + grounded * .08 + quality * .05); return { ...candidate, score, centrality, memberCoverage, distinction }; }).sort((a, b) => b.score - a.score || b.files.size - a.files.size || a.phrase.localeCompare(b.phrase)), best = ranked[0], runnerUp = ranked[1], margin = Number(best?.score || 0) - Number(runnerUp?.score || 0), supported = Number(best?.files?.size || 0) >= Math.min(2, members.length), confidence = best && supported && best.score >= .56 && (margin >= .018 || best.files.size > 1) ? Math.max(0, Math.min(1, (best.score - .46) * 1.7 + margin * 2 + Math.min(.14, best.files.size / members.length * .14))) : 0, fallbackLabel = best && supported && best.score >= .42 && !isTemporalConceptLabel(best.phrase) && String(best.phrase).split(/\s+/).length <= 4 ? String(best.phrase).replace(/\b\p{L}/gu, letter => letter.toUpperCase()) : ''; labels.set(community, { label: confidence >= .44 ? fallbackLabel : '', fallbackLabel, confidence }); }
  ...
  topicCommunityAnalysis(entries, communities, sensitivity = .58, minimumSize = 3) {
    ...
    const labels = new Map(), rankedByCommunity = new Map(), threshold = Math.max(.35, Math.min(.85, Number(sensitivity) || .58)), totalFiles = Math.max(2, requested.size), minimum = Math.max(2, Number(minimumSize) || 3);
    for (const [id, members] of groups) {
