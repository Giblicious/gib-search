...
export class MobileSearchRuntime {
  ...
  async conceptFacets(query, files) {
    const ordered = [...new Set((files || []).filter(Boolean))], vectors = this.fileVectors(ordered), queryVector = await this.queryVector(this.correctQuery(query)), entries = ordered.filter(id => vectors.has(id)).map(id => ({ id, vector: residualVector(vectors.get(id).vector, queryVector) })); if (!entries.length) return new Map();
  ...
  async multiRelationalLayout(query, nodes, relationships = new Map(), options = {}) {
    ...
    const basis = this.topicBasis(), trimmed = String(query || '').trim(); let center = trimmed ? await this.queryVector(this.correctQuery(trimmed)) : Float32Array.from(basis.center), centerNorm = Math.sqrt(dot(center, center)) || 1; if (!trimmed) for (let dimension = 0; dimension < center.length; dimension++) center[dimension] /= centerNorm;
