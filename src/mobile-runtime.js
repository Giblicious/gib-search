...
export class MobileSearchRuntime {
  ...
  async multiRelationalLayout(query, nodes, relationships = new Map(), options = {}) {
    ...
    const lens = options.magic === false ? 'relevance' : ['relevance', 'arguments', 'context'].includes(options.lens) ? options.lens : 'relevance', weights = vaultCentered ? { semantic: .18, residual: .74, entity: .08, angular: 0, community: 0, relation: 0 } : { relevance: { semantic: .26, residual: .34, entity: .08, angular: .12, community: .2, relation: 0 }, arguments: { semantic: .22, residual: .24, entity: .06, angular: .04, community: .08, relation: .36 }, context: { semantic: .4, residual: .18, entity: .12, angular: .06, community: .24, relation: 0 } }[lens]; if (conditioned) for (let index = 1; index < ids.length; index++) { const value = .06 + (1 - relevance.get(ids[index])) * (lens === 'relevance' ? .88 : .76); distances[0][index] = value; distances[index][0] = value; }
