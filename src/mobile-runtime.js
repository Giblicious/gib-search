...
export class MobileSearchRuntime {
  ...
  conceptFacetsFromVector(queryVector, files, evidence = new Map(), query = '') {
    const ordered = [...new Set((files || []).filter(Boolean))], vectors = this.fileVectors(ordered), entries = ordered.filter(id => vectors.has(id)).map(id => ({ id, vector: residualVector(vectors.get(id).vector, queryVector) })); if (!entries.length) return new Map();
  ...
  async multiRelationalLayout(query, nodes, relationships = new Map(), options = {}) {
    ...
    if (conditioned) {
      const positioned = new Map(); for (const entry of entries.filter(value => Number(nodeById.get(value.id)?.generation || 1) === 1)) { const point = layout.get(entry.id), topic = conceptTopics.get(entry.id), angle = point && Math.hypot(point.x, point.y) > .001 ? Math.atan2(point.y, point.x) : Number(topic?.topicAngle || stableAngle(entry.id)), radius = .08 + (1 - relevance.get(entry.id)) * .72; positioned.set(entry.id, { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius }); }
