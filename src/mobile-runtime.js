...
export class MobileSearchRuntime {
  ...
  async multiRelationalLayout(query, nodes, relationships = new Map(), options = {}) {
    ...
    const points = [...layout.values()], meanX = points.reduce((sum, point) => sum + point.x, 0) / Math.max(1, points.length), meanY = points.reduce((sum, point) => sum + point.y, 0) / Math.max(1, points.length), extent = Math.max(.001, ...points.map(point => Math.hypot(point.x - meanX, point.y - meanY))); for (const entry of entries) { const point = layout.get(entry.id), topic = topics.get(entry.id); if (!point || !topic) continue; const localX = (point.x - meanX) / extent * .82, localY = (point.y - meanY) / extent * .82, radius = .14 + Math.pow(Math.max(0, Math.min(1, Number(topic.topicStrength || 0))), .72) * .72, compassX = Math.cos(topic.topicAngle) * radius, compassY = Math.sin(topic.topicAngle) * radius; point.x = localX * .38 + compassX * .62; point.y = localY * .38 + compassY * .62; } const shaped = [...layout.values()], shapedMeanX = shaped.reduce((sum, point) => sum + point.x, 0) / Math.max(1, shaped.length), shapedMeanY = shaped.reduce((sum, point) => sum + point.y, 0) / Math.max(1, shaped.length), shapedExtent = Math.max(.001, ...shaped.map(point => Math.hypot(point.x - shapedMeanX, point.y - shapedMeanY))); for (const point of layout.values()) { point.x = (point.x - shapedMeanX) / shapedExtent * .88; point.y = (point.y - shapedMeanY) / shapedExtent * .88; } return layout;
  }
  semanticMap(files) {
