...
export class AtlasEngine {
  ...
  async prepareRelationshipField(state, files, roads = []) {
    const weights = { ...(state.frame?.relationships || {}) }, profiles = new Map(), requested = new Set(Object.entries(weights).filter(([signal, weight]) => !['semantic', 'links'].includes(signal) && Number(weight) > .001).map(([signal]) => signal)); if (state.frame?.appearance?.colorBy === 'emotion') requested.add('emotion');
  ...
  applyRelationshipPresentation(state, nodes, field) {
    ...
    for (const node of nodes) {
      const profile = signalProfiles?.get(node.id), emotionProfile = emotional?.get(node.id), definitions = preferredSignal === 'emotion' ? EMOTION_LANDMARKS : [], scores = profile?.scores || {}, ranked = Object.entries(scores).sort((a, b) => Number(b[1]) - Number(a[1])), strongest = ranked[0]; node.relationshipSignal = preferredSignal || 'semantic'; node.relationshipScores = scores; node.relationshipEvidence = profile?.evidence || null; node.relationshipIntensity = strongest ? Number(strongest[1] || 0) : Number(node.uniqueness ?? node.relevance ?? .5);
      if (colorBy === 'emotion' && emotionProfile) { const weighted = EMOTION_LANDMARKS.map(item => ({ item, score: Number(emotionProfile.scores?.[`emotion:${item.key}`] || 0) ** 2 })).filter(value => value.score > .015), x = weighted.reduce((sum, value) => sum + Math.cos(value.item.hue * Math.PI / 180) * value.score, 0), y = weighted.reduce((sum, value) => sum + Math.sin(value.item.hue * Math.PI / 180) * value.score, 0); if (weighted.length) node.topicHue = (Math.atan2(y, x) * 180 / Math.PI + 360) % 360; }
  ...
  async vaultScene(state, scope, options = {}) {
    const graph = await this.plugin.search.semanticStarfield('', scope.paths), scales = options.fileScales || fileScales(this.plugin.app), definition = atlasLens(state.lens), nodes = graph.nodes.map(node => ({ ...node, matched: false, generation: 1, relevance: .5, fileScale: scales.get(node.id) ?? .35, facet: node.community, conceptAffinities: node.topicAffinities })), roads = manualLinks(this.plugin.app, scope.paths), field = await this.prepareRelationshipField(state, nodes.map(node => node.id), roads); this.applyRelationshipPresentation(state, nodes, field); const naturalLayout = await this.plugin.search.multiRelationalLayout('', nodes, new Map(), { magic: this.plugin.settings.magicGraphEnabled, lens: definition.analysis, vaultCenter: true, mapMode: definition.mapMode, relationshipField: field }), guided = await this.applyGuidedFrame(state, nodes, naturalLayout, scope.paths), layout = guided.layout;
  ...
  async noteScene(state, scope, options = {}) {
    ...
    const scores = nodes.map(node => Number(node.score || 0)), low = scores.length ? Math.min(...scores) : 0, high = scores.length ? Math.max(...scores) : 1, spread = Math.max(.001, high - low), scales = options.fileScales || fileScales(this.plugin.app); nodes = nodes.map(node => ({ ...node, relevance: (Number(node.score || 0) - low) / spread, matched: true, generation: 1, fileScale: scales.get(node.id) ?? .35 })); const roads = manualLinks(this.plugin.app, [filePath, ...nodes.map(node => node.id)]), field = await this.prepareRelationshipField(state, nodes.map(node => node.id), roads); this.applyRelationshipPresentation(state, nodes, field); const naturalLayout = await this.plugin.search.multiRelationalLayout(basename(filePath), nodes, relationships, { lens: definition.analysis, magic: this.plugin.settings.magicGraphEnabled, mapMode: definition.mapMode, centerFile: filePath, relationshipField: field }), guided = await this.applyGuidedFrame(state, nodes, naturalLayout, scope.paths), layout = guided.layout;
    for (const node of nodes) { const target = layout.get(node.id); if (target) { node.layoutX = target.x; node.layoutY = target.y; } }
