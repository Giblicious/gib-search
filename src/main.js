...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setGraph(center, values, edges = []) {
    ...
    this.nodes = values.map((value, order) => {
      ...
      return { ...value, matched, order, fileScale, relevance: old?.relevance ?? relevance, targetRelevance: relevance, visibility: old?.visibility ?? 0, targetVisibility: hasQuery ? (matched ? .72 + relevance * .28 : .08) : targetVisibility, accent: old?.accent ?? 0, targetAccent: hasQuery && matched ? .3 + relevance * .7 : 0, depth: old?.depth ?? 0, targetDepth, x: old?.x ?? (Number.isFinite(value.x) ? value.x : Math.cos(angle) * (.18 + ((order % 17) / 17) * .64)), y: old?.y ?? (Number.isFinite(value.y) ? value.y : Math.sin(angle) * (.18 + ((order % 17) / 17) * .64)), vx: old?.vx || 0, vy: old?.vy || 0 };
    });
    const overallScores = edges.map(edge => Number(edge.score || 0)), overallLow = overallScores.length ? Math.min(...overallScores) : 0, overallHigh = overallScores.length ? Math.max(...overallScores) : 1, overallSpread = Math.max(.001, overallHigh - overallLow); this.activeEdges = edges.map(edge => { const overall = (Number(edge.score || overallLow) - overallLow) / overallSpread, residual = Math.max(-1, Math.min(1, Number(edge.residualScore || 0))); return { ...edge, overall, residual, strength: overall * (.65 + Math.max(0, residual) * .35) }; }).filter(edge => edge.strength > .05); this.relationships = new Map(this.activeEdges.map(edge => [mapEdgeKey(edge.source, edge.target), edge]));
    this.byId = new Map(this.nodes.map(node => [node.id, node])); this.selected = this.byId.has(this.selected) ? this.selected : null; this.hovered = this.byId.has(this.hovered) || this.hovered === '__query__' ? this.hovered : null; this.statusEl.textContent = hasQuery ? `${Number(center.resultCount || 0)} results · ${this.nodes.length} notes` : `${this.nodes.length} notes`; this.startSimulation(previous.size ? .82 : 1); this.updateDetail();
  }
  beginQuery(label) { if (!this.queryNode || this.hasQuery) return; this.hasQuery = true; this.center = { ...(this.center || {}), label, hasQuery: true, resultCount: 0 }; this.queryNode.label = label; this.targetQueryPresence = 1; for (const node of this.nodes) { node.matched = false; node.targetDepth = -1; node.targetVisibility = .08; node.targetAccent = 0; } this.startSimulation(.7); }
  endQuery() { if (!this.queryNode || !this.hasQuery) return; this.hasQuery = false; this.center = { ...(this.center || {}), label: 'Search', hasQuery: false, resultCount: 0 }; this.targetQueryPresence = 0; for (const node of this.nodes) { node.matched = false; node.targetDepth = 0; node.targetVisibility = .38 + node.fileScale * .42; node.targetAccent = 0; } this.startSimulation(.72); }
  physicsStep(alpha) {
    this.queryPresence += (this.targetQueryPresence - this.queryPresence) * .075; for (const node of this.nodes) { node.relevance += (node.targetRelevance - node.relevance) * .075; node.visibility += (node.targetVisibility - node.visibility) * .09; node.accent += (node.targetAccent - node.accent) * .09; node.depth += (node.targetDepth - node.depth) * .075; }
    for (const edge of this.activeEdges || []) { const a = this.byId.get(edge.source), b = this.byId.get(edge.target); if (!a || !b || this.hasQuery && a.matched !== b.matched) continue; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const layerStrength = !this.hasQuery ? 1.35 : a.matched ? 1.25 : .28, desired = .075 + (1 - edge.overall) * .32 + Math.max(0, -edge.residual) * .09, spring = (distance - desired) * (.0025 + edge.strength * .009) * alpha * layerStrength, contrast = -Math.max(0, -edge.residual) * .004 * alpha * layerStrength, force = spring + contrast; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second]; if (this.hasQuery && a.matched !== b.matched) continue; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const layerStrength = !this.hasQuery ? 1 : a.matched ? 1 : .18, collisionDistance = .035 + (a.fileScale + b.fileScale) * .022, collision = distance < collisionDistance ? -(collisionDistance - distance) * .16 * alpha : 0, charge = -Math.min(.0028, .000018 / (distance * distance)) * alpha, force = (collision + charge) * layerStrength; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    const query = this.queryNode, foreground = this.nodes.filter(node => node.matched); if (this.queryPresence > .01) for (const node of foreground) { let dx = node.x - query.x, dy = node.y - query.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .07 + (1 - node.relevance) * .48, force = (distance - desired) * (.003 + node.relevance * .011) * alpha * this.queryPresence; if (query !== this.dragging) { query.vx += dx * force * .24; query.vy += dy * force * .24; } if (node !== this.dragging) { node.vx -= dx * force; node.vy -= dy * force; } }
    const bodies = this.queryPresence > .04 ? [query, ...this.nodes] : this.nodes; for (const body of bodies) { if (body === this.dragging) { body.vx = body.vy = 0; continue; } body.vx *= .9; body.vy *= .9; body.x += body.vx; body.y += body.vy; } if (!this.dragging && !this.hasQuery && bodies.length) { const centerX = bodies.reduce((sum, body) => sum + body.x, 0) / bodies.length, centerY = bodies.reduce((sum, body) => sum + body.y, 0) / bodies.length; for (const body of bodies) { body.x -= centerX * .035; body.y -= centerY * .035; } }
    const cameraBodies = this.hasQuery && foreground.length ? [query, ...foreground] : this.nodes, targetX = this.hasQuery && cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.x, 0) / cameraBodies.length : 0, targetY = this.hasQuery && cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.y, 0) / cameraBodies.length : 0, extent = this.hasQuery && cameraBodies.length ? Math.max(.16, ...cameraBodies.map(body => Math.hypot(body.x - targetX, body.y - targetY))) : .8, targetZoom = this.hasQuery ? Math.max(1.12, Math.min(2.15, .7 / extent)) : 1; this.cameraX += (targetX - this.cameraX) * .055; this.cameraY += (targetY - this.cameraY) * .055; this.cameraZoom += (targetZoom - this.cameraZoom) * .05;
  }
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = () => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); else { this.queryPresence = this.targetQueryPresence; this.cameraZoom = this.hasQuery ? 1.35 : 1; for (const node of this.nodes) { node.relevance = node.targetRelevance; node.visibility = node.targetVisibility; node.accent = node.targetAccent; node.depth = node.targetDepth; } } this.alpha *= .986; this.draw(); const transitioning = Math.abs(this.queryPresence - this.targetQueryPresence) > .01 || Math.abs(this.cameraZoom - (this.hasQuery ? Math.max(1.12, this.cameraZoom) : 1)) > .01 || this.nodes.some(node => Math.abs(node.depth - node.targetDepth) > .015); if (this.alpha > .01 || this.dragging || transitioning) this.simulationFrame = requestAnimationFrame(tick); }; this.simulationFrame = requestAnimationFrame(tick); }
  coordinates(node, width, height) { const scale = Math.max(20, Math.min(width, height) / 2 - 68), background = !node.isQuery && node.depth < 0, depthScale = node.isQuery ? 1 : 1 + node.depth * .1, cameraInfluence = background ? .28 : 1, parallaxX = background ? this.parallaxX * 5 * -node.depth : 0, parallaxY = background ? this.parallaxY * 5 * -node.depth : 0; return [width / 2 + (Number(node.x) - this.cameraX * cameraInfluence) * scale * this.cameraZoom * depthScale + parallaxX, height / 2 + (Number(node.y) - this.cameraY * cameraInfluence) * scale * this.cameraZoom * depthScale + parallaxY]; }
  draw() {
    const rect = this.canvas.getBoundingClientRect(); if (!rect.width || !rect.height) return; const dpr = window.devicePixelRatio || 1, pixelWidth = Math.round(rect.width * dpr), pixelHeight = Math.round(rect.height * dpr); if (this.canvas.width !== pixelWidth) this.canvas.width = pixelWidth; if (this.canvas.height !== pixelHeight) this.canvas.height = pixelHeight; const ctx = this.canvas.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, rect.width, rect.height); const colors = this.colors(), focused = this.hovered || this.selected, queryFocused = focused === '__query__', labelNodes = new Set(); this.hit = [];
    const rankedSource = this.hasQuery ? this.nodes.filter(node => node.matched) : this.nodes, ranked = [...rankedSource].sort((a, b) => this.targetQueryPresence ? b.relevance - a.relevance : b.fileScale - a.fileScale); for (const node of ranked.slice(0, this.targetQueryPresence ? 7 : 5)) labelNodes.add(node.id); if (focused && !queryFocused) { labelNodes.add(focused); const related = (this.activeEdges || []).filter(edge => edge.source === focused || edge.target === focused).sort((a, b) => b.overall - a.overall).slice(0, 5); for (const edge of related) labelNodes.add(edge.source === focused ? edge.target : edge.source); }
    const labels = [], background = this.hasQuery ? this.nodes.filter(node => !node.matched) : [], foreground = this.hasQuery ? this.nodes.filter(node => node.matched) : this.nodes, renderNodes = (nodes, isBackground) => { ctx.filter = isBackground ? `blur(${Math.max(0, this.queryPresence * 1.45).toFixed(2)}px)` : 'none'; for (const node of nodes) { const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), depthScale = 1 + node.depth * .13, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * depthScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(isBackground ? .018 : .06, node.visibility * focusAlpha); ctx.globalAlpha = opacity; ctx.fillStyle = isBackground ? colors.faint : colors.normal; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (!isBackground && (node.accent > .01 || active)) { ctx.globalAlpha = opacity * Math.max(node.accent, active ? 1 : 0); ctx.fillStyle = colors.accent; ctx.beginPath(); ctx.arc(x, y, radius + (active ? 1 : 0), 0, Math.PI * 2); ctx.fill(); } if (!isBackground && labelNodes.has(node.id)) labels.push({ node, x, y, radius, active, opacity }); if (!isBackground) this.hit.push({ node, x, y, radius: radius + 9 }); } }; renderNodes(background, true); renderNodes(foreground, false); ctx.filter = 'none';
    this.drawLabels(ctx, labels, colors, rect.width, rect.height); if (this.queryPresence > .02) { const [queryX, queryY] = this.coordinates(this.queryNode, rect.width, rect.height); ctx.globalAlpha = this.queryPresence; ctx.fillStyle = colors.background; ctx.strokeStyle = colors.accent; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(queryX, queryY, 9, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); this.label(ctx, this.center?.label || '', queryX, queryY + 27, colors, true, true); this.hit.push({ node: this.queryNode, x: queryX, y: queryY, radius: 17 }); } ctx.globalAlpha = 1;
  }
  drawLabels(ctx, labels, colors, width, height) { const occupied = []; for (const item of labels.sort((a, b) => Number(b.active) - Number(a.active) || b.node.relevance - a.node.relevance)) { const text = String(item.node.label || ''), clipped = text.length > 28 ? `${text.slice(0, 27)}…` : text; ctx.font = `${item.active ? 600 : 500} 11px -apple-system, BlinkMacSystemFont, sans-serif`; const textWidth = ctx.measureText(clipped).width, candidates = [[item.x + item.radius + 5, item.y + 4, 'left'], [item.x - item.radius - 5, item.y + 4, 'right'], [item.x, item.y - item.radius - 7, 'center'], [item.x, item.y + item.radius + 14, 'center']]; let chosen = null; for (const [x, y, align] of candidates) { const left = align === 'left' ? x : align === 'right' ? x - textWidth : x - textWidth / 2, box = { left: left - 2, right: left + textWidth + 2, top: y - 11, bottom: y + 3 }; if (box.left < 4 || box.right > width - 4 || box.top < 4 || box.bottom > height - 4 || occupied.some(other => box.left < other.right && box.right > other.left && box.top < other.bottom && box.bottom > other.top)) continue; chosen = { x, y, align, box }; break; } if (!chosen) continue; occupied.push(chosen.box); ctx.globalAlpha = Math.max(.35, item.opacity); ctx.fillStyle = item.active ? colors.normal : colors.muted; ctx.textAlign = chosen.align; ctx.fillText(clipped, chosen.x, chosen.y); } }
  ...
  pointerMove(event) { const rect = this.canvas.getBoundingClientRect(); this.parallaxX = (event.clientX - rect.left) / Math.max(1, rect.width) - .5; this.parallaxY = (event.clientY - rect.top) / Math.max(1, rect.height) - .5; if (this.dragging) { const scale = Math.max(20, Math.min(rect.width, rect.height) / 2 - 68) * this.cameraZoom; this.dragging.x = (event.clientX - rect.left - rect.width / 2) / scale + this.cameraX; this.dragging.y = (event.clientY - rect.top - rect.height / 2) / scale + this.cameraY; this.dragging.vx = this.dragging.vy = 0; this.startSimulation(.38); return; } const node = this.hitAt(event); this.canvas.style.cursor = node ? 'grab' : 'default'; this.setHover(node?.id || null, true); this.draw(); }
  pointerUp(event) { if (!this.dragging) return; this.dragging = null; this.canvas.style.cursor = 'default'; this.canvas.releasePointerCapture?.(event.pointerId); this.startSimulation(.62); }
...
class SemanticSearchModal extends SuggestModal {
  constructor(app, plugin, filePath = null) {
    super(app); this.plugin = plugin; this.filePath = filePath; this.debounceTimer = null; this.searchVersion = 0; this.lastResults = []; this.mapResults = []; this.lastQuery = ''; this.visibleLimit = 0; this.canLoadMore = false; this.navigationHandler = null; this.map = null; this.mapVersion = 0;
  ...
  getSuggestions(query) {
    if (!query || query.trim().length < 2) { clearTimeout(this.debounceTimer); this.searchVersion++; const changed = Boolean(this.lastQuery); this.lastQuery = ''; this.lastResults = []; this.mapResults = []; if (changed) { this.map?.endQuery(); window.setTimeout(() => this.updateMap(), 0); } return []; }
  ...
  async updateMap() {
    if (!this.map || !this.plugin.settings.searchMapEnabled) return; const version = ++this.mapVersion, query = this.lastQuery, results = this.mapResults.slice(0, 80); this.map.setTitle('Semantic map');
    ...
    const graph = await this.plugin.search.semanticStarfield(query, paths, results.map(result => result.file)); if (version !== this.mapVersion) return; const byFile = new Map(results.map(result => [result.file, result])), rankingScores = results.map(result => Number(result.score || 0)), rankingLow = rankingScores.length ? Math.min(...rankingScores) : 0, rankingHigh = rankingScores.length ? Math.max(...rankingScores) : 1, rankingSpread = Math.max(.001, rankingHigh - rankingLow), semanticScores = graph.nodes.map(node => Number(node.semanticScore || 0)), semanticLow = semanticScores.length ? Math.min(...semanticScores) : 0, semanticHigh = semanticScores.length ? Math.max(...semanticScores) : 1, semanticSpread = Math.max(.001, semanticHigh - semanticLow);
    this.map.setGraph({ label: query || 'Search', hasQuery: Boolean(query), resultCount: results.length }, graph.nodes.map(node => { const result = byFile.get(node.id), semanticRelevance = (Number(node.semanticScore || 0) - semanticLow) / semanticSpread, rankedRelevance = result ? (Number(result.score || 0) - rankingLow) / rankingSpread : 0; return { ...node, matched: Boolean(result), relevance: query ? (result ? .72 + rankedRelevance * .28 : semanticRelevance * .62) : .5, fileScale: fileScale.get(node.id) ?? .35 }; }), graph.edges || []);
  }
