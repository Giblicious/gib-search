...
function buildSemanticEnvelopes(query, nodes, edges, size = 190) {
  const bodies = [query, ...nodes]; if (!bodies.length) return null; const extent = Math.min(1.12, Math.max(.36, ...bodies.map(body => Math.max(Math.abs(body.x), Math.abs(body.y)) + .19))), byId = new Map(nodes.map(node => [node.id, node])), bands = [];
  for (let band = 0; band < 6; band++) { const threshold = band / 6, eligible = nodes.filter(node => node.relevance >= threshold), eligibleIds = new Set(eligible.map(node => node.id)), points = [query, ...eligible], connections = (edges || []).filter(edge => edge.residual > 0 && eligibleIds.has(edge.source) && eligibleIds.has(edge.target)).map(edge => ({ source: byId.get(edge.source), target: byId.get(edge.target), width: (.045 + edge.strength * .04) * (1 - band * .055) })), radius = .145 - band * .009, field = new Float32Array(size * size); for (let row = 0; row < size; row++) for (let column = 0; column < size; column++) { const x = (column / (size - 1) * 2 - 1) * extent, y = (row / (size - 1) * 2 - 1) * extent; let signedDistance = -Infinity; for (const point of points) signedDistance = Math.max(signedDistance, radius - Math.hypot(x - point.x, y - point.y)); for (const connection of connections) signedDistance = Math.max(signedDistance, connection.width - pointSegmentDistance(x, y, connection.source, connection.target)); field[row * size + column] = signedDistance; } bands.push(field); }
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  drawContours(ctx, width, height, colors) {
    ...
    for (let band = 0; band < bands.length; band++) { const field = bands[band], outer = band === 0; ctx.beginPath(); for (let row = 0; row < size - 1; row++) for (let column = 0; column < size - 1; column++) { const corners = [field[row * size + column], field[row * size + column + 1], field[(row + 1) * size + column + 1], field[(row + 1) * size + column]], crossings = []; const cross = (first, second, x1, y1, x2, y2) => { if ((first < 0) === (second < 0) || first === second) return; const amount = -first / (second - first); crossings.push(toCanvas(x1 + (x2 - x1) * amount, y1 + (y2 - y1) * amount)); }; cross(corners[0], corners[1], column, row, column + 1, row); cross(corners[1], corners[2], column + 1, row, column + 1, row + 1); cross(corners[2], corners[3], column + 1, row + 1, column, row + 1); cross(corners[3], corners[0], column, row + 1, column, row); if (crossings.length === 2) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); } else if (crossings.length === 4) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); ctx.moveTo(crossings[2][0], crossings[2][1]); ctx.lineTo(crossings[3][0], crossings[3][1]); } } ctx.globalAlpha = outer ? .2 : .31; ctx.strokeStyle = outer ? colors.muted : colors.normal; ctx.lineWidth = outer ? .76 : 1.02; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
