...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  applyRelationships(details) { if (!(details instanceof Map) || !details.size) return; this.activeEdges = (this.activeEdges || []).map(edge => { const relation = details.get(mapEdgeKey(edge.source, edge.target)); if (!relation) return edge; const support = relation.type === 'support', contrast = relation.type === 'contrast'; return { ...edge, relation, residual: support ? Math.max(edge.residual, .58 + relation.confidence * .24) : contrast ? Math.min(edge.residual, -.58 - relation.confidence * .24) : edge.residual, strength: support ? edge.strength * (1.18 + relation.confidence * .22) : edge.strength }; }); this.relationships = new Map(this.activeEdges.map(edge => [mapEdgeKey(edge.source, edge.target), edge])); this.startSimulation(.78); }
  ...
  setUserZoom(value, x = null, y = null) { const rect = this.canvas.getBoundingClientRect(), previous = this.userZoom, next = Math.max(.45, Math.min(4.5, Number(value) || 1)); if (!rect.width || !rect.height || Math.abs(next - previous) < .0001) return; const relativeX = (x ?? rect.width / 2) - rect.width / 2, relativeY = (y ?? rect.height / 2) - rect.height / 2, factor = next / previous; this.panX = relativeX - (relativeX - this.panX) * factor; this.panY = relativeY - (relativeY - this.panY) * factor; this.userZoom = next; this.draw(); }
  zoomBy(factor) { this.setUserZoom(this.userZoom * factor); }
  resetViewport() { this.userZoom = 1; this.panX = 0; this.panY = 0; this.draw(); }
  ...
  setGraph(center, values, edges = []) {
    ...
    this.nodes = values.map((value, order) => {
      ...
      return { ...value, matched, generation, order, fileScale, relevance: old?.relevance ?? relevance, targetRelevance: relevance, visibility: old?.visibility ?? 0, targetVisibility: hasQuery ? (matched ? generationVisibility : 0) : targetVisibility, accent: old?.accent ?? 0, targetAccent: hasQuery && matched && generation === 1 ? .3 + relevance * .7 : 0, x: old?.x ?? (Number.isFinite(value.x) ? value.x : Math.cos(angle) * (.18 + ((order % 17) / 17) * .64)), y: old?.y ?? (Number.isFinite(value.y) ? value.y : Math.sin(angle) * (.18 + ((order % 17) / 17) * .64)), vx: old?.vx || 0, vy: old?.vy || 0 };
    });
  ...
  physicsStep(alpha) {
    this.queryPresence += (this.targetQueryPresence - this.queryPresence) * .075; for (const node of this.nodes) { node.relevance += (node.targetRelevance - node.relevance) * .075; node.visibility += (node.targetVisibility - node.visibility) * .09; node.accent += (node.targetAccent - node.accent) * .09; }
    for (const edge of this.activeEdges || []) { const a = this.byId.get(edge.source), b = this.byId.get(edge.target); if (!a || !b || this.hasQuery && !this.pendingQuery && (!a.matched || !b.matched)) continue; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const layerStrength = !this.hasQuery || this.pendingQuery ? 1.45 : 1.55, residualDistance = (1 - edge.residual) * .11, desired = .045 + (1 - edge.overall) * .34 + residualDistance, spring = (distance - desired) * (.0038 + edge.strength * .017) * alpha * layerStrength, contrast = -Math.max(0, -edge.residual) * .009 * alpha * layerStrength, force = spring + contrast; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second]; if (this.hasQuery && !this.pendingQuery && (!a.matched || !b.matched)) continue; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const collisionDistance = .035 + (a.fileScale + b.fileScale) * .022, collision = distance < collisionDistance ? -(collisionDistance - distance) * .19 * alpha : 0, charge = -Math.min(.0042, .00003 / (distance * distance)) * alpha, force = collision + charge; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    const query = this.queryNode, foreground = this.nodes.filter(node => node.matched); if (this.queryPresence > .01 && this.pendingQuery) { for (const node of this.nodes) { let dx = node.x - query.x, dy = node.y - query.y; const distance = Math.max(.035, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const force = Math.min(.012, .00034 / (distance * distance)) * alpha * this.queryPresence; if (query !== this.dragging) { query.vx -= dx * force * .08; query.vy -= dy * force * .08; } if (node !== this.dragging) { node.vx += dx * force; node.vy += dy * force; } } if (query !== this.dragging && this.nodes.length) { const centerX = this.nodes.reduce((sum, node) => sum + node.x, 0) / this.nodes.length, centerY = this.nodes.reduce((sum, node) => sum + node.y, 0) / this.nodes.length; query.vx += (centerX - query.x) * .035 * alpha; query.vy += (centerY - query.y) * .035 * alpha; } } else if (this.queryPresence > .01) for (const node of foreground.filter(value => value.generation === 1)) { let dx = node.x - query.x, dy = node.y - query.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .065 + (1 - node.relevance) * .58, force = (distance - desired) * (.0028 + node.relevance * .0085) * alpha * this.queryPresence; if (query !== this.dragging) { query.vx += dx * force * .18; query.vy += dy * force * .18; } if (node !== this.dragging) { node.vx -= dx * force; node.vy -= dy * force; } }
    if (this.options.magicGraph !== false) { const compassNodes = this.hasQuery && !this.pendingQuery ? foreground : this.nodes, originX = this.hasQuery ? query.x : compassNodes.reduce((sum, node) => sum + node.x, 0) / Math.max(1, compassNodes.length), originY = this.hasQuery ? query.y : compassNodes.reduce((sum, node) => sum + node.y, 0) / Math.max(1, compassNodes.length); for (const node of compassNodes) { if (node === this.dragging || !Number.isFinite(node.topicAngle)) continue; const radius = this.hasQuery ? .075 + (1 - node.relevance) * .56 : .2 + Number(node.topicStrength || 0) * .48, targetX = originX + Math.cos(node.topicAngle) * radius, targetY = originY + Math.sin(node.topicAngle) * radius, strength = (.0028 + Number(node.topicStrength || 0) * .0042) * alpha; node.vx += (targetX - node.x) * strength; node.vy += (targetY - node.y) * strength; } }
    const corralBodies = this.hasQuery && !this.pendingQuery ? [query, ...foreground] : this.queryPresence > .04 ? [query, ...this.nodes] : this.nodes; if (corralBodies.length) { const centerX = corralBodies.reduce((sum, body) => sum + body.x, 0) / corralBodies.length, centerY = corralBodies.reduce((sum, body) => sum + body.y, 0) / corralBodies.length, radius = this.hasQuery && !this.pendingQuery ? .74 : .88; for (const body of corralBodies) { if (body === this.dragging) continue; let dx = body.x - centerX, dy = body.y - centerY; const distance = Math.max(.001, Math.hypot(dx, dy)); if (distance <= radius) continue; dx /= distance; dy /= distance; const force = (distance - radius) * .055 * alpha; body.vx -= dx * force; body.vy -= dy * force; } }
    const bodies = this.queryPresence > .04 ? [query, ...this.nodes] : this.nodes; for (const body of bodies) { if (body === this.dragging) { body.vx = body.vy = 0; continue; } body.vx *= .89; body.vy *= .89; body.x += body.vx; body.y += body.vy; } if (!this.dragging && !this.hasQuery && bodies.length) { const centerX = bodies.reduce((sum, body) => sum + body.x, 0) / bodies.length, centerY = bodies.reduce((sum, body) => sum + body.y, 0) / bodies.length; for (const body of bodies) { body.x -= centerX * .035; body.y -= centerY * .035; } }
  ...
  draw() {
    ...
    const rankedSource = this.hasQuery && !this.pendingQuery ? this.nodes.filter(node => node.matched) : this.nodes, ranked = [...rankedSource].sort((a, b) => this.targetQueryPresence && !this.pendingQuery ? b.relevance - a.relevance : b.fileScale - a.fileScale); for (const node of ranked.slice(0, this.targetQueryPresence && !this.pendingQuery ? 7 : 5)) labelNodes.add(node.id); if (focused && !queryFocused) { labelNodes.add(focused); const related = (this.activeEdges || []).filter(edge => edge.source === focused || edge.target === focused).sort((a, b) => b.overall - a.overall).slice(0, 5); for (const edge of related) labelNodes.add(edge.source === focused ? edge.target : edge.source); }
    const labels = [], ordered = this.hasQuery && !this.pendingQuery ? [...this.nodes.filter(node => !node.matched), ...this.nodes.filter(node => node.matched)] : this.nodes; for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.01, node.visibility * focusAlpha), semanticColor = this.options.semanticColors !== false && Number.isFinite(node.topicHue) ? `hsl(${node.topicHue} 54% 64%)` : colors.normal; ctx.globalAlpha = opacity; ctx.fillStyle = node.matched || !this.hasQuery || this.pendingQuery ? semanticColor : colors.faint; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (node.matched && (node.accent > .01 || active)) { ctx.globalAlpha = opacity * Math.max(node.accent, active ? 1 : 0); ctx.strokeStyle = colors.accent; ctx.lineWidth = active ? 2 : 1; ctx.beginPath(); ctx.arc(x, y, radius + 1.5, 0, Math.PI * 2); ctx.stroke(); } if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery) this.hit.push({ node, x, y, radius: radius + 9 }); }
...
class SemanticSearchModal extends SuggestModal {
  ...
  async updateMap() {
    if (!this.map || !this.plugin.settings.searchMapEnabled) return; const version = ++this.mapVersion, query = this.lastQuery, results = this.lastResults.slice(0, 80); this.map.setTitle('Semantic map');
    ...
    this.map.setGraph({ label: query || 'Search', hasQuery: Boolean(query), resultCount: results.length }, graph.nodes.map(node => { const result = byFile.get(node.id), generation = generationByFile.get(node.id), semanticRelevance = (Number(node.semanticScore || 0) - semanticLow) / semanticSpread, rankedRelevance = result ? (Number(result.score || 0) - rankingLow) / rankingSpread : 0, expandedRelevance = generation && generation.generation > 1 ? Math.max(.22, Math.min(.62, Number(generation.relationScore || 0))) : semanticRelevance; return { ...node, generation: generation?.generation || 1, matched: Boolean(generation), relevance: query ? (result ? .08 + rankedRelevance * .92 : expandedRelevance) : .5, fileScale: fileScale.get(node.id) ?? .35 }; }), combinedEdges);
    if (query && this.plugin.settings.graphRelationshipIntelligence && combinedEdges.length) { const budget = this.plugin.isMobile ? this.plugin.settings.graphRelationshipBudgetMobile : this.plugin.settings.graphRelationshipBudgetDesktop; await new Promise(resolve => window.setTimeout(resolve, 320)); if (version !== this.mapVersion) return; this.map.setIntelligenceStatus('Reading relationships…'); try { const details = await this.plugin.search.graphRelationships(combinedEdges, budget); if (version !== this.mapVersion) return; this.map.applyRelationships(details); this.map.setIntelligenceStatus(''); } catch (error) { if (version === this.mapVersion) { this.map.setIntelligenceStatus('Relationship model unavailable'); this.plugin.logDiagnostic(`Relationship intelligence failed: ${error.message}`, true); } } }
  }
...
class SearchSettings extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(this.containerEl).setName('Magic graph intelligence').setDesc('Use stable topic directions, entity-aware neighborhoods, and query-conditioned physics to shape the semantic map.').addToggle(t => t.setValue(this.plugin.settings.magicGraphEnabled).onChange(async value => { this.plugin.settings.magicGraphEnabled = value; await this.plugin.save(); }));
