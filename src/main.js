...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setGraph(center, values, edges = []) {
    ...
    this.nodes = values.map((value, order) => {
      ...
      return { ...value, order, fileScale, relevance: old?.relevance ?? relevance, targetRelevance: relevance, visibility: old?.visibility ?? 0, targetVisibility, accent: old?.accent ?? 0, targetAccent: hasQuery && value.matched ? .3 + relevance * .7 : 0, x: old?.x ?? Math.cos(angle) * (.18 + ((order % 17) / 17) * .64), y: old?.y ?? Math.sin(angle) * (.18 + ((order % 17) / 17) * .64), vx: old?.vx || 0, vy: old?.vy || 0 };
    });
  ...
  physicsStep(alpha) {
    ...
    for (const edge of this.activeEdges || []) { const a = this.byId.get(edge.source), b = this.byId.get(edge.target); if (!a || !b) continue; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .1 + (1 - edge.overall) * .31 + Math.max(0, -edge.residual) * .09, spring = (distance - desired) * (.0015 + edge.strength * .005) * alpha, contrast = -Math.max(0, -edge.residual) * .0035 * alpha, force = spring + contrast; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second]; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); if (distance > .22) continue; dx /= distance; dy /= distance; const collisionDistance = .035 + (a.fileScale + b.fileScale) * .022, collision = distance < collisionDistance ? -(collisionDistance - distance) * .16 * alpha : 0, charge = -Math.min(.0025, .000025 / (distance * distance)) * alpha, force = collision + charge; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    const query = this.queryNode; if (this.queryPresence > .01) for (const node of this.nodes) { let dx = node.x - query.x, dy = node.y - query.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .09 + (1 - node.relevance) * .72, force = (distance - desired) * (.0018 + node.relevance * .008) * alpha * this.queryPresence; if (query !== this.dragging) { query.vx += dx * force * .2; query.vy += dy * force * .2; } if (node !== this.dragging) { node.vx -= dx * force; node.vy -= dy * force; } }
    const bodies = this.queryPresence > .04 ? [query, ...this.nodes] : this.nodes; for (const body of bodies) { if (body === this.dragging) { body.vx = body.vy = 0; continue; } body.vx *= .9; body.vy *= .9; body.x += body.vx; body.y += body.vy; } if (!this.dragging && bodies.length) { const weight = body => body.isQuery ? Math.max(.1, this.queryPresence) : .35 + body.visibility, total = bodies.reduce((sum, body) => sum + weight(body), 0), centerX = bodies.reduce((sum, body) => sum + body.x * weight(body), 0) / total, centerY = bodies.reduce((sum, body) => sum + body.y * weight(body), 0) / total; for (const body of bodies) { body.x -= centerX * .045; body.y -= centerY * .045; } const extent = Math.max(.001, ...bodies.map(body => Math.hypot(body.x, body.y))); if (extent > .96) for (const body of bodies) { body.x *= .96 / extent; body.y *= .96 / extent; body.vx *= .96 / extent; body.vy *= .96 / extent; } }
  }
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = () => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); else { this.queryPresence = this.targetQueryPresence; for (const node of this.nodes) { node.relevance = node.targetRelevance; node.visibility = node.targetVisibility; node.accent = node.targetAccent; } } this.alpha *= .986; this.draw(); if (this.alpha > .01 || this.dragging || Math.abs(this.queryPresence - this.targetQueryPresence) > .01) this.simulationFrame = requestAnimationFrame(tick); }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  draw() {
    ...
    const ranked = [...this.nodes].sort((a, b) => this.targetQueryPresence ? b.relevance - a.relevance : b.fileScale - a.fileScale); for (const node of ranked.slice(0, this.targetQueryPresence ? 6 : 4)) labelNodes.add(node.id); if (focused && !queryFocused) { labelNodes.add(focused); const related = (this.activeEdges || []).filter(edge => edge.source === focused || edge.target === focused).sort((a, b) => b.overall - a.overall).slice(0, 5); for (const edge of related) labelNodes.add(edge.source === focused ? edge.target : edge.source); }
    const labels = []; for (const node of this.nodes) { const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), radius = 2 + node.fileScale * 4.4 + (active ? 1.5 : 0), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.06, node.visibility * focusAlpha); ctx.globalAlpha = opacity; ctx.fillStyle = colors.normal; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (node.accent > .01 || active) { ctx.globalAlpha = opacity * Math.max(node.accent, active ? 1 : 0); ctx.fillStyle = colors.accent; ctx.beginPath(); ctx.arc(x, y, radius + (active ? 1 : 0), 0, Math.PI * 2); ctx.fill(); } if (labelNodes.has(node.id)) labels.push({ node, x, y, radius, active, opacity }); this.hit.push({ node, x, y, radius: radius + 9 }); }
  ...
  pointerMove(event) { if (this.dragging) { const rect = this.canvas.getBoundingClientRect(), scale = Math.max(20, Math.min(rect.width, rect.height) / 2 - 68); this.dragging.x = (event.clientX - rect.left - rect.width / 2) / scale; this.dragging.y = (event.clientY - rect.top - rect.height / 2) / scale; this.dragging.vx = this.dragging.vy = 0; this.startSimulation(.38); return; } const node = this.hitAt(event); this.canvas.style.cursor = node ? 'grab' : 'default'; this.setHover(node?.id || null, true); }
...
class SemanticSearchModal extends SuggestModal {
  ...
  getSuggestions(query) {
    if (!query || query.trim().length < 2) { clearTimeout(this.debounceTimer); this.searchVersion++; const changed = Boolean(this.lastQuery); this.lastQuery = ''; this.lastResults = []; this.mapResults = []; if (changed) window.setTimeout(() => this.updateMap(), 0); return []; }
