...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  semanticDensityField(width, height) {
    ...
    if (this.hasQuery && this.queryPresence > .04) { const [centerX, centerY] = this.coordinates(this.queryNode, width, height), center = { x: centerX, y: centerY }, directResults = [...points.values()].filter(({ node }) => node.matched && node.generation === 1).sort((first, second) => second.node.relevance - first.node.relevance).slice(0, 5); for (const target of directResults) addRidge(center, target, sigma * .46, this.queryPresence * target.node.visibility * (.07 + Math.max(0, Math.min(1, target.node.relevance)) * .11)); const supportMaximum = Math.max(0, ...values); addMass(centerX, centerY, sigma * .56, (supportMaximum * 1.18 + .42) * this.queryPresence); }
