...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  communityFallbackLabel(values) {
    const entities = new Map(), ignored = /^(?:note|notes|journal|entry|entries|study|thought|thoughts|idea|ideas|topic|topics)$/i;
    for (const value of values) for (const entity of value.node.entities || []) { const text = String(entity || '').trim(); if (text.length < 3 || ignored.test(text)) continue; const key = text.toLocaleLowerCase(); const item = entities.get(key) || { text, count: 0 }; item.count++; entities.set(key, item); }
    const entity = [...entities.values()].sort((first, second) => second.count - first.count || first.text.length - second.text.length || first.text.localeCompare(second.text))[0]; if (entity) return entity.text.replace(/\b\p{L}/gu, letter => letter.toUpperCase());
    const representative = [...values].sort((first, second) => Number(second.node.communityMembership || 0) - Number(first.node.communityMembership || 0) || Number(second.node.relevance || 0) - Number(first.node.relevance || 0))[0]?.node, title = String(representative?.label || '').replace(/^\d{4}-\d{2}-\d{2}\s*/, '').trim(); return title ? title.split(/\s+/).slice(0, 4).join(' ') : 'Related Notes';
  }
  ...
  paintTopicCommunities(ctx, width, height, colors) {
    ...
    if (refresh) { if (!this.communityCanvas) this.communityCanvas = document.createElement('canvas'); const canvas = this.communityCanvas; canvas.width = Math.ceil(width * dpr); canvas.height = Math.ceil(height * dpr); const communityContext = canvas.getContext('2d'); communityContext.setTransform(dpr, 0, 0, dpr, 0, 0); communityContext.clearRect(0, 0, width, height); const groups = this.topicCommunityGroups(width, height), tuning = this.tuning || MAP_TUNING_DEFAULTS, padding = 17 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))), labels = [];
      for (const group of groups) { const field = this.communityBoundaryField(group.values, padding), color = this.options.semanticColors !== false ? `hsl(${group.hue} 58% ${group.lightness}%)` : colors.muted; communityContext.save(); communityContext.translate(field.left, field.top); this.traceDensityLevel(communityContext, field, field.level); communityContext.strokeStyle = color; communityContext.globalAlpha = .34; communityContext.lineWidth = 1.1; communityContext.lineCap = 'round'; communityContext.lineJoin = 'round'; communityContext.stroke(); communityContext.restore(); const weight = group.values.reduce((sum, value) => sum + Math.max(.01, Number(value.node.communityMembership || 0)), 0), x = group.values.reduce((sum, value) => sum + value.x * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight, y = group.values.reduce((sum, value) => sum + value.y * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight; labels.push({ x, y, text: group.label || 'Related Notes', color, confidence: group.confidence }); }
