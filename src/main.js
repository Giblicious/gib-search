...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  physicsStep(alpha) {
    ...
    for (let first = 0; first < collisionNodes.length; first++) for (let second = first + 1; second < collisionNodes.length; second++) { const a = collisionNodes[first], b = collisionNodes[second]; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const baseCollision = this.hasQuery ? .035 + (a.fileScale + b.fileScale) * .022 : .016 + (a.fileScale + b.fileScale) * .012, collisionDistance = baseCollision * (vaultMode ? tuning.collisionSpacing : 1), collision = distance < collisionDistance ? -(collisionDistance - distance) * .1 * alpha : 0, relaxation = this.hasQuery ? -Math.min(.0012, .000008 / (distance * distance)) * alpha : -Math.min(.0009, .000006 / (distance * distance)) * Number(tuning.repulsion || 0) * alpha, force = collision + relaxation; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
