...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  semanticDensityField(width, height) {
    const step = 6, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), zoom = Math.max(.65, Math.min(2.4, this.cameraZoom * this.userZoom)), sigma = Math.max(20, Math.min(54, 27 * zoom)), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y }));
