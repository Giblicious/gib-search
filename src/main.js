...
function pointSegmentDistance(x, y, source, target) { const dx = target.x - source.x, dy = target.y - source.y, lengthSquared = dx * dx + dy * dy; if (!lengthSquared) return Math.hypot(x - source.x, y - source.y); const amount = Math.max(0, Math.min(1, ((x - source.x) * dx + (y - source.y) * dy) / lengthSquared)); return Math.hypot(x - (source.x + dx * amount), y - (source.y + dy * amount)); }
function pointInTriangle(x, y, triangle) { const [a, b, c] = triangle, first = (x - b.x) * (a.y - b.y) - (a.x - b.x) * (y - b.y), second = (x - c.x) * (b.y - c.y) - (b.x - c.x) * (y - c.y), third = (x - a.x) * (c.y - a.y) - (c.x - a.x) * (y - a.y), hasNegative = first < 0 || second < 0 || third < 0, hasPositive = first > 0 || second > 0 || third > 0; return !(hasNegative && hasPositive); }
function distanceToTriangle(x, y, triangle) { if (pointInTriangle(x, y, triangle)) return 0; return Math.min(pointSegmentDistance(x, y, triangle[0], triangle[1]), pointSegmentDistance(x, y, triangle[1], triangle[2]), pointSegmentDistance(x, y, triangle[2], triangle[0])); }
...
function buildPaddedFootprint(points, size = 105) {
  const padding = .12, extent = Math.min(1.12, Math.max(.36, ...points.map(point => Math.max(Math.abs(point.x), Math.abs(point.y)) + padding + .045))), nearest = points.map((point, index) => Math.min(...points.filter((_, other) => other !== index).map(other => Math.hypot(point.x - other.x, point.y - other.y)))), typicalSpacing = median(nearest.filter(Number.isFinite)), wrapReach = Math.max(padding * 2.35, Math.min(.62, typicalSpacing * 2.2 || padding * 2.35));
  ...
  for (let row = 0; row < size; row++) for (let column = 0; column < size; column++) { const x = (column / (size - 1) * 2 - 1) * extent, y = (row / (size - 1) * 2 - 1) * extent; let signedDistance = -Infinity; for (const point of points) signedDistance = Math.max(signedDistance, padding - Math.hypot(x - point.x, y - point.y)); for (const triangle of triangles) signedDistance = Math.max(signedDistance, padding - distanceToTriangle(x, y, triangle)); field[row * size + column] = signedDistance; }
  return { size, field, extent };
}
function contourSegments(field, size) {
  const segments = [], edgePoint = (edge, column, row, corners) => { const definitions = [[0, 1, column, row, column + 1, row], [1, 2, column + 1, row, column + 1, row + 1], [2, 3, column + 1, row + 1, column, row + 1], [3, 0, column, row + 1, column, row]], [first, second, x1, y1, x2, y2] = definitions[edge], amount = -corners[first] / (corners[second] - corners[first]); return { x: x1 + (x2 - x1) * amount, y: y1 + (y2 - y1) * amount }; };
  for (let row = 0; row < size - 1; row++) for (let column = 0; column < size - 1; column++) { const corners = [field[row * size + column], field[row * size + column + 1], field[(row + 1) * size + column + 1], field[(row + 1) * size + column]], mask = corners.reduce((value, corner, index) => value | (corner >= 0 ? 1 << index : 0), 0); if (!mask || mask === 15) continue; let pairs = { 1: [[3, 0]], 2: [[0, 1]], 3: [[3, 1]], 4: [[1, 2]], 6: [[0, 2]], 7: [[3, 2]], 8: [[2, 3]], 9: [[0, 2]], 11: [[1, 2]], 12: [[1, 3]], 13: [[0, 1]], 14: [[3, 0]] }[mask]; if (!pairs) { const centerInside = corners.reduce((sum, value) => sum + value, 0) >= 0; pairs = mask === 5 ? (centerInside ? [[0, 1], [2, 3]] : [[3, 0], [1, 2]]) : (centerInside ? [[3, 0], [1, 2]] : [[0, 1], [2, 3]]); } for (const [first, second] of pairs) segments.push([edgePoint(first, column, row, corners), edgePoint(second, column, row, corners)]); }
  return segments;
}
...
function buildTopographicTerrain(query, nodes) {
  const elevated = [{ x: query.x, y: query.y, z: 1 }, ...nodes.map(node => ({ x: node.x, y: node.y, z: node.elevation }))], footprint = buildPaddedFootprint(elevated), boundaryPaths = stitchContourSegments(contourSegments(footprint.field, footprint.size)), ground = [], seen = new Set();
  for (const path of boundaryPaths) { const stride = Math.max(1, Math.ceil(path.length / 48)); for (let index = 0; index < path.length; index += stride) { const point = path[index], sample = { x: (point.x / (footprint.size - 1) * 2 - 1) * footprint.extent, y: (point.y / (footprint.size - 1) * 2 - 1) * footprint.extent, z: 0 }, key = `${Math.round(sample.x * 10000)}:${Math.round(sample.y * 10000)}`; if (!seen.has(key)) { seen.add(key); ground.push(sample); } } }
  const triangles = triangulateTerrain([...elevated, ...ground]).filter(triangle => triangle.some(point => point.z > 0)); return { triangles, extent: footprint.extent };
}
function topographicContourSegments(triangles, level) {
  const segments = []; for (const triangle of triangles) { const crossings = []; for (const [first, second] of [[triangle[0], triangle[1]], [triangle[1], triangle[2]], [triangle[2], triangle[0]]]) { if ((first.z < level) === (second.z < level) || first.z === second.z) continue; const amount = (level - first.z) / (second.z - first.z); crossings.push({ x: first.x + (second.x - first.x) * amount, y: first.y + (second.y - first.y) * amount }); } if (crossings.length === 2) segments.push(crossings); } return segments;
}
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = time => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .985; if (!this.terrain || time - this.lastTerrainAt > 105) { this.terrain = buildTopographicTerrain(this.queryNode, this.nodes); this.lastTerrainAt = time; } this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); else { this.terrain = buildTopographicTerrain(this.queryNode, this.nodes); this.draw(); } }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  drawContours(ctx, width, height, colors) {
    if (!this.terrain) return; const { triangles } = this.terrain, scale = Math.max(20, Math.min(width, height) / 2 - 68), toCanvas = (x, y) => [width / 2 + x * scale, height / 2 + y * scale], levels = [.04, .2, .36, .52, .68, .84]; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    for (let band = 0; band < levels.length; band++) { const outer = band === 0, paths = stitchContourSegments(topographicContourSegments(triangles, levels[band])); ctx.beginPath(); for (const path of paths) drawSmoothContour(ctx, path, toCanvas); ctx.globalAlpha = outer ? .18 : .23 + band * .021; ctx.strokeStyle = outer ? colors.muted : colors.normal; ctx.lineWidth = outer ? .72 : .78 + band * .045; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
