...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  ambientAngle() { return matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : Math.sin((performance.now() - this.ambientStartedAt) * .000075) * .014; }
  ambientPosition(x, y, inverse = false) { const originX = this.queryPresence > .04 ? Number(this.queryNode.x) : 0, originY = this.queryPresence > .04 ? Number(this.queryNode.y) : 0, angle = this.ambientAngle() * (inverse ? -1 : 1), dx = Number(x) - originX, dy = Number(y) - originY, cosine = Math.cos(angle), sine = Math.sin(angle); return { x: originX + dx * cosine - dy * sine, y: originY + dx * sine + dy * cosine }; }
  coordinates(node, width, height) { const point = node.isQuery ? { x: Number(node.x), y: Number(node.y) } : this.ambientPosition(node.x, node.y), scale = Math.max(20, Math.min(width, height) / 2 - 68) * this.cameraZoom * this.userZoom; return [width / 2 + (point.x - this.cameraX) * scale + this.panX, height / 2 + (point.y - this.cameraY) * scale + this.panY]; }
