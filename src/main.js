...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  semanticDensityField(width, height) {
    const step = 6, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), zoom = Math.max(.65, Math.min(2.4, this.cameraZoom * this.userZoom)), sigma = Math.max(20, Math.min(54, 27 * zoom)), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y, coreRadius: Math.max(5.5, Math.min(10, 4 + Number(point.node.fileScale || 0) * 4.4)) }));
  ...
  anchorSummitMesas(values, columns, rows, step, anchors, sigma) {
    if (!anchors.length) return; const source = Float32Array.from(values), sample = (column, row) => source[Math.max(0, Math.min(rows - 1, row)) * columns + Math.max(0, Math.min(columns - 1, column))], mesas = anchors.map(anchor => { const coreRadius = Math.max(Number(anchor.coreRadius || 0), Math.max(5.5, Math.min(9, sigma * .25))), searchRadius = coreRadius * 2.25, left = Math.max(0, Math.floor((anchor.x - searchRadius) / step)), right = Math.min(columns - 1, Math.ceil((anchor.x + searchRadius) / step)), top = Math.max(0, Math.floor((anchor.y - searchRadius) / step)), bottom = Math.min(rows - 1, Math.ceil((anchor.y + searchRadius) / step)); let summit = 0; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) if (Math.hypot(column * step - anchor.x, row * step - anchor.y) <= searchRadius) summit = Math.max(summit, sample(column, row)); return { ...anchor, coreRadius, featherRadius: coreRadius + Math.max(4, step), summit: summit + .012 }; });
