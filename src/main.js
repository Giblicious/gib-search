...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  elevationSamples() { const byId = new Map(this.nodes.map(node => [node.id, node])), points = [{ x: 0, y: 0, z: 1, influence: .28 }, ...this.nodes.map(node => ({ x: node.x, y: node.y, z: node.elevation, influence: .17 + node.relevance * .035 }))], ridges = [...(this.activeEdges || [])].sort((first, second) => second.strength - first.strength).slice(0, Math.min(36, this.nodes.length * 2)); for (const edge of ridges) { const source = byId.get(edge.source), target = byId.get(edge.target); if (!source || !target) continue; points.push({ x: (source.x + target.x) / 2, y: (source.y + target.y) / 2, z: Math.min(.94, (source.elevation + target.elevation) / 2 + edge.strength * .055), influence: .15 + edge.strength * .055 }); } return points; }
