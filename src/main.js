...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  applyLinkGraphForces(nodes, alpha) { if (nodes.length < 2) return; const active = new Set(nodes.map(node => node.id)), degrees = new Map(), links = []; for (const edge of this.roadEdges) { if (!active.has(edge.source) || !active.has(edge.target)) continue; degrees.set(edge.source, (degrees.get(edge.source) || 0) + 1); degrees.set(edge.target, (degrees.get(edge.target) || 0) + 1); links.push(edge); } for (const edge of links) { const source = this.byId.get(edge.source), target = this.byId.get(edge.target); if (!source || !target) continue; let dx = target.x - source.x, dy = target.y - source.y, distance = Math.hypot(dx, dy); if (distance < .0001) { const angle = stableMapAngle(edge.key || mapEdgeKey(edge.source, edge.target)); dx = Math.cos(angle); dy = Math.sin(angle); distance = .0001; } else { dx /= distance; dy /= distance; } const desired = .17 + (source.fileScale + target.fileScale) * .025, degreeScale = 1 / Math.sqrt(Math.max(1, degrees.get(edge.source) || 1, degrees.get(edge.target) || 1)), force = Math.tanh((distance - desired) * 4.2) * .014 * degreeScale * alpha; if (source !== this.dragging) { source.vx += dx * force; source.vy += dy * force; } if (target !== this.dragging) { target.vx -= dx * force; target.vy -= dy * force; } } const cellSize = .36, cells = new Map(), key = (x, y) => `${x},${y}`; for (const node of nodes) { const cellX = Math.floor(node.x / cellSize), cellY = Math.floor(node.y / cellSize); for (let offsetX = -1; offsetX <= 1; offsetX++) for (let offsetY = -1; offsetY <= 1; offsetY++) for (const other of cells.get(key(cellX + offsetX, cellY + offsetY)) || []) { let dx = node.x - other.x, dy = node.y - other.y, distance = Math.hypot(dx, dy); if (distance >= cellSize) continue; if (distance < .0001) { const angle = stableMapAngle(`${other.id}\0${node.id}`); dx = Math.cos(angle); dy = Math.sin(angle); distance = .0001; } else { dx /= distance; dy /= distance; } const force = (1 - distance / cellSize) ** 2 * .0045 * alpha; if (other !== this.dragging) { other.vx -= dx * force; other.vy -= dy * force; } if (node !== this.dragging) { node.vx += dx * force; node.vy += dy * force; } } const values = cells.get(key(cellX, cellY)) || []; values.push(node); cells.set(key(cellX, cellY), values); } const centerX = nodes.reduce((sum, node) => sum + node.x, 0) / nodes.length, centerY = nodes.reduce((sum, node) => sum + node.y, 0) / nodes.length; for (const node of nodes) if (node !== this.dragging) { node.vx -= centerX * .0022 * alpha; node.vy -= centerY * .0022 * alpha; } }
