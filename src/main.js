...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  semanticDensityField(width, height) {
    const step = 6, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), clusterBase = new Float32Array(columns * rows), peaks = new Map(), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), zoom = Math.max(.65, Math.min(2.4, this.cameraZoom * this.userZoom)), sigma = Math.max(20, Math.min(54, 27 * zoom));
    const addMass = (field, x, y, radius, amplitude, combine = 'max') => { const reach = radius * 3, left = Math.max(0, Math.floor((x - reach) / step)), right = Math.min(columns - 1, Math.ceil((x + reach) / step)), top = Math.max(0, Math.floor((y - reach) / step)), bottom = Math.min(rows - 1, Math.ceil((y + reach) / step)), divisor = 2 * radius * radius; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) { const dx = column * step - x, dy = row * step - y, index = row * columns + column, height = amplitude * Math.exp(-(dx * dx + dy * dy) / divisor); field[index] = combine === 'sum' ? field[index] + height : Math.max(field[index], height); } };
    const addRidge = (source, target, radius, amplitude) => { const reach = radius * 3, left = Math.max(0, Math.floor((Math.min(source.x, target.x) - reach) / step)), right = Math.min(columns - 1, Math.ceil((Math.max(source.x, target.x) + reach) / step)), top = Math.max(0, Math.floor((Math.min(source.y, target.y) - reach) / step)), bottom = Math.min(rows - 1, Math.ceil((Math.max(source.y, target.y) + reach) / step)), dx = target.x - source.x, dy = target.y - source.y, lengthSquared = Math.max(1, dx * dx + dy * dy), divisor = 2 * radius * radius; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) { const x = column * step, y = row * step, along = Math.max(0, Math.min(1, ((x - source.x) * dx + (y - source.y) * dy) / lengthSquared)), nearestX = source.x + dx * along, nearestY = source.y + dy * along, distanceX = x - nearestX, distanceY = y - nearestY, endpointTaper = Math.min(1, along / .16, (1 - along) / .16), height = amplitude * endpointTaper * Math.exp(-(distanceX * distanceX + distanceY * distanceY) / divisor), index = row * columns + column; values[index] = Math.max(values[index], height); } };
    for (const { node, x, y } of points.values()) { const generationWeight = node.generation === 1 ? 1 : node.generation === 2 ? .58 : .36, amplitude = node.visibility * generationWeight * (.82 + Math.max(0, Math.min(1, node.relevance)) * .18); peaks.set(node.id, amplitude); addMass(values, x, y, sigma, amplitude); addMass(clusterBase, x, y, sigma * 1.45, amplitude * .16, 'sum'); }
    for (let index = 0; index < values.length; index++) values[index] = Math.max(values[index], Math.min(.28, clusterBase[index]));
    ...
    for (const edge of ridges) { const endpointPeak = Math.min(peaks.get(edge.source) || 0, peaks.get(edge.target) || 0); addRidge(points.get(edge.source), points.get(edge.target), ridgeRadius, endpointPeak * (.46 + Math.min(1, edge.strength) * .14)); }
    if (this.hasQuery && this.queryPresence > .04) { const [centerX, centerY] = this.coordinates(this.queryNode, width, height), center = { x: centerX, y: centerY }, directResults = [...points.values()].filter(({ node }) => node.matched && node.generation === 1).sort((first, second) => second.node.relevance - first.node.relevance).slice(0, 5); for (const target of directResults) addRidge(center, target, sigma * .46, this.queryPresence * (peaks.get(target.node.id) || 0) * .54); const supportMaximum = Math.max(0, ...values); addMass(values, centerX, centerY, sigma * .56, (supportMaximum * 1.18 + .42) * this.queryPresence); }
