...
class GraphView extends ItemView {
  ...
  handleQuery(value) {
    ...
    this.map.beginQuery(query, true); this.map.setTitle(`Searching “${query}”…`); this.setResultsCollapsed(false); this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: 'Searching…' }); this.resultStatus.textContent = ''; this.searchTimer = window.setTimeout(() => this.runSearch(query), 75);
  }
