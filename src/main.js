...
async function buildQueryMapModel(plugin, query, results, options = {}) {
  const lens = validSearchLens(options.lens), generations = Math.max(1, Math.min(3, Number(options.generations) || 1)), mapMode = validMapGrouping(options.mapMode), relationships = options.relationships instanceof Map ? options.relationships : new Map(), relationshipEdges = options.relationshipEdges || [], sourceResults = results.map(result => ({ ...result })), roots = sourceResults.map(result => result.file), indexSignature = `${plugin.search.meta.length}:${plugin.search.vectors.length}:${plugin.search.meta.reduce((sum, item) => sum + Number(item.mtime || 0), 0)}`, relationshipSignature = lens === 'arguments' ? [...relationships].sort(([first], [second]) => first.localeCompare(second)).map(([key, value]) => [key, value?.type, Number(value?.confidence || 0)]) : [], key = JSON.stringify([indexSignature, String(query).trim().toLowerCase(), generations, lens, mapMode, Boolean(plugin.settings.magicGraphEnabled), plugin.settings.mapTuning, sourceResults.map(result => [result.file, Number(result.lensScore ?? result.score ?? 0)]), relationshipSignature]), cache = plugin.queryMapCache || (plugin.queryMapCache = new Map()), clone = model => ({ nodes: model.nodes.map(node => ({ ...node })), edges: model.edges.map(edge => ({ ...edge })), roads: model.roads.map(road => ({ ...road })) });
  if (cache.has(key)) { const cached = await cache.get(key); cache.delete(key); cache.set(key, cached); return clone(cached); }
  const pending = (async () => {
    const expansion = plugin.search.semanticGenerations(roots, generations, 5), generationByFile = new Map(expansion.nodes.map(node => [node.id, node])), activeFiles = expansion.nodes.map(node => node.id), facets = await plugin.search.conceptFacets(query, activeFiles); for (const result of sourceResults) { const facet = facets.get(result.file); result.facet = facet?.facet; result.conceptAffinities = facet?.affinities; }
    const graph = await plugin.search.semanticStarfield(query, activeFiles, activeFiles), byFile = new Map(sourceResults.map(result => [result.file, result])), scales = options.fileScales || vaultFileScales(plugin.app), rankingScores = sourceResults.map(result => Number(result.lensScore ?? result.score ?? 0)), rankingLow = rankingScores.length ? Math.min(...rankingScores) : 0, rankingHigh = rankingScores.length ? Math.max(...rankingScores) : 1, rankingSpread = Math.max(.001, rankingHigh - rankingLow), semanticScores = graph.nodes.map(node => Number(node.semanticScore || 0)), semanticLow = semanticScores.length ? Math.min(...semanticScores) : 0, semanticHigh = semanticScores.length ? Math.max(...semanticScores) : 1, semanticSpread = Math.max(.001, semanticHigh - semanticLow), expansionEdges = expansion.edges.map(edge => ({ ...edge, residualScore: 0 })), edgeKeys = new Set((graph.edges || []).map(edge => mapEdgeKey(edge.source, edge.target))), combinedEdges = [...(graph.edges || []), ...expansionEdges.filter(edge => !edgeKeys.has(mapEdgeKey(edge.source, edge.target)))];
    const nodes = graph.nodes.map(node => { const result = byFile.get(node.id), generation = generationByFile.get(node.id), subtopic = facets.get(node.id), semanticRelevance = (Number(node.semanticScore || 0) - semanticLow) / semanticSpread, rankedRelevance = result ? (Number(result.lensScore ?? result.score ?? 0) - rankingLow) / rankingSpread : 0, expandedRelevance = generation && generation.generation > 1 ? Math.max(.22, Math.min(.62, Number(generation.relationScore || 0))) : semanticRelevance, community = mapMode === 'topics' && subtopic ? subtopic.facet : node.community, communityMembership = subtopic ? Number(subtopic.affinities?.[subtopic.facet] || 0) : Number(node.communityMembership || 0); return { ...node, generation: generation?.generation || 1, parent: generation?.parent || null, matched: Boolean(generation), relevance: result ? .08 + rankedRelevance * .92 : expandedRelevance, fileScale: scales.get(node.id) ?? .35, facet: subtopic?.facet ?? result?.facet ?? node.community, community, communityMembership: mapMode === 'topics' ? communityMembership : node.communityMembership, communityLabel: mapMode === 'topics' ? subtopic?.label || '' : node.communityLabel, communityFallbackLabel: mapMode === 'topics' ? subtopic?.fallbackLabel || '' : node.communityFallbackLabel, communityLabelConfidence: mapMode === 'topics' ? Number(subtopic?.confidence || 0) : node.communityLabelConfidence, conceptAffinities: subtopic?.affinities || result?.conceptAffinities || node.topicAffinities, topicAffinities: mapMode === 'topics' && subtopic ? subtopic.affinities : node.topicAffinities, contextScore: result?.contextScore }; }), layout = await plugin.search.multiRelationalLayout(query, nodes, lens === 'arguments' ? relationships : new Map(), { magic: plugin.settings.magicGraphEnabled, lens, mapMode });
    ...
    return { nodes, edges, roads: manualRoadEdges(plugin.app, nodes.map(node => node.id)) };
  })();
  cache.set(key, pending); try { const model = await pending; cache.set(key, model); while (cache.size > 24) cache.delete(cache.keys().next().value); return clone(model); } catch (error) { cache.delete(key); throw error; }
}
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setGraph(center, values, edges = [], roads = []) {
    this.lastTerrainAt = 0; this.lastCommunityAt = 0;
...
class SemanticSearchModal extends SuggestModal {
  ...
  async updateMap() {
    ...
    if (!query) { const indexable = this.app.vault.getFiles().filter(file => /\.(?:md|txt|markdown)$/i.test(file.path)), paths = indexable.map(file => file.path), graph = await this.plugin.search.semanticStarfield('', paths); if (version !== this.mapVersion) return; const scales = vaultFileScales(this.app), nodes = graph.nodes.map(node => ({ ...node, matched: false, generation: 1, relevance: .5, fileScale: scales.get(node.id) ?? .35, facet: node.community, conceptAffinities: node.topicAffinities })), layout = await this.plugin.search.multiRelationalLayout('', nodes, new Map(), { magic: this.plugin.settings.magicGraphEnabled, lens: 'relevance', vaultCenter: true, mapMode: this.mapGroupingMode }); if (version !== this.mapVersion) return; for (const node of nodes) { const target = layout.get(node.id); if (target) { node.layoutX = target.x; node.layoutY = target.y; } } this.map.setGraph({ label: 'Search', hasQuery: false, resultCount: 0 }, nodes, graph.edges || [], manualRoadEdges(this.app, paths)); return; }
...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.previewVersion = 0; this.previewFilePath = null; this.results = []; this.resultsCollapsed = true; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.mapGroupingMode = validMapGrouping(plugin.settings.mapGroupingMode); this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1));
  }
  getViewType() { return GRAPH_VIEW; }
  getDisplayText() { return 'Gib Search map'; }
  ...
  async onOpen() {
    ...
    this.previewPanel = this.contentEl.createDiv({ cls: 'gib-graph-preview-panel' }); const previewHeader = this.previewPanel.createDiv({ cls: 'gib-graph-preview-header' }), previewHeading = previewHeader.createDiv({ cls: 'gib-graph-preview-heading' }), previewIcon = previewHeading.createSpan({ cls: 'gib-graph-preview-icon' }); setIcon(previewIcon, 'sticky-note'); const previewNames = previewHeading.createDiv({ cls: 'gib-graph-preview-names' }); this.previewTitle = previewNames.createDiv({ cls: 'gib-graph-preview-title' }); this.previewPath = previewNames.createDiv({ cls: 'gib-graph-preview-path' }); const previewOpen = previewHeader.createEl('button', { attr: { type: 'button', title: 'Open note', 'aria-label': 'Open note' } }); setIcon(previewOpen, 'external-link'); const previewClose = previewHeader.createEl('button', { attr: { type: 'button', title: 'Close preview', 'aria-label': 'Close preview' } }); setIcon(previewClose, 'panel-right-close'); this.previewBody = this.previewPanel.createDiv({ cls: 'gib-graph-preview-body markdown-rendered' }); this.previewPanel.hide();
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      title: 'Vault map', preserveBackground: true, generations: this.mapGenerations, mapGroupingMode: this.mapGroupingMode, showLinks: this.plugin.settings.showWikilinks, manualLinkInfluence: this.plugin.settings.graphManualLinkInfluence, semanticColors: this.plugin.settings.graphSemanticColors, tuning: this.plugin.settings.mapTuning,
      onMapMode: async value => { this.mapGroupingMode = validMapGrouping(value); this.plugin.settings.mapGroupingMode = this.mapGroupingMode; await this.plugin.save(); this.query.length >= 3 ? this.runSearch(this.query, true) : this.loadVault(); },
      onTune: (values, key) => { this.plugin.settings.mapTuning = values; window.clearTimeout(this.tuneTimer); this.tuneTimer = window.setTimeout(async () => { if (['commonnessSuppression', 'passageCoverage', 'passageDiversity', 'communitySensitivity', 'communityMinSize', 'communityLabelSensitivity', 'reset'].includes(key)) { this.plugin.search.contentProfileCache = null; this.plugin.search.starfieldCache = null; } await this.plugin.save(); if (mapTuningNeedsLayout(key)) this.query.length >= 3 ? this.runSearch(this.query, true) : this.loadVault(); else this.map?.draw(); }, 180); },
      onGenerations: async value => { this.mapGenerations = value; this.plugin.settings.searchMapGenerations = value; await this.plugin.save(); if (this.query.length >= 3) this.runSearch(this.query, true); }, onShowLinks: async value => { this.plugin.settings.showWikilinks = value; await this.plugin.save(); }, onManualLinkInfluence: async value => { this.plugin.settings.graphManualLinkInfluence = value; await this.plugin.save(); }, onHover: file => this.hoverResult(file), onSelect: file => this.showPreview(file), onOpen: file => this.openFile(file)
    });
  ...
  async loadVault() {
    const version = ++this.loadVersion; this.map?.setTitle('Loading vault…');
    try {
      const files = this.indexableFiles(), paths = files.map(file => file.path), scales = vaultFileScales(this.app); if (!this.baseNodes.length && !this.map?.nodes?.length) { const count = Math.max(1, files.length), seeds = files.map((file, index) => { const angle = stableMapAngle(file.path), radius = .035 + Math.sqrt((index + .5) / count) * .16; return { id: file.path, label: file.basename, matched: false, generation: 1, relevance: .5, uniqueness: .5, fileScale: scales.get(file.path) ?? .35, layoutX: Math.cos(angle) * radius, layoutY: Math.sin(angle) * radius }; }); this.map.setTitle(`Finding places for ${files.length} notes…`); this.map.setGraph({ label: 'Search', hasQuery: false, resultCount: 0 }, seeds, [], []); }
      const graph = await this.plugin.search.semanticStarfield('', paths); if (version !== this.loadVersion || this.query.length >= 3) return;
      let nodes = graph.nodes.map(node => ({ ...node, matched: false, generation: 1, relevance: .5, fileScale: scales.get(node.id) ?? .35, facet: node.community, conceptAffinities: node.topicAffinities })); const layout = await this.plugin.search.multiRelationalLayout('', nodes, new Map(), { magic: this.plugin.settings.magicGraphEnabled, lens: 'relevance', vaultCenter: true, mapMode: this.mapGroupingMode }); if (version !== this.loadVersion || this.query.length >= 3) return;
      nodes = nodes.map(node => ({ ...node, layoutX: layout.get(node.id)?.x, layoutY: layout.get(node.id)?.y })); this.baseNodes = nodes; this.baseEdges = graph.edges || []; this.baseRoads = manualRoadEdges(this.app, paths); this.map.setTitle(`${mapGroupingLabel(this.mapGroupingMode)} · vault`); this.map.setGraph({ label: 'Search', hasQuery: false, resultCount: 0 }, nodes, this.baseEdges, this.baseRoads);
    } catch (error) { if (version === this.loadVersion) this.map?.setTitle(error.message); }
  }
  handleQuery(value) {
    const query = String(value || '').trim(); this.query = query; window.clearTimeout(this.searchTimer);
    if (query.length < 3) { this.searchVersion++; this.results = []; this.resultsPanel.hide(); this.reopenButton.hide(); this.contentEl.addClass('is-results-collapsed'); if (this.baseNodes.length) { this.map.setTitle(`${mapGroupingLabel(this.mapGroupingMode)} · vault`); this.map.setGraph({ label: 'Search', hasQuery: false, resultCount: 0 }, this.baseNodes, this.baseEdges, this.baseRoads); } else this.loadVault(); return; }
    this.map.beginQuery(query, true); this.map.setTitle(`Searching “${query}”…`); this.resultsPanel.toggle(!this.resultsCollapsed); this.reopenButton.toggle(this.resultsCollapsed); this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: 'Searching…' }); this.resultStatus.textContent = ''; this.searchTimer = window.setTimeout(() => this.runSearch(query), 75);
  }
  async runSearch(query, immediate = false) {
    if (String(query).trim().length < 3) return; if (!immediate) this.map.beginQuery(query, true); const version = ++this.searchVersion, tweaks = activeTweaks(this.plugin), limit = Math.max(30, tweaks.topK);
    try {
      const runSearch = this.plugin.search.searchLive?.bind(this.plugin.search) || this.plugin.search.search.bind(this.plugin.search), raw = await runSearch(query, Math.min(240, limit * 5), tweaks.minScore, { scoreWindow: tweaks.scoreWindow, folderPathBoost: this.plugin.settings.folderPathBoostEnabled ? tweaks.folderPathBoost : 0, semanticHighlights: tweaks.semanticHighlights, resultMinScore: tweaks.highlightResultMinScore, singleWordMinScore: tweaks.highlightSingleWordMinScore, phraseMinScore: tweaks.highlightPhraseMinScore, maxPhrases: tweaks.highlightMaxPhrases, highlightLimit: 20 }); if (version !== this.searchVersion || query !== this.query) return;
      const results = groupSearchResults(raw, query, limit), model = await buildQueryMapModel(this.plugin, query, results, { lens: 'relevance', generations: this.mapGenerations, mapMode: this.mapGroupingMode, fileScales: new Map(this.baseNodes.map(node => [node.id, node.fileScale])) }); if (version !== this.searchVersion || query !== this.query) return; const queryById = new Map(model.nodes.map(node => [node.id, node])), merged = this.baseNodes.map(base => queryById.get(base.id) || { ...base, matched: false, relevance: 0 });
      this.results = results; this.renderResults(); this.map.setTitle(`${results.length} results · ${mapGroupingLabel(this.mapGroupingMode)}`); const edgeKeys = new Set(model.edges.map(edge => mapEdgeKey(edge.source, edge.target))), backgroundEdges = this.baseEdges.filter(edge => !edgeKeys.has(mapEdgeKey(edge.source, edge.target))); this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, merged, [...model.edges, ...backgroundEdges], model.roads);
  ...
  renderResults() {
    ...
    this.resultsPanel.toggle(!this.resultsCollapsed); this.reopenButton.toggle(Boolean(this.resultsCollapsed));
  }
  ...
  focusResult(file) { const item = [...this.resultList.querySelectorAll('.gib-graph-result')].find(value => value.dataset.gibFile === file); if (item && !this.resultsCollapsed) item.scrollIntoView({ block: 'nearest' }); }
  ...
  async openFile(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (file instanceof TFile) await this.app.workspace.getLeaf(false).openFile(file); }
  async onClose() { this.loadVersion++; this.searchVersion++; window.clearTimeout(this.searchTimer); window.clearTimeout(this.tuneTimer); this.map?.destroy(); }
}
...
module.exports = class GibSearch extends Plugin {
  async onload() {
    ...
    this.settings = Object.assign({}, DEFAULTS, loaded); delete this.settings.mapProjection; this.settings.mapTuning = Object.assign({}, MAP_TUNING_DEFAULTS, loaded.mapTuning || {}); this.isMobile = Platform.isMobileApp;
