...
class SemanticSearchModal extends SuggestModal {
  ...
  async applyLens() {
    ...
    try {
      ...
      await new Promise(resolve => window.setTimeout(resolve, 180)); if (version !== this.lensVersion) return; const candidates = source.slice(0, Math.min(source.length, Math.max(this.visibleLimit || 10, 10))), edges = this.plugin.search.argumentCandidateEdges(candidates.map(result => result.file)), budget = this.plugin.isMobile ? this.plugin.settings.graphRelationshipBudgetMobile : this.plugin.settings.graphRelationshipBudgetDesktop; this.map?.setIntelligenceStatus('Comparing positions…'); const details = await this.plugin.search.graphRelationships(edges, budget); if (version !== this.lensVersion) return; this.lensRelationships = details; this.lensRelationshipEdges = edges; const counts = new Map(candidates.map(result => [result.file, { support: 0, contrast: 0 }])); for (const edge of edges) { const relation = details.get(mapEdgeKey(edge.source, edge.target)); if (!relation || relation.type === 'related') continue; const key = relation.type === 'contrast' ? 'contrast' : 'support'; counts.get(edge.source)[key]++; counts.get(edge.target)[key]++; } for (const result of source) { const count = counts.get(result.file); result.lensLabel = !count ? 'Position' : count.contrast && count.support ? 'Mixed position' : count.contrast ? `${count.contrast} tension${count.contrast === 1 ? '' : 's'}` : count.support ? `${count.support} alignment${count.support === 1 ? '' : 's'}` : 'Position'; } this.commitLensResults(source, version); this.map?.setIntelligenceStatus('');
...
class NeighborhoodView extends ItemView {
  ...
  async centerOn(filePath, pin = this.pinned) {
    ...
    try { const graph = this.plugin.search.semanticNeighbors(file.path, 18); if (version !== this.loadVersion) return; let nodes = graph.nodes.map(node => ({ ...node, semanticScore: Number(node.score || 0) })), relationships = new Map(), edges = graph.edges, intelligenceMessage = '';
      if (this.lens === 'concepts') { const facets = this.plugin.search.conceptFacetsFromFile(file.path, nodes.map(node => node.id)); nodes = nodes.map(node => ({ ...node, facet: facets.get(node.id)?.facet })); }
