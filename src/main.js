...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  coordinates(node, width, height) { const scale = Math.max(20, Math.min(width, height) / 2 - 68); return [width / 2 + (Number(node.x) - this.cameraX) * scale * this.cameraZoom, height / 2 + (Number(node.y) - this.cameraY) * scale * this.cameraZoom]; }
  draw() {
    ...
    const labels = [], ordered = this.hasQuery && !this.pendingQuery ? [...this.nodes.filter(node => !node.matched), ...this.nodes.filter(node => node.matched)] : this.nodes; for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.01, node.visibility * focusAlpha); ctx.globalAlpha = opacity; ctx.fillStyle = node.matched || !this.hasQuery || this.pendingQuery ? colors.normal : colors.faint; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (node.matched && (node.accent > .01 || active)) { ctx.globalAlpha = opacity * Math.max(node.accent, active ? 1 : 0); ctx.fillStyle = colors.accent; ctx.beginPath(); ctx.arc(x, y, radius + (active ? 1 : 0), 0, Math.PI * 2); ctx.fill(); } if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery) this.hit.push({ node, x, y, radius: radius + 9 }); }
  ...
  pointerMove(event) { const rect = this.canvas.getBoundingClientRect(); if (this.dragging) { const scale = Math.max(20, Math.min(rect.width, rect.height) / 2 - 68) * this.cameraZoom; this.dragging.x = (event.clientX - rect.left - rect.width / 2) / scale + this.cameraX; this.dragging.y = (event.clientY - rect.top - rect.height / 2) / scale + this.cameraY; this.dragging.vx = this.dragging.vy = 0; this.startSimulation(.38); return; } const node = this.hitAt(event); this.canvas.style.cursor = node ? 'grab' : 'default'; this.setHover(node?.id || null, true); }
...
class SemanticSearchModal extends SuggestModal {
  ...
  setupMap() {
    ...
    this.map = new LivingSemanticMapCanvas(panel, this.app, { title: 'Search map', generations: this.mapGenerations, onGenerations: async value => { this.mapGenerations = value; this.plugin.settings.searchMapGenerations = value; await this.plugin.save(); this.updateMap(); }, onHover: file => this.hoverResult(file), onSelect: file => this.focusResult(file), onOpen: file => this.openFile(file), onExplore: file => { this.close(); this.plugin.openNeighborhood(file, true); } });
