...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setGraph(center, values, edges = []) {
    ...
    const similarities = edges.map(edge => Number(edge.score || 0)), similarityLow = similarities.length ? Math.min(...similarities) : 0, similarityHigh = similarities.length ? Math.max(...similarities) : 1, similaritySpread = Math.max(.001, similarityHigh - similarityLow); this.forceStrength = new Map(edges.map(edge => [mapEdgeKey(edge.source, edge.target), (Number(edge.score || similarityLow) - similarityLow) / similaritySpread])); for (let step = 0; step < 24; step++) this.physicsStep(.9);
  ...
  physicsStep(alpha) {
    for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second]; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const similarity = this.forceStrength.get(mapEdgeKey(a.id, b.id)) || 0, desired = .12 + (1 - similarity) * .61, spring = (distance - desired) * (.0016 + similarity * .0075) * alpha, collision = distance < .115 ? -(0.115 - distance) * .1 * alpha : 0, force = spring + collision; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    for (const node of this.nodes) { if (node === this.dragging) { node.vx = node.vy = 0; continue; } const distance = Math.max(.001, Math.hypot(node.x, node.y)), radial = (node.preferredRadius - distance) * .025 * alpha; node.vx += node.x / distance * radial; node.vy += node.y / distance * radial; node.vx *= .885; node.vy *= .885; node.x += node.vx; node.y += node.vy; const extent = Math.hypot(node.x, node.y); if (extent > .94) { node.x *= .94 / extent; node.y *= .94 / extent; } }
  }
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = time => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .985; if (!this.terrain || time - this.lastTerrainAt > 72) { this.terrain = buildSmoothElevation([{ x: 0, y: 0, z: 1 }, ...this.nodes.map(node => ({ x: node.x, y: node.y, z: node.elevation }))], this.alpha < .08 ? 200 : 125); this.lastTerrainAt = time; } this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); else { this.terrain = buildSmoothElevation([{ x: 0, y: 0, z: 1 }, ...this.nodes.map(node => ({ x: node.x, y: node.y, z: node.elevation }))], 220); this.draw(); } }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  drawContours(ctx, width, height, colors) {
    ...
    for (let step = 1; step <= 9; step++) { const level = step / 10, major = step % 2 === 0; ctx.beginPath(); for (let row = 0; row < size - 1; row++) for (let column = 0; column < size - 1; column++) { const corners = [field[row * size + column], field[row * size + column + 1], field[(row + 1) * size + column + 1], field[(row + 1) * size + column]], crossings = []; const cross = (first, second, x1, y1, x2, y2) => { if ((first < level) === (second < level) || first === second) return; const amount = (level - first) / (second - first); crossings.push(toCanvas(x1 + (x2 - x1) * amount, y1 + (y2 - y1) * amount)); }; cross(corners[0], corners[1], column, row, column + 1, row); cross(corners[1], corners[2], column + 1, row, column + 1, row + 1); cross(corners[2], corners[3], column + 1, row + 1, column, row + 1); cross(corners[3], corners[0], column, row + 1, column, row); if (crossings.length === 2) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); } else if (crossings.length === 4) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); ctx.moveTo(crossings[2][0], crossings[2][1]); ctx.lineTo(crossings[3][0], crossings[3][1]); } } ctx.globalAlpha = major ? .36 : .21; ctx.strokeStyle = major ? colors.normal : colors.muted; ctx.lineWidth = major ? 1.08 : .72; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
