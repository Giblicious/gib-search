...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  semanticDensityField(width, height) {
    const step = 6, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), zoom = Math.max(.65, Math.min(2.4, this.cameraZoom * this.userZoom)), sigma = Math.max(20, Math.min(54, 27 * zoom));
    ...
    for (const edge of ridges) { const source = points.get(edge.source), target = points.get(edge.target), reach = ridgeRadius * 3, left = Math.max(0, Math.floor((Math.min(source.x, target.x) - reach) / step)), right = Math.min(columns - 1, Math.ceil((Math.max(source.x, target.x) + reach) / step)), top = Math.max(0, Math.floor((Math.min(source.y, target.y) - reach) / step)), bottom = Math.min(rows - 1, Math.ceil((Math.max(source.y, target.y) + reach) / step)), dx = target.x - source.x, dy = target.y - source.y, lengthSquared = Math.max(1, dx * dx + dy * dy), amplitude = .12 + Math.min(1, edge.strength) * .2, divisor = 2 * ridgeRadius * ridgeRadius; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) { const x = column * step, y = row * step, along = Math.max(0, Math.min(1, ((x - source.x) * dx + (y - source.y) * dy) / lengthSquared)), nearestX = source.x + dx * along, nearestY = source.y + dy * along, distanceX = x - nearestX, distanceY = y - nearestY, taper = Math.sin(Math.PI * along) ** .6; values[row * columns + column] += amplitude * taper * Math.exp(-(distanceX * distanceX + distanceY * distanceY) / divisor); } }
    return { values, columns, rows, step };
  }
  paintDensityTerrain(ctx, width, height, colors) {
    const now = performance.now(), colorKey = `${colors.normal}|${colors.muted}`, refresh = !this.terrainCanvas || this.terrainCanvas.width !== Math.ceil(width) || this.terrainCanvas.height !== Math.ceil(height) || this.terrainColorKey !== colorKey || now - Number(this.lastTerrainAt || 0) >= 48; if (refresh) { const field = this.semanticDensityField(width, height), maximum = Math.max(0, ...field.values); if (!this.terrainCanvas) this.terrainCanvas = document.createElement('canvas'); if (!this.terrainMask) this.terrainMask = document.createElement('canvas'); const terrain = this.terrainCanvas, mask = this.terrainMask; terrain.width = Math.ceil(width); terrain.height = Math.ceil(height); mask.width = field.columns; mask.height = field.rows; const terrainContext = terrain.getContext('2d'), maskContext = mask.getContext('2d'), image = maskContext.createImageData(field.columns, field.rows); maskContext.fillStyle = colors.normal; maskContext.fillRect(0, 0, 1, 1); const sample = maskContext.getImageData(0, 0, 1, 1).data; for (let index = 0; index < field.values.length; index++) { const alpha = Math.round(Math.max(0, Math.min(.055, (field.values[index] - .14) / 3 * .055)) * 255), offset = index * 4; image.data[offset] = sample[0]; image.data[offset + 1] = sample[1]; image.data[offset + 2] = sample[2]; image.data[offset + 3] = alpha; } maskContext.putImageData(image, 0, 0); terrainContext.clearRect(0, 0, terrain.width, terrain.height); if (maximum >= .16) { terrainContext.imageSmoothingEnabled = true; terrainContext.drawImage(mask, 0, 0, field.columns, field.rows, 0, 0, field.columns * field.step, field.rows * field.step); const levels = [.35, .62, .95, 1.35, 1.9, 2.6]; levels.forEach((level, index) => { if (maximum < level) return; this.traceDensityLevel(terrainContext, field, level); terrainContext.strokeStyle = colors.muted; terrainContext.globalAlpha = .14 + index * .035; terrainContext.lineWidth = index === levels.length - 1 ? 1.05 : .72; terrainContext.stroke(); }); terrainContext.globalAlpha = 1; } this.lastTerrainAt = now; this.terrainColorKey = colorKey; }
