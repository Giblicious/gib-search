...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  constructor(host, app, options = {}) { super(host, app, options); cancelAnimationFrame(this.animationFrame); this.animationFrame = null; this.simulationFrame = null; this.alpha = 0; }
  setGraph(center, values, edges = []) {
    ...
    this.nodes = values.map((value, order) => { const relevance = spread > .001 && Number.isFinite(Number(value.score)) ? (Number(value.score) - low) / spread : 1 - order / Math.max(1, values.length), projected = Number.isFinite(value.x) && Number.isFinite(value.y), angle = stableMapAngle(value.id), old = previous.get(value.id); return { ...value, fileScale: value.fileScale ?? vaultScale.get(value.id) ?? .35, order, relevance, x: old?.x ?? (projected ? value.x : Math.cos(angle) * .45), y: old?.y ?? (projected ? value.y : Math.sin(angle) * .45), vx: old?.vx || 0, vy: old?.vy || 0 }; });
    ...
    this.byId = new Map(this.nodes.map(node => [node.id, node])); this.selected = this.byId.has(this.selected) ? this.selected : null; this.hovered = this.byId.has(this.hovered) ? this.hovered : null; this.statusEl.textContent = `${this.nodes.length} note${this.nodes.length === 1 ? '' : 's'}`; this.startSimulation(.9); this.updateDetail();
  }
  physicsStep(alpha) {
    const active = new Set((this.activeEdges || []).map(edge => mapEdgeKey(edge.source, edge.target))); for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second], key = mapEdgeKey(a.id, b.id), relationship = this.relationships.get(key); let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = relationship ? .16 + (1 - relationship.overall) * .34 + Math.max(0, -relationship.residual) * .1 : .5, attraction = relationship && active.has(key) ? (distance - desired) * (.0018 + relationship.overall * .0052) * alpha : 0, contrast = relationship ? -Math.max(0, -relationship.residual) * (.004 + relationship.overall * .003) * alpha : 0, charge = -Math.min(.012, .0002 / (distance * distance)) * alpha, collisionDistance = .085 + (Number(a.fileScale ?? .35) + Number(b.fileScale ?? .35)) * .035, collision = distance < collisionDistance ? -(collisionDistance - distance) * .13 * alpha : 0, force = attraction + contrast + charge + collision; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
  ...
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = () => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .982; this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  draw() {
    const rect = this.canvas.getBoundingClientRect(); if (!rect.width || !rect.height) return; const dpr = window.devicePixelRatio || 1; this.canvas.width = Math.round(rect.width * dpr); this.canvas.height = Math.round(rect.height * dpr); const ctx = this.canvas.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, rect.width, rect.height); const colors = this.colors(), focused = this.hovered || this.selected, queryFocused = focused === this.queryNode.id; this.hit = []; for (const node of this.nodes) { const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), radius = 3.2 + Math.max(0, Math.min(1, Number(node.fileScale ?? .35))) * 4.8 + (active ? 1.2 : 0); ctx.globalAlpha = !focused ? .72 + node.relevance * .2 : active ? 1 : queryFocused ? .4 + node.relevance * .48 : .3 + related * .55; ctx.fillStyle = active || node.id === this.selected ? colors.accent : colors.normal; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (active || node.order < 4) this.label(ctx, node.label, x + radius + 5, y + 4, colors, active); this.hit.push({ node, x, y, radius: radius + 11 }); } const [queryX, queryY] = this.coordinates(this.queryNode, rect.width, rect.height); ctx.globalAlpha = 1; ctx.fillStyle = colors.background; ctx.strokeStyle = colors.accent; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(queryX, queryY, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); this.label(ctx, this.center?.label || '', queryX, queryY + 29, colors, true, true); this.hit.push({ node: this.queryNode, x: queryX, y: queryY, radius: 18 });
  }
