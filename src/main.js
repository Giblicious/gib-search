...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  paintRoads(ctx, width, height, colors) {
    if (!this.roadEdges.length || !this.lastTerrainField) return; const now = performance.now(), moving = this.alpha >= .22 || this.dragging || this.panning; if (this.roadsDirty && this.roadWorkHandle === null && !moving) this.beginRoadRouteRefresh(this.lastTerrainField, width, height); if (this.roadTrafficRouteCount !== this.roadRoutes.size) { this.rebuildRoadTrafficField(this.lastTerrainField, width, height); this.roadTrafficRouteCount = this.roadRoutes.size; } const focused = this.hovered || this.selected, reduced = matchMedia('(prefers-reduced-motion: reduce)').matches, orderedRoutes = [...this.roadRoutes].sort((first, second) => Number(first[1].networkWeight || 0) - Number(second[1].networkWeight || 0)); let animating = false;
    for (const [key, route] of orderedRoutes) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const opacity = Math.min(source.visibility, target.visibility); if (opacity < .035) continue; const targetPoints = this.adjustedRoadPoints(route, source, target), transition = reduced ? 1 : Math.max(0, Math.min(1, (now - Number(route.transitionAt || 0)) / 220)), eased = 1 - (1 - transition) ** 3, points = route.previousPoints && transition < 1 ? targetPoints.map((point, index) => ({ x: route.previousPoints[index].x + (point.x - route.previousPoints[index].x) * eased, y: route.previousPoints[index].y + (point.y - route.previousPoints[index].y) * eased })) : targetPoints, introduced = this.roadIntroducedAt.get(key) || now, fade = reduced ? 1 : Math.max(0, Math.min(1, (now - introduced) / 160)); if (transition < 1 || fade < 1) animating = true; const active = focused === route.road.source || focused === route.road.target, trunk = Math.min(3.4, Math.sqrt(Math.max(0, Number(route.networkWeight || 0))) * .58); ctx.globalAlpha = opacity * fade * (active ? .82 : .22 + Math.min(.18, trunk * .045)); ctx.strokeStyle = active ? colors.normal : colors.muted; ctx.lineWidth = (active ? 1.5 : .78) + trunk; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; if (this.traceRoad(ctx, points, 1)) ctx.stroke(); const chevron = (fraction, reverse = false) => { const point = this.roadPointAt(points, fraction), angle = point.angle + (reverse ? Math.PI : 0), size = active ? 3.8 : 3.1; ctx.lineWidth = active ? 1.4 : .8; ctx.beginPath(); ctx.moveTo(point.x - Math.cos(angle - .62) * size, point.y - Math.sin(angle - .62) * size); ctx.lineTo(point.x, point.y); ctx.lineTo(point.x - Math.cos(angle + .62) * size, point.y - Math.sin(angle + .62) * size); ctx.stroke(); }; if (route.road.forward) chevron(.62); if (route.road.reverse) chevron(.38, true); }
