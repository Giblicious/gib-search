...
function circumcircle(first, second, third) {
  const denominator = 2 * (first.x * (second.y - third.y) + second.x * (third.y - first.y) + third.x * (first.y - second.y)); if (Math.abs(denominator) < 1e-8) return null;
  const a = first.x ** 2 + first.y ** 2, b = second.x ** 2 + second.y ** 2, c = third.x ** 2 + third.y ** 2; const x = (a * (second.y - third.y) + b * (third.y - first.y) + c * (first.y - second.y)) / denominator, y = (a * (third.x - second.x) + b * (first.x - third.x) + c * (second.x - first.x)) / denominator;
  return { x, y, radiusSquared: (x - first.x) ** 2 + (y - first.y) ** 2 };
}
function triangulateTerrain(input) {
  if (input.length < 3) return []; const points = input.map(point => ({ ...point })); const extent = Math.max(1, ...points.flatMap(point => [Math.abs(point.x), Math.abs(point.y)])); const start = points.length; points.push({ x: -20 * extent, y: -12 * extent }, { x: 0, y: 20 * extent }, { x: 20 * extent, y: -12 * extent }); let triangles = [[start, start + 1, start + 2]];
  for (let index = 0; index < start; index++) { const bad = triangles.filter(triangle => { const circle = circumcircle(points[triangle[0]], points[triangle[1]], points[triangle[2]]); return circle && (points[index].x - circle.x) ** 2 + (points[index].y - circle.y) ** 2 <= circle.radiusSquared + 1e-8; }); const boundary = new Map(); for (const triangle of bad) for (const edge of [[triangle[0], triangle[1]], [triangle[1], triangle[2]], [triangle[2], triangle[0]]]) { const key = edge[0] < edge[1] ? `${edge[0]}:${edge[1]}` : `${edge[1]}:${edge[0]}`; if (boundary.has(key)) boundary.delete(key); else boundary.set(key, edge); } const removed = new Set(bad); triangles = triangles.filter(triangle => !removed.has(triangle)); for (const edge of boundary.values()) triangles.push([edge[0], edge[1], index]); }
  return triangles.filter(triangle => triangle.every(index => index < start)).map(triangle => triangle.map(index => points[index]));
}
class SemanticMapCanvas {
  ...
  setGraph(center, values, edges = []) {
    ...
    this.center = center; this.edges = edges; this.nodes = values.map((value, order) => {
      ...
      return { ...value, order, relevance, elevation: .12 + relevance * .76, radius, x: old?.x ?? 0, y: old?.y ?? 0, targetX: Math.cos(angle) * radius, targetY: Math.sin(angle) * radius };
    });
  ...
  drawContours(ctx, width, height, colors) {
    const points = [{ x: 0, y: 0, z: 1 }, ...this.nodes.map(node => ({ x: Number(node.renderX ?? node.targetX), y: Number(node.renderY ?? node.targetY), z: node.elevation }))], triangles = triangulateTerrain(points); if (!triangles.length) return; const scale = Math.max(20, Math.min(width, height) / 2 - 42), convert = point => [width / 2 + point.x * scale, height / 2 + point.y * scale]; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    for (let step = 1; step <= 9; step++) { const level = step / 10, major = step % 2 === 0; ctx.beginPath(); for (const triangle of triangles) { const crossings = []; for (const [first, second] of [[triangle[0], triangle[1]], [triangle[1], triangle[2]], [triangle[2], triangle[0]]]) { if ((first.z < level) === (second.z < level) || first.z === second.z) continue; const amount = (level - first.z) / (second.z - first.z); crossings.push({ x: first.x + (second.x - first.x) * amount, y: first.y + (second.y - first.y) * amount }); } if (crossings.length === 2) { const from = convert(crossings[0]), to = convert(crossings[1]); ctx.moveTo(from[0], from[1]); ctx.lineTo(to[0], to[1]); } } ctx.globalAlpha = major ? .34 : .2; ctx.strokeStyle = major ? colors.normal : colors.muted; ctx.lineWidth = major ? 1.05 : .7; ctx.stroke(); }
    ctx.globalAlpha = 1;
  }
  draw() {
...
function contourSegments(field, size, level = 0) {
  const segments = [], edgePoint = (edge, column, row, corners) => { const definitions = [[0, 1, column, row, column + 1, row], [1, 2, column + 1, row, column + 1, row + 1], [2, 3, column + 1, row + 1, column, row + 1], [3, 0, column, row + 1, column, row]], [first, second, x1, y1, x2, y2] = definitions[edge], amount = (level - corners[first]) / (corners[second] - corners[first]); return { x: x1 + (x2 - x1) * amount, y: y1 + (y2 - y1) * amount }; };
  for (let row = 0; row < size - 1; row++) for (let column = 0; column < size - 1; column++) { const corners = [field[row * size + column], field[row * size + column + 1], field[(row + 1) * size + column + 1], field[(row + 1) * size + column]], mask = corners.reduce((value, corner, index) => value | (corner >= level ? 1 << index : 0), 0); if (!mask || mask === 15) continue; let pairs = { 1: [[3, 0]], 2: [[0, 1]], 3: [[3, 1]], 4: [[1, 2]], 6: [[0, 2]], 7: [[3, 2]], 8: [[2, 3]], 9: [[0, 2]], 11: [[1, 2]], 12: [[1, 3]], 13: [[0, 1]], 14: [[3, 0]] }[mask]; if (!pairs) { const centerInside = corners.reduce((sum, value) => sum + value, 0) / 4 >= level; pairs = mask === 5 ? (centerInside ? [[0, 1], [2, 3]] : [[3, 0], [1, 2]]) : (centerInside ? [[3, 0], [1, 2]] : [[0, 1], [2, 3]]); } for (const [first, second] of pairs) segments.push([edgePoint(first, column, row, corners), edgePoint(second, column, row, corners)]); }
  return segments;
}
function stitchContourSegments(segments) {
  const key = point => `${Math.round(point.x * 1000)}:${Math.round(point.y * 1000)}`, byEndpoint = new Map(); segments.forEach((segment, index) => segment.forEach(point => { const value = key(point); if (!byEndpoint.has(value)) byEndpoint.set(value, []); byEndpoint.get(value).push(index); })); const unused = new Set(segments.map((_, index) => index)), paths = [];
  while (unused.size) { const firstIndex = unused.values().next().value, path = [...segments[firstIndex]]; unused.delete(firstIndex); const extend = atStart => { while (true) { const endpoint = atStart ? path[0] : path[path.length - 1], nextIndex = (byEndpoint.get(key(endpoint)) || []).find(index => unused.has(index)); if (nextIndex === undefined) break; const segment = segments[nextIndex], next = key(segment[0]) === key(endpoint) ? segment[1] : segment[0]; unused.delete(nextIndex); if (atStart) path.unshift(next); else path.push(next); } }; extend(false); extend(true); if (path.length > 5) paths.push(path); }
  return paths;
}
function prepareHeatInfluence(influence) { const angle = influence.angle || 0; return { ...influence, cosine: Math.cos(angle), sine: Math.sin(angle), inverseAlong: 1 / (influence.along * influence.along), inverseAcross: 1 / (influence.across * influence.across) }; }
function ellipticalHeat(x, y, influence) { const dx = x - influence.x, dy = y - influence.y, along = dx * influence.cosine + dy * influence.sine, across = -dx * influence.sine + dy * influence.cosine; return influence.weight * Math.exp(-.5 * (along * along * influence.inverseAlong + across * across * influence.inverseAcross)); }
function buildSemanticHeatField(query, nodes, edges, size = 150) {
  const bodies = [query, ...nodes], byId = new Map(nodes.map(node => [node.id, node])), positiveByNode = new Map(nodes.map(node => [node.id, []])); for (const edge of edges || []) if (edge.residual > 0) { positiveByNode.get(edge.source)?.push({ edge, other: byId.get(edge.target) }); positiveByNode.get(edge.target)?.push({ edge, other: byId.get(edge.source) }); }
  const influences = bodies.map(body => { const distances = bodies.filter(other => other !== body).map(other => Math.hypot(other.x - body.x, other.y - body.y)).sort((a, b) => a - b), localSpacing = median(distances.slice(0, 3)), bandwidth = Math.max(.13, Math.min(.27, .095 + localSpacing * .38)); if (body.isQuery) return prepareHeatInfluence({ x: body.x, y: body.y, weight: 1.7, along: bandwidth * 1.08, across: bandwidth * 1.08, angle: 0 }); let directionX = 0, directionY = 0, directionWeight = 0; const relationships = [...(positiveByNode.get(body.id) || []), { edge: { strength: .45 + body.relevance * .4, residual: 1 }, other: query }]; for (const { edge, other } of relationships) { if (!other) continue; const dx = other.x - body.x, dy = other.y - body.y, distance = Math.max(.001, Math.hypot(dx, dy)), weight = edge.strength * Math.max(0, edge.residual); directionX += dx / distance * weight; directionY += dy / distance * weight; directionWeight += weight; } const coherence = directionWeight ? Math.min(1, Math.hypot(directionX, directionY) / directionWeight) : 0; return prepareHeatInfluence({ x: body.x, y: body.y, weight: .2 + body.relevance * .58, along: bandwidth * (1 + coherence * .9), across: bandwidth * (1 - coherence * .18), angle: Math.atan2(directionY, directionX) }); });
  const relational = (edges || []).map(edge => { const source = byId.get(edge.source), target = byId.get(edge.target); if (!source || !target || Math.abs(edge.residual) < .04) return null; const distance = Math.max(.02, Math.hypot(target.x - source.x, target.y - source.y)), positive = edge.residual > 0; return prepareHeatInfluence({ x: (source.x + target.x) / 2, y: (source.y + target.y) / 2, angle: Math.atan2(target.y - source.y, target.x - source.x), along: distance * .52 + .055, across: .055 + edge.strength * .035, weight: (positive ? .2 : -.16) * edge.strength * Math.abs(edge.residual) }); }).filter(Boolean), extent = Math.min(1.18, Math.max(.46, ...bodies.map(body => Math.max(Math.abs(body.x), Math.abs(body.y)) + .32))), field = new Float32Array(size * size); let maximum = 0;
  for (let row = 0; row < size; row++) for (let column = 0; column < size; column++) { const x = (column / (size - 1) * 2 - 1) * extent, y = (row / (size - 1) * 2 - 1) * extent; let heat = 0; for (const influence of influences) heat += ellipticalHeat(x, y, influence); for (const influence of relational) heat += ellipticalHeat(x, y, influence); heat = Math.max(0, heat); field[row * size + column] = heat; maximum = Math.max(maximum, heat); }
  if (maximum > 0) for (let index = 0; index < field.length; index++) { const normalized = Math.pow(field[index] / maximum, .82); field[index] = normalized < .025 ? 0 : normalized; } return { size, field, extent };
}
function drawSmoothContour(ctx, path, toCanvas) {
  const points = path.map(point => toCanvas(point.x, point.y)), closed = Math.hypot(points[0][0] - points[points.length - 1][0], points[0][1] - points[points.length - 1][1]) < 2; if (closed) points.pop(); if (points.length < 4) return; const midpoint = (first, second) => [(first[0] + second[0]) / 2, (first[1] + second[1]) / 2], start = closed ? midpoint(points[points.length - 1], points[0]) : points[0]; ctx.moveTo(start[0], start[1]); const end = closed ? points.length : points.length - 1; for (let index = 0; index < end; index++) { const point = points[index], next = points[(index + 1) % points.length], middle = midpoint(point, next); ctx.quadraticCurveTo(point[0], point[1], middle[0], middle[1]); } if (closed) ctx.closePath(); else ctx.lineTo(points[points.length - 1][0], points[points.length - 1][1]);
}
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  constructor(host, app, options = {}) { super(host, app, options); cancelAnimationFrame(this.animationFrame); this.animationFrame = null; this.simulationFrame = null; this.alpha = 0; this.lastTerrainAt = 0; }
  setGraph(center, values, edges = []) {
    const sameCenter = this.center?.label === center?.label, previous = sameCenter ? this.byId : new Map(), previousQuery = sameCenter ? this.queryNode : null, scores = values.map(value => Number(value.score || 0)), low = scores.length ? Math.min(...scores) : 0, high = scores.length ? Math.max(...scores) : 1, spread = Math.max(.001, high - low), vaultFiles = this.app.vault.getFiles().filter(file => /\.(?:md|txt|markdown)$/i.test(file.path)), vaultSizes = vaultFiles.map(file => Math.log1p(Number(file.stat?.size || 0))), vaultLow = vaultSizes.length ? Math.min(...vaultSizes) : 0, vaultHigh = vaultSizes.length ? Math.max(...vaultSizes) : 1, vaultSpread = Math.max(.001, vaultHigh - vaultLow), vaultScale = new Map(vaultFiles.map(file => [file.path, (Math.log1p(Number(file.stat?.size || 0)) - vaultLow) / vaultSpread])); this.center = center; this.edges = edges; this.queryNode = { id: '__query__', label: center?.label || 'Query', isQuery: true, x: previousQuery?.x || 0, y: previousQuery?.y || 0, vx: previousQuery?.vx || 0, vy: previousQuery?.vy || 0 };
    this.nodes = values.map((value, order) => { const relevance = spread > .001 && Number.isFinite(Number(value.score)) ? (Number(value.score) - low) / spread : 1 - order / Math.max(1, values.length), projected = Number.isFinite(value.x) && Number.isFinite(value.y), angle = stableMapAngle(value.id), old = previous.get(value.id); return { ...value, fileScale: value.fileScale ?? vaultScale.get(value.id) ?? .35, order, relevance, elevation: .08 + relevance * .88, x: old?.x ?? (projected ? value.x : Math.cos(angle) * .45), y: old?.y ?? (projected ? value.y : Math.sin(angle) * .45), vx: old?.vx || 0, vy: old?.vy || 0 }; });
    const overallScores = edges.map(edge => Number(edge.score || 0)), overallLow = overallScores.length ? Math.min(...overallScores) : 0, overallHigh = overallScores.length ? Math.max(...overallScores) : 1, overallSpread = Math.max(.001, overallHigh - overallLow), relationships = edges.map(edge => { const overall = (Number(edge.score || overallLow) - overallLow) / overallSpread, residual = Math.max(-1, Math.min(1, Number(edge.residualScore || 0))); return { ...edge, overall, residual, strength: overall * (.65 + Math.max(0, residual) * .35) }; }); this.relationships = new Map(relationships.map(edge => [mapEdgeKey(edge.source, edge.target), edge])); const nearest = new Map(this.nodes.map(node => [node.id, []])); for (const edge of relationships) { nearest.get(edge.source)?.push(edge); nearest.get(edge.target)?.push(edge); } const activeKeys = new Set(); for (const candidates of nearest.values()) for (const edge of candidates.sort((first, second) => second.strength - first.strength).slice(0, 3)) if (edge.strength > .08) activeKeys.add(mapEdgeKey(edge.source, edge.target)); this.activeEdges = relationships.filter(edge => activeKeys.has(mapEdgeKey(edge.source, edge.target))); for (let step = 0; step < 36; step++) this.physicsStep(.9);
    this.byId = new Map(this.nodes.map(node => [node.id, node])); this.selected = this.byId.has(this.selected) ? this.selected : null; this.hovered = this.byId.has(this.hovered) ? this.hovered : null; this.statusEl.textContent = `${this.nodes.length} note${this.nodes.length === 1 ? '' : 's'}`; this.terrain = null; this.startSimulation(.9); this.updateDetail();
  }
  physicsStep(alpha) {
    const active = new Set((this.activeEdges || []).map(edge => mapEdgeKey(edge.source, edge.target))); for (let first = 0; first < this.nodes.length; first++) for (let second = first + 1; second < this.nodes.length; second++) { const a = this.nodes[first], b = this.nodes[second], key = mapEdgeKey(a.id, b.id), relationship = this.relationships.get(key); let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const attraction = relationship && active.has(key) ? (distance - (.14 + (1 - relationship.strength) * .3)) * (.0025 + relationship.strength * .0065) * alpha : 0, contrast = relationship ? -Math.max(0, -relationship.residual) * .006 * alpha : 0, charge = -Math.min(.014, .00022 / (distance * distance)) * alpha, collision = distance < .12 ? -(0.12 - distance) * .11 * alpha : 0, force = attraction + contrast + charge + collision; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    const query = this.queryNode; for (const node of this.nodes) { let dx = node.x - query.x, dy = node.y - query.y; const distance = Math.max(.025, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .14 + (1 - node.relevance) * .56, force = (distance - desired) * (.0035 + node.relevance * .007) * alpha; if (query !== this.dragging) { query.vx += dx * force; query.vy += dy * force; } if (node !== this.dragging) { node.vx -= dx * force; node.vy -= dy * force; } }
    const bodies = [query, ...this.nodes]; for (const body of bodies) { if (body === this.dragging) { body.vx = body.vy = 0; continue; } body.vx *= .885; body.vy *= .885; body.x += body.vx; body.y += body.vy; } if (!this.dragging) { const centerX = bodies.reduce((sum, body) => sum + body.x, 0) / Math.max(1, bodies.length), centerY = bodies.reduce((sum, body) => sum + body.y, 0) / Math.max(1, bodies.length); for (const body of bodies) { body.x -= centerX; body.y -= centerY; } const extent = Math.max(.001, ...bodies.map(body => Math.hypot(body.x, body.y))); if (extent > .94) for (const body of bodies) { body.x *= .94 / extent; body.y *= .94 / extent; body.vx *= .94 / extent; body.vy *= .94 / extent; } }
  }
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = time => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .985; if (!this.terrain || time - this.lastTerrainAt > 105) { this.terrain = buildSemanticHeatField(this.queryNode, this.nodes, this.activeEdges, this.alpha < .08 ? 180 : 105); this.lastTerrainAt = time; } this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); else { this.terrain = buildSemanticHeatField(this.queryNode, this.nodes, this.activeEdges, 220); this.draw(); } }; this.simulationFrame = requestAnimationFrame(tick); }
  coordinates(node, width, height) { const scale = Math.max(20, Math.min(width, height) / 2 - 68); return [width / 2 + Number(node.x) * scale, height / 2 + Number(node.y) * scale]; }
  drawContours(ctx, width, height, colors) {
    if (!this.terrain) return; const { size, field, extent } = this.terrain, scale = Math.max(20, Math.min(width, height) / 2 - 68), toCanvas = (column, row) => [width / 2 + ((column / (size - 1) * 2 - 1) * extent) * scale, height / 2 + ((row / (size - 1) * 2 - 1) * extent) * scale], levels = [.08, .21, .35, .5, .67, .84]; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    for (let band = 0; band < levels.length; band++) { const outer = band === 0, paths = stitchContourSegments(contourSegments(field, size, levels[band])); ctx.beginPath(); for (const path of paths) drawSmoothContour(ctx, path, toCanvas); ctx.globalAlpha = outer ? .16 : .21 + band * .025; ctx.strokeStyle = outer ? colors.muted : colors.normal; ctx.lineWidth = outer ? .68 : .76 + band * .05; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
    const rect = this.canvas.getBoundingClientRect(); if (!rect.width || !rect.height) return; const dpr = window.devicePixelRatio || 1; this.canvas.width = Math.round(rect.width * dpr); this.canvas.height = Math.round(rect.height * dpr); const ctx = this.canvas.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, rect.width, rect.height); const colors = this.colors(); this.drawContours(ctx, rect.width, rect.height, colors); const focused = this.hovered || this.selected; this.hit = []; for (const node of this.nodes) { const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, radius = 3.2 + Math.max(0, Math.min(1, Number(node.fileScale ?? .35))) * 4.8 + (active ? 1.2 : 0); ctx.globalAlpha = focused && !active ? .45 : active ? 1 : .72 + node.relevance * .2; ctx.fillStyle = active || node.id === this.selected ? colors.accent : colors.normal; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (active || node.order < 4) this.label(ctx, node.label, x + radius + 5, y + 4, colors, active); this.hit.push({ node, x, y, radius: radius + 11 }); } const [queryX, queryY] = this.coordinates(this.queryNode, rect.width, rect.height); ctx.globalAlpha = 1; ctx.fillStyle = colors.background; ctx.strokeStyle = colors.accent; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(queryX, queryY, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); this.label(ctx, this.center?.label || '', queryX, queryY + 29, colors, true, true); this.hit.push({ node: this.queryNode, x: queryX, y: queryY, radius: 18 });
  }
  pointerDown(event) { const node = this.hitAt(event); if (!node) return; event.preventDefault(); this.dragging = node; this.canvas.style.cursor = 'grabbing'; this.canvas.setPointerCapture?.(event.pointerId); if (!node.isQuery) this.setSelected(node.id); this.startSimulation(.45); }
  pointerMove(event) { if (this.dragging) { const rect = this.canvas.getBoundingClientRect(), scale = Math.max(20, Math.min(rect.width, rect.height) / 2 - 68); this.dragging.x = (event.clientX - rect.left - rect.width / 2) / scale; this.dragging.y = (event.clientY - rect.top - rect.height / 2) / scale; this.dragging.vx = this.dragging.vy = 0; this.terrain = null; this.startSimulation(.38); return; } const node = this.hitAt(event); this.canvas.style.cursor = node ? 'grab' : 'default'; this.setHover(node?.id || null, true); }
  pointerUp(event) { if (!this.dragging) return; this.dragging = null; this.canvas.style.cursor = 'default'; this.canvas.releasePointerCapture?.(event.pointerId); this.startSimulation(.78); }
  click(event) { const node = this.hitAt(event); if (node?.isQuery) return; super.click(event); }
  open(event) { const node = this.hitAt(event); if (node?.isQuery) return; super.open(event); }
...
class SemanticSearchModal extends SuggestModal {
  ...
  setupMap() {
    ...
    this.map = new LivingSemanticMapCanvas(panel, this.app, { title: 'Search map', onHover: file => this.hoverResult(file), onSelect: file => this.focusResult(file), onOpen: file => this.openFile(file), onExplore: file => { this.close(); this.plugin.openNeighborhood(file, true); } });
  ...
  async updateMap() {
    ...
    this.map.setGraph({ label: this.lastQuery }, terrain.nodes.map(node => ({ ...node, score: Number(byFile.get(node.id)?.score || 0), fileScale: fileScale.get(node.id) ?? .35 })), terrain.edges || []);
  }
