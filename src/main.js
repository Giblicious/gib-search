...
class NeighborhoodView extends ItemView {
  ...
  async onOpen() {
    ...
    const mapHost = this.contentEl.createDiv({ cls: 'gib-neighborhood-map' }); this.map = new LivingSemanticMapCanvas(mapHost, this.app, { title: 'Closest notes', showLinks: this.plugin.settings.showWikilinks, manualLinkInfluence: this.plugin.settings.graphManualLinkInfluence, onShowLinks: async value => { this.plugin.settings.showWikilinks = value; await this.plugin.save(); }, onManualLinkInfluence: async value => { this.plugin.settings.graphManualLinkInfluence = value; await this.plugin.save(); }, onSelect: file => this.openFile(file), onOpen: file => this.openFile(file) });
  ...
  async centerOn(filePath, pin = this.pinned) {
    ...
    try { const state = this.plugin.atlas.state({ anchor: { type: 'note', value: file.path }, lens: this.lens, scale: 'neighborhood' }), scene = await this.plugin.atlas.scene(state); if (version !== this.loadVersion) return; this.map.setMapGroupingMode(atlasLens(this.lens).mapMode); this.map.setTitle(`${SEARCH_LENSES[this.lens].label} · ${file.basename}`); this.map.setGraph(scene.center, scene.nodes, scene.edges, scene.roads); this.map.setIntelligenceStatus(''); }
...
class GraphView extends ItemView {
  ...
  async onOpen() {
    ...
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      ...
      onHover: file => this.hoverResult(file), onSelect: file => file ? this.showPreview(file) : this.closePreview(), onOpen: file => this.openFile(file)
    });
  ...
  handleQuery(value) {
    const query = String(value || '').trim(), version = ++this.searchVersion; this.query = query; window.clearTimeout(this.searchTimer); this.plugin.search.cancelLiveSearch?.();
