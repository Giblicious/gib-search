...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  physicsStep(alpha) {
    ...
    const query = this.queryNode, foreground = this.nodes.filter(node => node.matched), preserveBackground = Boolean(this.options.preserveBackground), activeNodes = preserveBackground ? this.nodes : this.hasQuery && !this.pendingQuery ? foreground : this.nodes, collisionNodes = preserveBackground && this.hasQuery && !this.pendingQuery ? foreground : activeNodes, vaultMode = !this.hasQuery && !this.pendingQuery, tuning = this.tuning || MAP_TUNING_DEFAULTS, anchorStrength = vaultMode ? tuning.anchorStrength : 1; for (const node of activeNodes) { if (node === this.dragging) continue; const strength = preserveBackground && this.hasQuery && !node.matched ? .0035 : .018 * anchorStrength; node.vx += (node.layoutX - node.x) * strength * alpha; node.vy += (node.layoutY - node.y) * strength * alpha; } if (this.queryPresence > .01 && query !== this.dragging) { query.vx += (query.layoutX - query.x) * .012 * alpha; query.vy += (query.layoutY - query.y) * .012 * alpha; }
    ...
    if (this.mapGroupingMode === 'topics' && tuning.boundaryRepulsionEnabled && Number(tuning.boundaryRepulsion) > 0) { const threshold = Number(tuning.communityMembership || .42), minimum = Math.max(2, Number(tuning.communityMinSize || 3)), groups = new Map(), sigma = .11 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))), divisor = 2 * sigma * sigma; for (const node of activeNodes) if (Number(node.communityMembership || 0) >= threshold) { const values = groups.get(node.community) || []; values.push(node); groups.set(node.community, values); } for (const [community, members] of groups) { if (members.length < minimum) continue; for (const node of activeNodes) { if (node.community === community || node === this.dragging) continue; let density = 0, gradientX = 0, gradientY = 0; for (const member of members) { const dx = node.x - member.x, dy = node.y - member.y, weight = Math.exp(-(dx * dx + dy * dy) / divisor); density += weight; gradientX += dx * weight; gradientY += dy * weight; } if (density < .34) continue; const length = Math.max(.008, Math.hypot(gradientX, gradientY)), force = Math.min(.012, (density - .34) * .0028) * Number(tuning.boundaryRepulsion) * alpha; node.vx += gradientX / length * force; node.vy += gradientY / length * force; } } }
...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.results = []; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.mapGroupingMode = validMapGrouping(plugin.settings.mapGroupingMode);
  }
  ...
  async onOpen() {
    ...
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      title: 'Vault map', preserveBackground: true, mapGroupingMode: this.mapGroupingMode, showLinks: this.plugin.settings.showWikilinks, manualLinkInfluence: this.plugin.settings.graphManualLinkInfluence, semanticColors: this.plugin.settings.graphSemanticColors, tuning: this.plugin.settings.mapTuning,
      ...
      onShowLinks: async value => { this.plugin.settings.showWikilinks = value; await this.plugin.save(); }, onManualLinkInfluence: async value => { this.plugin.settings.graphManualLinkInfluence = value; await this.plugin.save(); }, onHover: file => this.hoverResult(file), onSelect: file => this.focusResult(file), onOpen: file => this.openFile(file)
    });
  ...
  fileScales(files) { const sizes = files.map(file => Math.log1p(Number(file.stat?.size || 0))), low = sizes.length ? Math.min(...sizes) : 0, high = sizes.length ? Math.max(...sizes) : 1, spread = Math.max(.001, high - low); return new Map(files.map(file => [file.path, (Math.log1p(Number(file.stat?.size || 0)) - low) / spread])); }
  async loadVault() {
    const version = ++this.loadVersion; this.map?.setTitle('Loading vault…');
    try {
      const files = this.indexableFiles(), paths = files.map(file => file.path), scales = this.fileScales(files), graph = await this.plugin.search.semanticStarfield('', paths); if (version !== this.loadVersion || this.query.length >= 3) return;
  ...
  async runSearch(query, immediate = false) {
    ...
    try {
      ...
      const results = groupSearchResults(raw, query, limit), resultFiles = results.map(result => result.file), graph = await this.plugin.search.semanticStarfield(query, resultFiles, resultFiles), facets = await this.plugin.search.conceptFacets(query, resultFiles); if (version !== this.searchVersion || query !== this.query) return;
      const graphById = new Map(graph.nodes.map(node => [node.id, node])), scores = results.map(result => Number(result.score || 0)), low = scores.length ? Math.min(...scores) : 0, high = scores.length ? Math.max(...scores) : 1, spread = Math.max(.001, high - low), baseById = new Map(this.baseNodes.map(node => [node.id, node]));
      const queryNodes = results.map(result => { const node = graphById.get(result.file) || { id: result.file, label: result.file.replace(/\.md$/i, '').split('/').pop() }, facet = facets.get(result.file); return { ...node, matched: true, generation: 1, relevance: .08 + (Number(result.score || 0) - low) / spread * .92, fileScale: baseById.get(result.file)?.fileScale ?? .35, facet: facet?.facet ?? node.community, conceptAffinities: facet?.affinities || node.topicAffinities }; });
      const layout = await this.plugin.search.multiRelationalLayout(query, queryNodes, new Map(), { magic: this.plugin.settings.magicGraphEnabled, lens: 'relevance', mapMode: this.mapGroupingMode }); if (version !== this.searchVersion || query !== this.query) return; const queryById = new Map(queryNodes.map(node => [node.id, { ...node, layoutX: layout.get(node.id)?.x, layoutY: layout.get(node.id)?.y }])), merged = this.baseNodes.map(base => queryById.get(base.id) || { ...base, matched: false, relevance: 0 });
      this.results = results; this.renderResults(); this.map.setTitle(`${results.length} results · ${this.mapGroupingMode === 'topics' ? 'Topics' : 'Similarity'}`); this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, merged, graph.edges || [], this.baseRoads);
