...
const MAP_TUNING_DEFAULTS = { contentWeight: .72, residualWeight: .18, semanticWeight: .06, entityWeight: .04, commonnessSuppression: .75, passageCoverage: .72, passageDiversity: .28, distanceContrast: 1, neighborAttraction: 0, collisionSpacing: 1, repulsion: 0, anchorStrength: 1, terrainSpread: 1, terrainContrast: 1 };
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setupTuningPanel() {
    ...
    for (const control of MAP_TUNING_CONTROLS) { if (control.section) this.tuningPanel.createDiv({ cls: 'gib-search-map-tuning-section', text: control.section }); const row = this.tuningPanel.createDiv({ cls: 'gib-search-map-tuning-row' }), label = row.createEl('label', { text: control.label }), input = row.createEl('input', { attr: { type: 'range', min: String(control.min), max: String(control.max), step: String(control.step), value: String(this.tuning[control.key]) } }), output = row.createEl('output'); label.htmlFor = `gib-map-${control.key}`; input.id = `gib-map-${control.key}`; output.textContent = Number(this.tuning[control.key]).toFixed(control.step < .05 ? 2 : 1); input.addEventListener('input', () => { this.tuning[control.key] = Number(input.value); output.textContent = Number(input.value).toFixed(control.step < .05 ? 2 : 1); this.lastTerrainAt = 0; this.startSimulation(.58); this.options.onTune({ ...this.tuning }, control.key); }); this.tuningInputs.set(control.key, { input, output, control }); }
  ...
  setTuning(values, notify = false, key = 'reset') { this.tuning = Object.assign({}, MAP_TUNING_DEFAULTS, values || {}); for (const [name, view] of this.tuningInputs || []) { view.input.value = String(this.tuning[name]); view.output.textContent = Number(this.tuning[name]).toFixed(view.control.step < .05 ? 2 : 1); } this.lastTerrainAt = 0; this.startSimulation(.72); if (notify) this.options.onTune?.({ ...this.tuning }, key); }
