...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setMapGroupingMode(value) { this.mapGroupingMode = validMapGrouping(value); this.mapModeButtons?.forEach((button, index) => { const active = ['similarity', 'topics', 'links'][index] === this.mapGroupingMode; button.toggleClass('is-active', active); button.setAttribute('aria-pressed', String(active)); }); }
  setupTuningPanel() {
  ...
  setManualLinkInfluence(value, animate = true) { this.manualLinkInfluence = Boolean(value); this.linkInfluenceButton?.toggleClass('is-active', this.manualLinkInfluence); this.linkInfluenceButton?.setAttribute('aria-pressed', String(this.manualLinkInfluence)); this.linkInfluenceButton?.setAttribute('title', this.manualLinkInfluence ? 'Manual links gently influence placement' : 'Manual links do not influence placement'); if (animate) this.startSimulation(.72); }
  ...
  physicsStep(alpha) {
    ...
    if ((this.manualLinkInfluence || this.mapGroupingMode === 'links') && this.roadEdges.length) { const isActive = body => body === query ? this.queryPresence > .04 : !this.hasQuery || this.pendingQuery || body.matched, linkMode = this.mapGroupingMode === 'links'; for (const edge of this.roadEdges) { const source = edge.source === this.center?.id ? query : this.byId.get(edge.source), target = edge.target === this.center?.id ? query : this.byId.get(edge.target); if (!source || !target || !isActive(source) || !isActive(target)) continue; let dx = target.x - source.x, dy = target.y - source.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const hubScale = 1 / Math.sqrt(Math.max(1, Number(this.roadDegrees?.get(edge.source) || 1), Number(this.roadDegrees?.get(edge.target) || 1))), desired = linkMode ? .13 : .2, force = Math.tanh((distance - desired) * (linkMode ? 4.5 : 3.2)) * (linkMode ? .009 : .0036) * hubScale * alpha; if (source !== this.dragging) { source.vx += dx * force; source.vy += dy * force; } if (target !== this.dragging) { target.vx -= dx * force; target.vy -= dy * force; } } }
  ...
  coordinates(node, width, height) { let x = Number(node.x), y = Number(node.y); if (this.options.preserveBackground && this.hasQuery && !this.pendingQuery && !node.matched && !node.isQuery) { const angle = Math.sin((performance.now() - this.ambientStartedAt) * .00018) * .008, dx = x - this.queryNode.x, dy = y - this.queryNode.y, cosine = Math.cos(angle), sine = Math.sin(angle); x = this.queryNode.x + dx * cosine - dy * sine; y = this.queryNode.y + dx * sine + dy * cosine; } const scale = Math.max(20, Math.min(width, height) / 2 - 68) * this.cameraZoom * this.userZoom; return [width / 2 + (x - this.cameraX) * scale + this.panX, height / 2 + (y - this.cameraY) * scale + this.panY]; }
