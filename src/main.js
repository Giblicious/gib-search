...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  coordinates(node, width, height) { const point = node.isQuery ? { x: Number(node.x), y: Number(node.y) } : this.ambientPosition(node.x, node.y, false, node.id), scale = Math.max(20, Math.min(width, height) / 2 - 68) * this.cameraZoom * this.userZoom; return [width / 2 + (point.x - this.cameraX) * scale + this.panX, height / 2 + (point.y - this.cameraY) * scale + this.panY]; }
  semanticDensityField(width, height) {
    const tuning = this.tuning || MAP_TUNING_DEFAULTS, step = 5, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), heightWeights = new Float32Array(values.length), heightTotals = new Float32Array(values.length), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), vaultTerrain = !this.hasQuery && !this.pendingQuery, zoom = Math.max(.35, this.cameraZoom * this.userZoom), vaultDensityScale = this.hasQuery ? 1 : Math.max(.48, Math.min(1, Math.sqrt(48 / Math.max(1, visible.length)))), sigma = Math.max(this.hasQuery ? 9 : 11, 27 * zoom * vaultDensityScale) * (vaultTerrain ? Number(tuning.terrainSpread || 1) : 1), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y })), gaussian = ratio => this.gaussianLookup[Math.max(0, Math.min(1024, Math.round(ratio / 9 * 1024)))];
  ...
  topicCommunityGroups(width, height) {
    ...
    for (const node of visible) { const values = grouped.get(node.community) || []; const [x, y] = this.coordinates(node, width, height); values.push({ node, x, y }); grouped.set(node.community, values); }
    const groups = [...grouped].filter(([, values]) => values.length >= minimum).map(([id, values]) => { const weightedHue = values.filter(value => Number.isFinite(value.node.topicHue)), hueX = weightedHue.reduce((sum, value) => sum + Math.cos(value.node.topicHue * Math.PI / 180), 0), hueY = weightedHue.reduce((sum, value) => sum + Math.sin(value.node.topicHue * Math.PI / 180), 0); return { id, values, label: values.find(value => value.node.communityLabel)?.node.communityLabel || values.find(value => value.node.communityFallbackLabel)?.node.communityFallbackLabel || this.communityFallbackLabel(values), confidence: Math.max(...values.map(value => Number(value.node.communityLabelConfidence || 0))), naturalHue: weightedHue.length ? (Math.atan2(hueY, hueX) * 180 / Math.PI + 360) % 360 : stableMapAngle(id) * 180 / Math.PI }; }).sort((first, second) => first.naturalHue - second.naturalHue || String(first.id).localeCompare(String(second.id)));
  ...
  communityFallbackLabel(values) {
    const entities = new Map(), ignored = /^(?:note|notes|journal|entry|entries|study|thought|thoughts|idea|ideas|topic|topics|today|tomorrow|yesterday)$/i, temporal = /(?:^|\D)(?:19|20)\d{2}(?:\D|$)|\b\d{1,4}[-/.]\d{1,2}(?:[-/.]\d{1,4})?\b|\b(?:january|february|march|april|may|june|july|august|september|october|november|december|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i;
    for (const value of values) for (const entity of value.node.entities || []) { const text = String(entity || '').trim(); if (text.length < 3 || ignored.test(text) || temporal.test(text)) continue; const key = text.toLocaleLowerCase(); const item = entities.get(key) || { text, files: new Set() }; item.files.add(value.node.id); entities.set(key, item); }
    const entity = [...entities.values()].filter(item => item.files.size >= Math.min(2, values.length)).sort((first, second) => second.files.size - first.files.size || first.text.length - second.text.length || first.text.localeCompare(second.text))[0]; return entity ? entity.text.replace(/\b\p{L}/gu, letter => letter.toUpperCase()) : 'Mixed Topic';
  }
  ...
  drawCommunityLabels(ctx, colors) { for (const label of this.communityLabelsForDraw || []) { ctx.font = '600 9px -apple-system, BlinkMacSystemFont, sans-serif'; const text = String(label.text).toUpperCase(), width = ctx.measureText(text).width; ctx.globalAlpha = .88; ctx.fillStyle = colors.background; ctx.fillRect(label.x - width / 2 - 5, label.y - 8, width + 10, 15); ctx.globalAlpha = .72; ctx.fillStyle = label.color; ctx.textAlign = 'center'; ctx.fillText(text, label.x, label.y + 3); } ctx.globalAlpha = 1; ctx.textAlign = 'left'; }
  ...
  draw() {
    const rect = this.canvas.getBoundingClientRect(); if (!rect.width || !rect.height) return; const dpr = window.devicePixelRatio || 1, pixelWidth = Math.round(rect.width * dpr), pixelHeight = Math.round(rect.height * dpr); if (this.canvas.width !== pixelWidth) this.canvas.width = pixelWidth; if (this.canvas.height !== pixelHeight) this.canvas.height = pixelHeight; const ctx = this.canvas.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, rect.width, rect.height); const colors = this.colors(), focused = this.hovered || this.selected, queryFocused = focused === '__query__', labelNodes = new Set(); this.hit = []; this.paintDensityTerrain(ctx, rect.width, rect.height, colors); this.paintTopicCommunities(ctx, rect.width, rect.height, colors); this.paintSweepLinks(ctx, rect.width, rect.height, colors);
    if (focused && !queryFocused) labelNodes.add(focused);
    const labels = [], ordered = this.hasQuery && !this.pendingQuery ? [...this.nodes.filter(node => !node.matched), ...this.nodes.filter(node => node.matched)] : this.nodes; for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.01, node.visibility * focusAlpha), blur = Math.max(0, Math.min(1, Number(node.blur || 0))), hue = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicHue) ? node.displayTopicHue : node.topicHue, lightness = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicLightness) ? node.displayTopicLightness : 64, semanticColor = this.options.semanticColors !== false && Number.isFinite(hue) ? `hsl(${hue} 58% ${lightness}%)` : colors.normal; ctx.fillStyle = node.matched || !this.hasQuery || this.pendingQuery ? semanticColor : colors.faint; if (blur > .04) { ctx.globalAlpha = opacity * blur * .22; ctx.beginPath(); ctx.arc(x, y, radius + 2.2 * blur, 0, Math.PI * 2); ctx.fill(); } ctx.globalAlpha = opacity * (1 - blur * .28); ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery || this.options.preserveBackground) this.hit.push({ node, x, y, radius: radius + 9 }); }
  ...
  destroy() { if (this.ambientTimer) window.clearInterval(this.ambientTimer); cancelAnimationFrame(this.roadAnimationFrame); cancelAnimationFrame(this.simulationFrame); cancelAnimationFrame(this.animationFrame); this.resizeObserver?.disconnect(); }
}
...
module.exports = class GibSearch extends Plugin {
  async onload() {
    ...
    this.settings = Object.assign({}, DEFAULTS, loaded); this.settings.mapTuning = Object.assign({}, MAP_TUNING_DEFAULTS, loaded.mapTuning || {}); this.isMobile = Platform.isMobileApp;
