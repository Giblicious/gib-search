...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  physicsStep(alpha) {
    ...
    const query = this.queryNode, foreground = this.nodes.filter(node => node.matched), preserveBackground = Boolean(this.options.preserveBackground), activeNodes = preserveBackground ? this.nodes : this.hasQuery && !this.pendingQuery ? foreground : this.nodes, vaultMode = !this.hasQuery && !this.pendingQuery, querySettling = this.hasQuery && !this.pendingQuery, tuning = this.tuning || MAP_TUNING_DEFAULTS, anchorStrength = vaultMode ? tuning.anchorStrength : 1; for (const node of activeNodes) { if (node === this.dragging) continue; const background = preserveBackground && this.hasQuery && !node.matched, captureBoost = node.matched ? Number(node.capture || 0) * .032 : 0, strength = background ? .0035 * alpha : .018 * anchorStrength * (querySettling ? Math.max(alpha, .28) : alpha) + captureBoost; node.vx += (node.layoutX - node.x) * strength; node.vy += (node.layoutY - node.y) * strength; } if (this.queryPresence > .01 && query !== this.dragging) { const strength = .012 * (querySettling ? Math.max(alpha, .28) : alpha); query.vx += (query.layoutX - query.x) * strength; query.vy += (query.layoutY - query.y) * strength; }
    this.applyCollisionForces(this.queryPresence > .04 ? [query, ...activeNodes] : activeNodes, alpha); const relaxationNodes = this.hasQuery ? foreground : Number(tuning.repulsion || 0) > 0 ? activeNodes : []; for (let first = 0; first < relaxationNodes.length; first++) for (let second = first + 1; second < relaxationNodes.length; second++) { const a = relaxationNodes[first], b = relaxationNodes[second]; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const force = this.hasQuery ? -Math.min(.0012, .000008 / (distance * distance)) * alpha : -Math.min(.0009, .000006 / (distance * distance)) * Number(tuning.repulsion || 0) * alpha; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
