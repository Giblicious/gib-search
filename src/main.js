...
function buildSmoothElevation(points, size = 190) {
  if (points.length < 2) return null; const influence = point => Number(point.influence || .19), extent = Math.min(1.12, Math.max(.38, ...points.map(point => Math.max(Math.abs(point.x), Math.abs(point.y)) + influence(point) + .045))), field = new Float32Array(size * size); for (let row = 0; row < size; row++) for (let column = 0; column < size; column++) { const x = (column / (size - 1) * 2 - 1) * extent, y = (row / (size - 1) * 2 - 1) * extent; let weightedElevation = 0, weightTotal = 0, supportTotal = 0; for (const point of points) { const radius = influence(point), distanceSquared = (x - point.x) ** 2 + (y - point.y) ** 2; if (distanceSquared >= radius ** 2) continue; const distance = Math.sqrt(distanceSquared), ratio = distance / radius, support = (1 - ratio) ** 4 * (4 * ratio + 1), weight = support / (distanceSquared + .00018); supportTotal += support; weightTotal += weight; weightedElevation += Number(point.z || 0) * weight; } const support = Math.min(1, supportTotal * 1.28); field[row * size + column] = weightTotal ? Math.max(0, Math.min(1, weightedElevation / weightTotal * support)) : 0; }
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  elevationSamples() { const query = this.queryNode, byId = new Map(this.nodes.map(node => [node.id, node])), points = [{ x: query.x, y: query.y, z: 1, influence: .24 }, ...this.nodes.map(node => ({ x: node.x, y: node.y, z: node.elevation, influence: .145 + node.relevance * .035 }))], queryRidges = [...this.nodes].sort((first, second) => second.relevance - first.relevance).slice(0, 12); for (const node of queryRidges) points.push({ x: (query.x + node.x) / 2, y: (query.y + node.y) / 2, z: (1 + node.elevation) / 2, influence: .11 + node.relevance * .045 }); const ridges = [...(this.activeEdges || [])].filter(edge => edge.residual > 0).sort((first, second) => second.strength - first.strength).slice(0, Math.min(30, this.nodes.length * 2)); for (const edge of ridges) { const source = byId.get(edge.source), target = byId.get(edge.target); if (!source || !target) continue; points.push({ x: (source.x + target.x) / 2, y: (source.y + target.y) / 2, z: (source.elevation + target.elevation) / 2, influence: .12 + edge.strength * .05 }); } return points; }
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = time => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .985; if (!this.terrain || time - this.lastTerrainAt > 88) { this.terrain = buildSmoothElevation(this.elevationSamples(), this.alpha < .08 ? 200 : 125); this.lastTerrainAt = time; } this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); else { this.terrain = buildSmoothElevation(this.elevationSamples(), 220); this.draw(); } }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  drawContours(ctx, width, height, colors) {
    if (!this.terrain) return; const { size, field, extent } = this.terrain, scale = Math.max(20, Math.min(width, height) / 2 - 68), toCanvas = (column, row) => [width / 2 + ((column / (size - 1) * 2 - 1) * extent) * scale, height / 2 + ((row / (size - 1) * 2 - 1) * extent) * scale]; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    const levels = [.025, 1 / 6, 2 / 6, 3 / 6, 4 / 6, 5 / 6]; for (let levelIndex = 0; levelIndex < levels.length; levelIndex++) { const level = levels[levelIndex], outer = levelIndex === 0; ctx.beginPath(); for (let row = 0; row < size - 1; row++) for (let column = 0; column < size - 1; column++) { const corners = [field[row * size + column], field[row * size + column + 1], field[(row + 1) * size + column + 1], field[(row + 1) * size + column]], crossings = []; const cross = (first, second, x1, y1, x2, y2) => { if ((first < level) === (second < level) || first === second) return; const amount = (level - first) / (second - first); crossings.push(toCanvas(x1 + (x2 - x1) * amount, y1 + (y2 - y1) * amount)); }; cross(corners[0], corners[1], column, row, column + 1, row); cross(corners[1], corners[2], column + 1, row, column + 1, row + 1); cross(corners[2], corners[3], column + 1, row + 1, column, row + 1); cross(corners[3], corners[0], column, row + 1, column, row); if (crossings.length === 2) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); } else if (crossings.length === 4) { ctx.moveTo(crossings[0][0], crossings[0][1]); ctx.lineTo(crossings[1][0], crossings[1][1]); ctx.moveTo(crossings[2][0], crossings[2][1]); ctx.lineTo(crossings[3][0], crossings[3][1]); } } ctx.globalAlpha = outer ? .18 : .31; ctx.strokeStyle = outer ? colors.muted : colors.normal; ctx.lineWidth = outer ? .72 : 1.02; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
