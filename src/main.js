...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  beginRoadRouteRefresh(field, width, height) {
    this.cancelRoadWork(); const token = this.roadWorkToken, queue = this.roadEdges.slice(0, 72), nextRoutes = new Map(), work = deadline => { if (token !== this.roadWorkToken) return; this.roadWorkHandle = null; let processed = 0; while (queue.length && processed < 1 && (processed === 0 || deadline.timeRemaining() > 3)) { const road = queue.shift(), key = road.key || mapEdgeKey(road.source, road.target), source = this.roadEndpoint(road.source, width, height), target = this.roadEndpoint(road.target, width, height); if (source && target) { const points = this.routeRoad(field, source, target), cumulative = [0]; for (let index = 1; index < points.length; index++) cumulative.push(cumulative[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); nextRoutes.set(key, { road, points, cumulative, total: cumulative[cumulative.length - 1] || 1, source, target }); if (!this.roadIntroducedAt.has(key)) this.roadIntroducedAt.set(key, performance.now()); } processed++; } this.roadRoutes = new Map([...this.roadRoutes, ...nextRoutes]); this.draw(); if (queue.length) this.scheduleRoadWork(work); else this.lastRoadAt = performance.now(); }; this.roadsDirty = false; this.scheduleRoadWork(work);
  }
  ...
  paintRoads(ctx, width, height, colors) {
    if (!this.roadEdges.length || !this.lastTerrainField) return; const now = performance.now(); if (this.roadsDirty && this.roadWorkHandle === null) this.beginRoadRouteRefresh(this.lastTerrainField, width, height); const focused = this.hovered || this.selected, reduced = matchMedia('(prefers-reduced-motion: reduce)').matches; let animating = false;
    for (const [key, route] of this.roadRoutes) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const opacity = Math.min(source.visibility, target.visibility); if (opacity < .035) continue; const points = route.conformedAt === this.lastTerrainAt ? this.adjustedRoadPoints(route, source, target) : this.conformRoadRoute(route, this.lastTerrainField, source, target), introduced = this.roadIntroducedAt.get(key) || now, progress = reduced ? 1 : Math.max(0, Math.min(1, (now - introduced) / 430)); if (progress < 1) animating = true; const active = focused === route.road.source || focused === route.road.target; ctx.globalAlpha = opacity * (active ? .78 : .28); ctx.strokeStyle = active ? colors.normal : colors.muted; ctx.lineWidth = active ? 1.45 : .85; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; if (this.traceRoad(ctx, points, progress)) ctx.stroke(); const chevron = (fraction, reverse = false) => { if (progress < fraction + .04) return; const point = this.roadPointAt(points, fraction), angle = point.angle + (reverse ? Math.PI : 0), size = active ? 3.8 : 3.1; ctx.beginPath(); ctx.moveTo(point.x - Math.cos(angle - .62) * size, point.y - Math.sin(angle - .62) * size); ctx.lineTo(point.x, point.y); ctx.lineTo(point.x - Math.cos(angle + .62) * size, point.y - Math.sin(angle + .62) * size); ctx.stroke(); }; if (route.road.forward) chevron(.62); if (route.road.reverse) chevron(.38, true); }
