...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.results = []; this.resultsCollapsed = true; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.mapGroupingMode = validMapGrouping(plugin.settings.mapGroupingMode); this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1));
  }
  ...
  async runSearch(query, immediate = false) {
    ...
    try {
      ...
      this.results = results; this.renderResults(); this.map.setTitle(`${results.length} results · ${this.mapGroupingMode === 'topics' ? 'Topics' : 'Similarity'}`); this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, merged, model.edges, model.roads);
