...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  topicCommunityGroups(width, height) { if (this.mapGroupingMode !== 'topics') return []; const tuning = this.tuning || MAP_TUNING_DEFAULTS, threshold = this.hasQuery ? .01 : Number(tuning.communityMembership || .42), minimum = Math.max(2, Number(tuning.communityMinSize || 3)), visible = this.nodes.filter(node => node.visibility > .08 && (!this.hasQuery || this.pendingQuery || node.matched) && Number(node.communityMembership || 0) >= threshold), groups = new Map(); for (const node of visible) { const values = groups.get(node.community) || []; const [x, y] = this.coordinates(node, width, height); values.push({ node, x, y }); groups.set(node.community, values); } return [...groups].filter(([, values]) => values.length >= minimum).map(([id, values]) => ({ id, values, label: values.find(value => value.node.communityLabel)?.node.communityLabel || '', confidence: Math.max(...values.map(value => Number(value.node.communityLabelConfidence || 0))) })); }
  communityHull(points, padding) { const samples = points.flatMap(point => Array.from({ length: 12 }, (_, index) => { const angle = index / 12 * Math.PI * 2; return { x: point.x + Math.cos(angle) * padding, y: point.y + Math.sin(angle) * padding }; })).sort((a, b) => a.x - b.x || a.y - b.y), cross = (origin, a, b) => (a.x - origin.x) * (b.y - origin.y) - (a.y - origin.y) * (b.x - origin.x), half = values => { const hull = []; for (const point of values) { while (hull.length >= 2 && cross(hull[hull.length - 2], hull[hull.length - 1], point) <= 0) hull.pop(); hull.push(point); } return hull; }, lower = half(samples), upper = half(samples.slice().reverse()); lower.pop(); upper.pop(); return [...lower, ...upper]; }
  paintSmoothCommunityHull(ctx, hull) { if (hull.length < 3) return; const midpoint = (a, b) => ({ x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 }), start = midpoint(hull[hull.length - 1], hull[0]); ctx.beginPath(); ctx.moveTo(start.x, start.y); for (let index = 0; index < hull.length; index++) { const point = hull[index], next = hull[(index + 1) % hull.length], end = midpoint(point, next); ctx.bezierCurveTo(point.x, point.y, point.x, point.y, end.x, end.y); } ctx.closePath(); }
  paintTopicCommunities(ctx, width, height, colors) { const groups = this.topicCommunityGroups(width, height), tuning = this.tuning || MAP_TUNING_DEFAULTS, padding = 20 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))); this.communityLabelsForDraw = []; for (const group of groups) { const hull = this.communityHull(group.values, padding), hue = group.values.find(value => Number.isFinite(value.node.topicHue))?.node.topicHue, color = this.options.semanticColors !== false && Number.isFinite(hue) ? `hsl(${hue} 48% 62%)` : colors.muted; this.paintSmoothCommunityHull(ctx, hull); ctx.strokeStyle = color; ctx.globalAlpha = .3; ctx.lineWidth = 1.05; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.stroke(); const weight = group.values.reduce((sum, value) => sum + Math.max(.01, Number(value.node.communityMembership || 0)), 0), x = group.values.reduce((sum, value) => sum + value.x * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight, y = group.values.reduce((sum, value) => sum + value.y * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight; if (group.label) this.communityLabelsForDraw.push({ x, y, text: group.label, color, confidence: group.confidence }); } ctx.globalAlpha = 1; }
  ...
  draw() {
    ...
    const labels = [], ordered = this.hasQuery && !this.pendingQuery ? [...this.nodes.filter(node => !node.matched), ...this.nodes.filter(node => node.matched)] : this.nodes; for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.01, node.visibility * focusAlpha), blur = Math.max(0, Math.min(1, Number(node.blur || 0))), semanticColor = this.options.semanticColors !== false && Number.isFinite(node.topicHue) ? `hsl(${node.topicHue} 54% 64%)` : colors.normal; ctx.fillStyle = node.matched || !this.hasQuery || this.pendingQuery ? semanticColor : colors.faint; if (blur > .04) { ctx.globalAlpha = opacity * blur * .22; ctx.beginPath(); ctx.arc(x, y, radius + 2.2 * blur, 0, Math.PI * 2); ctx.fill(); } ctx.globalAlpha = opacity * (1 - blur * .28); ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery || this.options.preserveBackground) this.hit.push({ node, x, y, radius: radius + 9 }); }
