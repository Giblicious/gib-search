...
const { AtlasEngine, ATLAS_LENSES, DEFAULT_ATLAS_VIEW, validAtlasLens, atlasLens } = require('./atlas-engine');
...
const MAP_TUNING_CONTROLS = [
  ...
  { key: 'boundaryRepulsion', label: 'Outsider repulsion', min: 0, max: 2, step: .05 },
  { section: 'Physics', key: 'neighborAttraction', label: 'Neighbor attraction', min: 0, max: 2, step: .05 },
  { key: 'collisionSpacing', label: 'Collision spacing', min: .3, max: 2, step: .05 },
  { key: 'repulsion', label: 'Background repulsion', min: 0, max: 2, step: .05 },
  { key: 'anchorStrength', label: 'Layout anchor', min: .25, max: 2, step: .05 },
  { section: 'Terrain', key: 'showTopography', label: 'Show topography', type: 'toggle' },
  { key: 'terrainSpread', label: 'Terrain spread', min: .5, max: 2, step: .05 },
  { key: 'terrainContrast', label: 'Height contrast', min: .5, max: 2, step: .05 },
];
const SEARCH_LENSES = ATLAS_LENSES;
function validSearchLens(value) { return value === 'concepts' ? 'relevance' : validAtlasLens(value); }
...
async function buildQueryMapModel(plugin, query, results, options = {}) {
  const lens = validSearchLens(options.lens), definition = atlasLens(lens), view = options.view || plugin.atlas.activeView(options.viewId), state = plugin.atlas.state({ view, anchor: { type: 'query', value: String(query || '').trim() }, lens, scale: options.scale });
  const scene = await plugin.atlas.scene(state, results, { ...options, mapMode: definition.mapMode });
...
class SemanticMapCanvas {
  constructor(host, app, options = {}) {
    ...
    this.stage = host.createDiv({ cls: 'gib-search-map-stage' }); this.webglCanvas = this.stage.createEl('canvas', { cls: 'gib-search-map-gl' }); this.canvas = this.stage.createEl('canvas', { cls: 'gib-search-map-canvas' });
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  constructor(host, app, options = {}) { super(host, app, options); cancelAnimationFrame(this.animationFrame); this.animationFrame = null; this.simulationFrame = null; this.alpha = 0; this.queryPresence = 0; this.targetQueryPresence = 0; this.queryMarkerFocus = 0; this.gaussianLookup = Float32Array.from({ length: 1025 }, (_, index) => Math.exp(-index / 1024 * 9)); this.roadEdges = []; this.roadIntroducedAt = new Map(); this.roadAnimationFrame = null; this.showLinks = options.showLinks !== false; this.manualLinkInfluence = options.manualLinkInfluence !== false; this.tuning = Object.assign({}, MAP_TUNING_DEFAULTS, options.tuning || {}); this.mapGroupingMode = validMapGrouping(options.mapGroupingMode); this.cameraX = 0; this.cameraY = 0; this.cameraZoom = 1; this.userZoom = 1; this.panX = 0; this.panY = 0; this.viewportInsets = { left: 0, right: 0, top: 0, bottom: 0 }; this.targetViewportInsets = { ...this.viewportInsets }; this.pointers = new Map(); this.pendingQuery = false; this.ambientStartedAt = performance.now(); this.ambientDrawing = false; this.linkIdleAnchored = false; this.visibilityHandler = () => { if (!document.hidden) this.startSimulation(0); }; document.addEventListener('visibilitychange', this.visibilityHandler); if (options.onGenerations) { this.headingEl.addClass('has-generations'); this.generationControl = this.headingEl.createDiv({ cls: 'gib-search-map-generations', attr: { 'aria-label': 'Semantic generations' } }); this.headingEl.insertBefore(this.generationControl, this.statusEl); this.generationControl.createSpan({ cls: 'gib-search-map-generations-label', text: 'Gen' }); this.generationButtons = [1, 2, 3].map(value => { const button = this.generationControl.createEl('button', { text: String(value), attr: { type: 'button', title: value === 1 ? 'Query results only' : `${value} semantic generations`, 'aria-label': value === 1 ? 'Show query results only' : `Show ${value} semantic generations` } }); button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.setGenerations(value); options.onGenerations(value); }); return button; }); this.setGenerations(options.generations || 1); } if (options.onShowLinks) { this.headingEl.addClass('has-link-lines'); this.linkLinesButton = this.headingEl.createEl('button', { cls: 'gib-search-map-link-lines', attr: { type: 'button', 'aria-label': 'Manual link lines' } }); this.headingEl.insertBefore(this.linkLinesButton, this.statusEl); const icon = this.linkLinesButton.createSpan({ cls: 'gib-search-map-link-lines-icon' }); setIcon(icon, 'git-branch'); this.linkLinesButton.createSpan({ text: 'Lines' }); this.linkLinesButton.addEventListener('mousedown', event => event.preventDefault()); this.linkLinesButton.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.setShowLinks(!this.showLinks); options.onShowLinks(this.showLinks); }); this.setShowLinks(this.showLinks, false); } if (options.onManualLinkInfluence) { this.headingEl.addClass('has-link-influence'); this.linkInfluenceButton = this.headingEl.createEl('button', { cls: 'gib-search-map-link-influence', attr: { type: 'button', 'aria-label': 'Manual link influence' } }); this.headingEl.insertBefore(this.linkInfluenceButton, this.statusEl); const icon = this.linkInfluenceButton.createSpan({ cls: 'gib-search-map-link-influence-icon' }); setIcon(icon, 'magnet'); this.linkInfluenceButton.createSpan({ text: 'Pull' }); this.linkInfluenceButton.addEventListener('mousedown', event => event.preventDefault()); this.linkInfluenceButton.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.setManualLinkInfluence(!this.manualLinkInfluence); options.onManualLinkInfluence(this.manualLinkInfluence); }); this.setManualLinkInfluence(this.manualLinkInfluence, false); } if (options.onMapMode) this.setupMapModeControl(); if (options.onTune) this.setupTuningPanel(); if (!options.minimalChrome) this.setupViewportControls(); this.canvas.addEventListener('wheel', event => this.wheel(event), { passive: false }); }
  setupMapModeControl() { this.mapModeControl = this.headingEl.createDiv({ cls: 'gib-search-map-mode', attr: { 'aria-label': 'Map grouping mode' } }); this.headingEl.insertBefore(this.mapModeControl, this.statusEl); const modes = [['similarity', 'Similarity', 'Arrange notes by continuous semantic similarity'], ['topics', 'Topics', 'Group notes by query-relative topics'], ['links', 'Links', 'Let manual links drive the layout, supported by meaning']]; this.mapModeButtons = modes.map(([value, label, title]) => { const button = this.mapModeControl.createEl('button', { text: label, attr: { type: 'button', title } }); button.addEventListener('mousedown', event => event.preventDefault()); button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.setMapGroupingMode(value); this.options.onMapMode(value); }); return button; }); this.setMapGroupingMode(this.mapGroupingMode); }
  setMapGroupingMode(value) { this.mapGroupingMode = validMapGrouping(value); this.lastCommunityAt = 0; this.mapModeButtons?.forEach((button, index) => { const active = ['similarity', 'topics', 'links'][index] === this.mapGroupingMode; button.toggleClass('is-active', active); button.setAttribute('aria-pressed', String(active)); }); if (this.linkInfluenceButton) { const integrated = this.mapGroupingMode === 'links', active = this.manualLinkInfluence && !integrated; this.linkInfluenceButton.disabled = integrated; this.linkInfluenceButton.toggleClass('is-active', active); this.linkInfluenceButton.setAttribute('aria-pressed', String(active)); this.linkInfluenceButton.setAttribute('title', integrated ? 'Links mode already uses manual links for placement' : this.manualLinkInfluence ? 'Manual links gently influence placement' : 'Manual links do not influence placement'); } }
  setupTuningPanel() {
    this.tuneButton = this.headingEl.createEl('button', { cls: 'gib-search-map-tune', attr: { type: 'button', title: 'Tune vault map', 'aria-label': 'Tune vault map', 'aria-expanded': 'false' } }); this.headingEl.insertBefore(this.tuneButton, this.statusEl); setIcon(this.tuneButton, 'sliders-horizontal');
    this.tuningPanel = this.stage.createDiv({ cls: 'gib-search-map-tuning' }); this.tuningOpen = false; this.tuningPanel.hide(); this.tuningInputs = new Map();
    this.tuningPanel.createDiv({ cls: 'gib-search-map-tuning-title', text: 'Vault map tuning' });
    for (const control of MAP_TUNING_CONTROLS) { if (control.section) this.tuningPanel.createDiv({ cls: 'gib-search-map-tuning-section', text: control.section }); const row = this.tuningPanel.createDiv({ cls: `gib-search-map-tuning-row${control.type === 'toggle' ? ' is-toggle' : ''}` }), label = row.createEl('label', { text: control.label }), input = row.createEl('input', { attr: { type: control.type === 'toggle' ? 'checkbox' : 'range' } }), output = row.createEl('output'); label.htmlFor = `gib-map-${control.key}`; input.id = `gib-map-${control.key}`; if (control.type === 'toggle') { input.checked = Boolean(this.tuning[control.key]); output.textContent = input.checked ? 'On' : 'Off'; } else { input.min = String(control.min); input.max = String(control.max); input.step = String(control.step); input.value = String(this.tuning[control.key]); output.textContent = Number(this.tuning[control.key]).toFixed(control.step < .05 ? 2 : control.step >= 1 ? 0 : 1); } const update = notify => { this.tuning[control.key] = control.type === 'toggle' ? input.checked : Number(input.value); output.textContent = control.type === 'toggle' ? input.checked ? 'On' : 'Off' : Number(input.value).toFixed(control.step < .05 ? 2 : control.step >= 1 ? 0 : 1); this.lastTerrainAt = 0; this.lastCommunityAt = 0; this.startSimulation(.58); if (notify) this.options.onTune({ ...this.tuning }, control.key); else this.draw(); }; if (control.type === 'toggle') input.addEventListener('change', () => update(true)); else { input.addEventListener('input', () => update(!mapTuningNeedsLayout(control.key))); if (mapTuningNeedsLayout(control.key)) input.addEventListener('change', () => update(true)); } this.tuningInputs.set(control.key, { input, output, control }); }
    const footer = this.tuningPanel.createDiv({ cls: 'gib-search-map-tuning-footer' }), reset = footer.createEl('button', { text: 'Reset', attr: { type: 'button' } }); reset.addEventListener('click', () => this.setTuning(MAP_TUNING_DEFAULTS, true, 'reset'));
    for (const event of ['pointerdown', 'pointermove', 'pointerup', 'wheel', 'click']) this.tuningPanel.addEventListener(event, value => value.stopPropagation()); this.tuneButton.addEventListener('mousedown', event => event.preventDefault()); this.tuneButton.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.tuningOpen = !this.tuningOpen; this.tuningPanel.toggle(this.tuningOpen); this.tuneButton.toggleClass('is-active', this.tuningOpen); this.tuneButton.setAttribute('aria-expanded', String(this.tuningOpen)); });
  }
  setTuning(values, notify = false, key = 'reset') { this.tuning = Object.assign({}, MAP_TUNING_DEFAULTS, values || {}); for (const [name, view] of this.tuningInputs || []) { if (view.control.type === 'toggle') { view.input.checked = Boolean(this.tuning[name]); view.output.textContent = view.input.checked ? 'On' : 'Off'; } else { view.input.value = String(this.tuning[name]); view.output.textContent = Number(this.tuning[name]).toFixed(view.control.step < .05 ? 2 : view.control.step >= 1 ? 0 : 1); } } this.lastTerrainAt = 0; this.lastCommunityAt = 0; this.startSimulation(.72); if (notify) this.options.onTune?.({ ...this.tuning }, key); }
  ...
  setManualLinkInfluence(value, animate = true) { this.manualLinkInfluence = Boolean(value); this.linkInfluenceButton?.toggleClass('is-active', this.manualLinkInfluence && this.mapGroupingMode !== 'links'); this.linkInfluenceButton?.setAttribute('aria-pressed', String(this.manualLinkInfluence && this.mapGroupingMode !== 'links')); this.linkInfluenceButton?.setAttribute('title', this.mapGroupingMode === 'links' ? 'Links mode already uses manual links for placement' : this.manualLinkInfluence ? 'Manual links gently influence placement' : 'Manual links do not influence placement'); if (animate && this.mapGroupingMode !== 'links') this.startSimulation(.72); }
  ...
  collisionDistance(a, b, queryMode = this.hasQuery) { if (a?.isQuery || b?.isQuery) return .052 + Math.max(0, Math.min(1, Number((a?.isQuery ? b : a)?.fileScale ?? .35))) * .014; return (queryMode ? .035 + (Number(a.fileScale || 0) + Number(b.fileScale || 0)) * .022 : .016 + (Number(a.fileScale || 0) + Number(b.fileScale || 0)) * .012) * (queryMode ? 1 : Number(this.tuning?.collisionSpacing || 1)); }
  ...
  prepareLinkComponents() { const parent = new Map(this.nodes.map(node => [node.id, node.id])), find = id => { let root = id; while (parent.get(root) !== root) root = parent.get(root); while (parent.get(id) !== id) { const next = parent.get(id); parent.set(id, root); id = next; } return root; }, join = (first, second) => { const a = find(first), b = find(second); if (a !== b) parent.set(b, a); }; for (const edge of this.roadEdges) if (parent.has(edge.source) && parent.has(edge.target)) join(edge.source, edge.target); const groups = new Map(); for (const node of this.nodes) { const root = find(node.id), group = groups.get(root) || []; group.push(node); groups.set(root, group); } this.linkComponentById = new Map(); this.linkComponentAnchors = new Map(); for (const [root, members] of groups) { const anchor = { x: members.reduce((sum, node) => sum + node.layoutX, 0) / members.length, y: members.reduce((sum, node) => sum + node.layoutY, 0) / members.length }; this.linkComponentAnchors.set(root, anchor); for (const node of members) this.linkComponentById.set(node.id, root); } }
  applyLinkGraphForces(nodes, alpha) { if (nodes.length < 2) return; const temperature = Math.max(.22, alpha), active = new Set(nodes.map(node => node.id)), degrees = new Map(), links = []; for (const edge of this.roadEdges) { if (!active.has(edge.source) || !active.has(edge.target)) continue; degrees.set(edge.source, (degrees.get(edge.source) || 0) + 1); degrees.set(edge.target, (degrees.get(edge.target) || 0) + 1); links.push(edge); } for (const edge of links) { const source = this.byId.get(edge.source), target = this.byId.get(edge.target); if (!source || !target) continue; let dx = target.x - source.x, dy = target.y - source.y, distance = Math.hypot(dx, dy); if (distance < .0001) { const angle = stableMapAngle(edge.key || mapEdgeKey(edge.source, edge.target)); dx = Math.cos(angle); dy = Math.sin(angle); distance = .0001; } else { dx /= distance; dy /= distance; } const desired = .2 + (source.fileScale + target.fileScale) * .03, degreeScale = 1 / Math.sqrt(Math.max(1, Math.min(degrees.get(edge.source) || 1, degrees.get(edge.target) || 1))), force = Math.max(-.012, Math.min(.012, (distance - desired) * .035)) * degreeScale * temperature; if (source !== this.dragging) { source.vx += dx * force; source.vy += dy * force; } if (target !== this.dragging) { target.vx -= dx * force; target.vy -= dy * force; } } for (let first = 0; first < nodes.length; first++) for (let second = first + 1; second < nodes.length; second++) { const a = nodes[first], b = nodes[second]; let dx = b.x - a.x, dy = b.y - a.y, distanceSquared = dx * dx + dy * dy, distance = Math.sqrt(distanceSquared); if (distance < .0001) { const angle = stableMapAngle(`${a.id}\0${b.id}`); dx = Math.cos(angle); dy = Math.sin(angle); distance = .0001; distanceSquared = .0001; } else { dx /= distance; dy /= distance; } const force = Math.min(.0032, .000032 / (distanceSquared + .0035)) * temperature; if (a !== this.dragging) { a.vx -= dx * force; a.vy -= dy * force; } if (b !== this.dragging) { b.vx += dx * force; b.vy += dy * force; } } const components = new Map(); for (const node of nodes) { const root = this.linkComponentById?.get(node.id) ?? node.id, group = components.get(root) || []; group.push(node); components.set(root, group); } for (const [root, members] of components) { const anchor = this.linkComponentAnchors?.get(root); if (!anchor) continue; const centerX = members.reduce((sum, node) => sum + node.x, 0) / members.length, centerY = members.reduce((sum, node) => sum + node.y, 0) / members.length, forceX = (anchor.x - centerX) * .0012 * temperature, forceY = (anchor.y - centerY) * .0012 * temperature; for (const node of members) if (node !== this.dragging) { node.vx += forceX; node.vy += forceY; } } const centerX = nodes.reduce((sum, node) => sum + node.x, 0) / nodes.length, centerY = nodes.reduce((sum, node) => sum + node.y, 0) / nodes.length; for (const node of nodes) if (node !== this.dragging) { node.vx -= centerX * .0015 * temperature; node.vy -= centerY * .0015 * temperature; } }
  ...
  setGraph(center, values, edges = [], roads = []) {
    const transition = center?.transition || 'full', smoothQueryTransition = Boolean(center?.hasQuery && this.hasQuery && (transition === 'provisional' || transition === 'refine')); if (smoothQueryTransition) { const now = performance.now(); this.lastTerrainAt = Math.max(Number(this.lastTerrainAt || 0), now - 34); this.lastCommunityAt = Math.max(Number(this.lastCommunityAt || 0), now - 34); } else { this.lastTerrainAt = 0; this.lastCommunityAt = 0; }
  ...
  physicsStep(alpha) {
    ...
    const query = this.queryNode, foreground = this.nodes.filter(node => node.matched), preserveBackground = Boolean(this.options.preserveBackground), activeNodes = preserveBackground ? this.nodes : this.hasQuery && !this.pendingQuery ? foreground : this.nodes, vaultMode = !this.hasQuery && !this.pendingQuery, querySettling = this.hasQuery && !this.pendingQuery, linkMode = this.mapGroupingMode === 'links', linkNodes = querySettling ? foreground : activeNodes, linkNodeIds = linkMode ? new Set(linkNodes.map(node => node.id)) : null, tuning = this.tuning || MAP_TUNING_DEFAULTS, anchorStrength = vaultMode ? tuning.anchorStrength : 1; for (const node of activeNodes) { if (node === this.dragging) continue; const background = preserveBackground && this.hasQuery && !node.matched, captureBoost = node.matched ? Number(node.capture || 0) * .032 : 0, strength = background ? .0035 * alpha : linkNodeIds?.has(node.id) ? 0 : .018 * anchorStrength * (querySettling ? Math.max(alpha, .28) : alpha) + captureBoost; node.vx += (node.layoutX - node.x) * strength; node.vy += (node.layoutY - node.y) * strength; } if (this.queryPresence > .01 && query !== this.dragging) { const strength = .012 * (querySettling ? Math.max(alpha, .28) : alpha); query.vx += (query.layoutX - query.x) * strength; query.vy += (query.layoutY - query.y) * strength; }
    const collisionBodies = preserveBackground && this.hasQuery ? (this.queryPresence > .04 ? [query, ...foreground] : foreground) : this.queryPresence > .04 ? [query, ...activeNodes] : activeNodes; this.applyCollisionForces(collisionBodies, alpha); if (preserveBackground && this.hasQuery && (this.backgroundCollisionFrame = (Number(this.backgroundCollisionFrame || 0) + 1) % 6) === 0) this.applyCollisionForces(this.nodes.filter(node => !node.matched), alpha * .2); if (linkMode) this.applyLinkGraphForces(linkNodes, alpha); const relaxationNodes = linkMode ? [] : this.hasQuery ? foreground : Number(tuning.repulsion || 0) > 0 ? activeNodes : []; for (let first = 0; first < relaxationNodes.length; first++) for (let second = first + 1; second < relaxationNodes.length; second++) { const a = relaxationNodes[first], b = relaxationNodes[second]; let dx = b.x - a.x, dy = b.y - a.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const force = this.hasQuery ? -Math.min(.0012, .000008 / (distance * distance)) * alpha : -Math.min(.0009, .000006 / (distance * distance)) * Number(tuning.repulsion || 0) * alpha; if (a !== this.dragging) { a.vx += dx * force; a.vy += dy * force; } if (b !== this.dragging) { b.vx -= dx * force; b.vy -= dy * force; } }
    ...
    if (vaultMode && !linkMode && Number(tuning.neighborAttraction) > 0) for (const edge of this.activeEdges || []) { if (edge.bridge || edge.entityBridge || Number(edge.overall || 0) < .12) continue; const source = this.byId.get(edge.source), target = this.byId.get(edge.target); if (!source || !target) continue; let dx = target.x - source.x, dy = target.y - source.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const desired = .035 + (1 - Number(edge.overall || 0)) * .15, force = Math.tanh((distance - desired) * 4) * .0028 * Number(tuning.neighborAttraction) * Number(edge.overall || 0) * alpha; if (source !== this.dragging) { source.vx += dx * force; source.vy += dy * force; } if (target !== this.dragging) { target.vx -= dx * force; target.vy -= dy * force; } }
    ...
    if (this.mapGroupingMode === 'topics' && tuning.boundaryRepulsionEnabled && Number(tuning.boundaryRepulsion) > 0) { const topicNodes = querySettling ? foreground : activeNodes, threshold = Number(tuning.communityMembership || .42), minimum = Math.max(2, Number(tuning.communityMinSize || 3)), groups = new Map(), sigma = .11 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))), divisor = 2 * sigma * sigma; for (const node of topicNodes) if (Number(node.communityMembership || 0) >= threshold) { const values = groups.get(node.community) || []; values.push(node); groups.set(node.community, values); } for (const [community, members] of groups) { if (members.length < minimum) continue; for (const node of topicNodes) { if (node.community === community || node === this.dragging) continue; let density = 0, gradientX = 0, gradientY = 0; for (const member of members) { const dx = node.x - member.x, dy = node.y - member.y, weight = Math.exp(-(dx * dx + dy * dy) / divisor); density += weight; gradientX += dx * weight; gradientY += dy * weight; } if (density < .34) continue; const length = Math.max(.008, Math.hypot(gradientX, gradientY)), force = Math.min(.012, (density - .34) * .0028) * Number(tuning.boundaryRepulsion) * alpha; node.vx += gradientX / length * force; node.vy += gradientY / length * force; } } }
    if (this.manualLinkInfluence && this.mapGroupingMode !== 'links' && this.roadEdges.length) { const isActive = body => body === query ? this.queryPresence > .04 : !this.hasQuery || this.pendingQuery || body.matched; for (const edge of this.roadEdges) { const source = edge.source === this.center?.id ? query : this.byId.get(edge.source), target = edge.target === this.center?.id ? query : this.byId.get(edge.target); if (!source || !target || !isActive(source) || !isActive(target)) continue; let dx = target.x - source.x, dy = target.y - source.y; const distance = Math.max(.018, Math.hypot(dx, dy)); dx /= distance; dy /= distance; const hubScale = 1 / Math.sqrt(Math.max(1, Number(this.roadDegrees?.get(edge.source) || 1), Number(this.roadDegrees?.get(edge.target) || 1))), desired = .2, force = Math.tanh((distance - desired) * 3.2) * .0036 * hubScale * alpha; if (source !== this.dragging) { source.vx += dx * force; source.vy += dy * force; } if (target !== this.dragging) { target.vx -= dx * force; target.vy -= dy * force; } } }
    if (!linkMode) { const corralBodies = this.queryPresence > .04 ? [query, ...activeNodes] : activeNodes; for (const body of corralBodies) { if (body === this.dragging) continue; const distance = Math.hypot(body.x, body.y); if (distance <= .92) continue; body.vx -= body.x / distance * (distance - .92) * .045 * alpha; body.vy -= body.y / distance * (distance - .92) * .045 * alpha; } }
    ...
    const keepQueryFraming = this.hasQuery && (!this.pendingQuery || this.pendingQueryHasResults), cameraBodies = this.hasQuery ? (keepQueryFraming ? [query, ...foreground] : this.nodes) : this.nodes, followGraph = linkMode || this.hasQuery, targetX = followGraph && cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.x, 0) / cameraBodies.length : 0, targetY = followGraph && cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.y, 0) / cameraBodies.length : 0, extent = followGraph && cameraBodies.length ? Math.max(.16, ...cameraBodies.map(body => Math.hypot(body.x - targetX, body.y - targetY))) : .8, targetZoom = linkMode ? Math.max(.48, Math.min(1.45, .72 / extent)) : keepQueryFraming ? Math.max(1.12, Math.min(2.15, .7 / extent)) : 1; this.cameraX += (targetX - this.cameraX) * .055; this.cameraY += (targetY - this.cameraY) * .055; this.cameraZoom += (targetZoom - this.cameraZoom) * .05;
  }
  idlePhysicsStep(now) {
    const queryActive = this.hasQuery && !this.pendingQuery, activeNodes = queryActive ? this.nodes.filter(node => node.matched) : this.nodes, origin = this.queryPresence > .04 ? this.queryNode : { x: 0, y: 0 };
    if (this.mapGroupingMode === 'links' && !this.linkIdleAnchored) { for (const node of activeNodes) { node.layoutX = node.x; node.layoutY = node.y; } this.linkIdleAnchored = true; }
    for (const node of activeNodes) {
      if (node === this.dragging) continue;
      const phase = stableMapAngle(node.id), speed = .00012 + (phase / (Math.PI * 2)) * .000055, orbit = .0022 + Math.max(0, Math.min(1, Number(node.fileScale || .35))) * .0024, angle = now * speed + phase, targetX = node.layoutX + Math.cos(angle) * orbit, targetY = node.layoutY + Math.sin(angle * .83 + phase) * orbit;
      node.vx += (targetX - node.x) * .0065; node.vy += (targetY - node.y) * .0065; node.vx *= .91; node.vy *= .91; node.x += node.vx; node.y += node.vy;
    }
    ...
    const cameraBodies = queryActive ? [this.queryNode, ...activeNodes] : activeNodes, targetX = cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.x, 0) / cameraBodies.length : Number(origin.x), targetY = cameraBodies.length ? cameraBodies.reduce((sum, body) => sum + body.y, 0) / cameraBodies.length : Number(origin.y); if (queryActive || this.mapGroupingMode === 'links') { this.cameraX += (targetX - this.cameraX) * .012; this.cameraY += (targetY - this.cameraY) * .012; }
  }
  startSimulation(alpha = .7) { const startingLinkLayout = this.mapGroupingMode === 'links' && alpha >= .38; if (startingLinkLayout) { this.linkStableFrames = 0; this.linkSimulationTicks = 0; this.linkIdleAnchored = false; } this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = now => { this.simulationFrame = null; if (!this.canvas.isConnected) return; let framing = false; for (const key of ['left', 'right', 'top', 'bottom']) { const delta = this.targetViewportInsets[key] - this.viewportInsets[key]; if (Math.abs(delta) > .25) { this.viewportInsets[key] += delta * .14; framing = true; } else this.viewportInsets[key] = this.targetViewportInsets[key]; } if (framing) { this.lastTerrainAt = 0; this.lastCommunityAt = 0; } const reducedMotion = matchMedia('(prefers-reduced-motion: reduce)').matches, activePhysics = this.alpha > .012 || this.dragging || this.pendingQuery; if (!reducedMotion) { if (activePhysics) this.physicsStep(Math.max(this.alpha, .025)); else this.idlePhysicsStep(now); } else { this.viewportInsets = { ...this.targetViewportInsets }; this.queryPresence = this.targetQueryPresence; this.cameraZoom = this.hasQuery ? 1.35 : 1; this.queryNode.x = this.queryNode.layoutX; this.queryNode.y = this.queryNode.layoutY; for (const node of this.nodes) { node.relevance = node.targetRelevance; node.visibility = node.targetVisibility; node.blur = node.targetBlur; node.accent = node.targetAccent; node.x = node.layoutX; node.y = node.layoutY; } } this.alpha = Math.max(0, this.alpha * (this.mapGroupingMode === 'links' ? .992 : .984) - .00012); this.ambientDrawing = !activePhysics; this.draw(); this.ambientDrawing = false; if (!reducedMotion && !document.hidden) this.simulationFrame = requestAnimationFrame(tick); }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  baseCoordinates(node, width, height) { const point = node.isQuery ? { x: Number(node.x), y: Number(node.y) } : this.ambientPosition(node.x, node.y, false, node.id), frame = this.viewportFrame(width, height), scale = Math.max(20, Math.min(frame.width, frame.height) / 2 - 68) * this.cameraZoom * this.userZoom; return [frame.centerX + (point.x - this.cameraX) * scale + this.panX, frame.centerY + (point.y - this.cameraY) * scale + this.panY]; }
  coordinates(node, width, height) { return this.baseCoordinates(node, width, height); }
  semanticDensityField(width, height) {
    const tuning = this.tuning || MAP_TUNING_DEFAULTS, step = 5, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), heightWeights = new Float32Array(values.length), heightTotals = new Float32Array(values.length), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.baseCoordinates(node, width, height); return [node.id, { node, x, y }]; })), vaultTerrain = !this.hasQuery && !this.pendingQuery, zoom = Math.max(.35, this.cameraZoom * this.userZoom), vaultDensityScale = this.hasQuery ? 1 : Math.max(.48, Math.min(1, Math.sqrt(48 / Math.max(1, visible.length)))), sigma = Math.max(this.hasQuery ? 9 : 11, 27 * zoom * vaultDensityScale) * (vaultTerrain ? Number(tuning.terrainSpread || 1) : 1), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y })), gaussian = ratio => this.gaussianLookup[Math.max(0, Math.min(1024, Math.round(ratio / 9 * 1024)))];
    ...
    const addHeight = (x, y, radius, visibility, uniqueness) => { const reach = radius * 3, left = Math.max(0, Math.floor((x - reach) / step)), right = Math.min(columns - 1, Math.ceil((x + reach) / step)), top = Math.max(0, Math.floor((y - reach) / step)), bottom = Math.min(rows - 1, Math.ceil((y + reach) / step)), divisor = 2 * radius * radius, height = .08 + Math.pow(Math.max(0, Math.min(1, uniqueness)), Number(tuning.terrainContrast || 1)) * .92; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) { const dx = column * step - x, dy = row * step - y, weight = visibility * gaussian((dx * dx + dy * dy) / divisor), index = row * columns + column; heightWeights[index] += weight; heightTotals[index] += weight * height; } };
    ...
    for (const { node, x, y } of points.values()) { const generationWeight = node.generation === 1 ? 1 : node.generation === 2 ? .58 : .36, amplitude = node.visibility * generationWeight * (.82 + Math.max(0, Math.min(1, node.relevance)) * .18); if (vaultTerrain) addHeight(x, y, sigma, node.visibility, Number(node.uniqueness ?? .5)); else addMass(x, y, sigma, amplitude); } if (vaultTerrain) for (let index = 0; index < values.length; index++) { const weight = heightWeights[index]; values[index] = weight > .0001 ? heightTotals[index] / weight * (1 - Math.exp(-weight * 4)) : 0; }
    const degree = new Map(), ridges = vaultTerrain ? [] : (this.activeEdges || []).filter(edge => !edge.crossCommunity && edge.strength > .14 && points.has(edge.source) && points.has(edge.target)).sort((a, b) => b.strength - a.strength).filter(edge => { const source = degree.get(edge.source) || 0, target = degree.get(edge.target) || 0; if (source >= 2 || target >= 2) return false; degree.set(edge.source, source + 1); degree.set(edge.target, target + 1); return true; }).slice(0, 28), ridgeRadius = sigma * .58;
    ...
    if (this.hasQuery && this.queryPresence > .04) { const [centerX, centerY] = this.baseCoordinates(this.queryNode, width, height), center = { x: centerX, y: centerY }, directResults = [...points.values()].filter(({ node }) => node.matched && node.generation === 1).sort((first, second) => second.node.relevance - first.node.relevance).slice(0, 5); for (const target of directResults) addRidge(center, target, sigma * .46, this.queryPresence * target.node.visibility * (.07 + Math.max(0, Math.min(1, target.node.relevance)) * .11)); const supportMaximum = Math.max(0, ...values); addMass(centerX, centerY, sigma * .56, (supportMaximum * 1.18 + .42) * this.queryPresence); anchors.push(center); }
    if (!vaultTerrain && !this.pendingQuery) this.suppressEmptySummits(values, columns, rows, step, anchors, sigma);
    return { values, columns, rows, step, maximum: Math.max(.001, ...values) };
  }
  suppressEmptySummits(values, columns, rows, step, anchors, sigma) {
    if (anchors.length < 2) return; const anchorRadius = Math.max(step * .8, 3.2 * sigma / 27), sample = ({ x, y }) => { const gridX = Math.max(0, Math.min(columns - 1, x / step)), gridY = Math.max(0, Math.min(rows - 1, y / step)), left = Math.floor(gridX), right = Math.min(columns - 1, left + 1), top = Math.floor(gridY), bottom = Math.min(rows - 1, top + 1), horizontal = gridX - left, vertical = gridY - top, upper = values[top * columns + left] * (1 - horizontal) + values[top * columns + right] * horizontal, lower = values[bottom * columns + left] * (1 - horizontal) + values[bottom * columns + right] * horizontal; return upper * (1 - vertical) + lower * vertical; }, anchorHeights = anchors.map(anchor => ({ ...anchor, height: sample(anchor) })), candidates = [];
    for (let row = 1; row < rows - 1; row++) for (let column = 1; column < columns - 1; column++) { const index = row * columns + column, height = values[index]; if (height < .35) continue; let maximum = true, hasLowerNeighbor = false; for (let dy = -1; dy <= 1 && maximum; dy++) for (let dx = -1; dx <= 1; dx++) { if (!dx && !dy) continue; const neighbor = values[(row + dy) * columns + column + dx]; if (neighbor > height) { maximum = false; break; } if (neighbor < height) hasLowerNeighbor = true; } if (!maximum || !hasLowerNeighbor) continue; const x = column * step, y = row * step, nearest = anchorHeights.map(anchor => ({ anchor, distance: Math.hypot(anchor.x - x, anchor.y - y) })).sort((first, second) => first.distance - second.distance)[0], nearestDistance = nearest?.distance ?? Infinity; if (nearestDistance <= anchorRadius) continue; const excess = height - (Number(nearest.anchor.height) - .008); if (excess > .012) candidates.push({ x, y, excess, anchor: nearest.anchor, nearestDistance, safeRadius: Math.min(anchorRadius * .7, nearestDistance * .55), radius: Math.max(step * .9, Math.min(sigma * .55, nearestDistance * .8)) }); }
    if (!candidates.length) return; const depression = new Float32Array(values.length); for (const candidate of candidates.slice(0, 24)) { const reach = candidate.radius * 2.6, left = Math.max(0, Math.floor((candidate.x - reach) / step)), right = Math.min(columns - 1, Math.ceil((candidate.x + reach) / step)), top = Math.max(0, Math.floor((candidate.y - reach) / step)), bottom = Math.min(rows - 1, Math.ceil((candidate.y + reach) / step)), divisor = 2 * candidate.radius * candidate.radius; for (let row = top; row <= bottom; row++) for (let column = left; column <= right; column++) { const dx = column * step - candidate.x, dy = row * step - candidate.y, anchorDistance = Math.hypot(column * step - candidate.anchor.x, row * step - candidate.anchor.y), rawGuard = Math.max(0, Math.min(1, (anchorDistance - candidate.safeRadius) / Math.max(1, candidate.nearestDistance - candidate.safeRadius))), guard = rawGuard * rawGuard * (3 - 2 * rawGuard), index = row * columns + column, amount = candidate.excess * guard * Math.exp(-(dx * dx + dy * dy) / divisor); depression[index] = Math.max(depression[index], amount); } } for (let index = 0; index < values.length; index++) values[index] = Math.max(0, values[index] - depression[index]);
  }
  paintDensityTerrain(ctx, width, height, colors) {
    if (this.tuning?.showTopography === false) return;
    const now = performance.now(), colorKey = `${colors.normal}|${colors.muted}`, refreshInterval = this.ambientDrawing ? 180 : 48, dimensionsChanged = !this.terrainCanvas || this.terrainCanvas.width !== Math.ceil(width) || this.terrainCanvas.height !== Math.ceil(height), refresh = dimensionsChanged || !this.pendingQuery && (this.terrainColorKey !== colorKey || now - Number(this.lastTerrainAt || 0) >= refreshInterval); if (refresh) { const field = this.semanticDensityField(width, height), maximum = Math.max(0, ...field.values), vaultTerrain = !this.hasQuery && !this.pendingQuery, terrainRange = vaultTerrain ? .92 : 3; this.lastTerrainField = field; if (!this.terrainCanvas) this.terrainCanvas = document.createElement('canvas'); if (!this.terrainMask) this.terrainMask = document.createElement('canvas'); const terrain = this.terrainCanvas, mask = this.terrainMask; terrain.width = Math.ceil(width); terrain.height = Math.ceil(height); mask.width = field.columns; mask.height = field.rows; const terrainContext = terrain.getContext('2d'), maskContext = mask.getContext('2d'), image = maskContext.createImageData(field.columns, field.rows); maskContext.fillStyle = colors.normal; maskContext.fillRect(0, 0, 1, 1); const sample = maskContext.getImageData(0, 0, 1, 1).data; for (let index = 0; index < field.values.length; index++) { const alpha = Math.round(Math.max(0, Math.min(.055, (field.values[index] - .08) / terrainRange * .055)) * 255), offset = index * 4; image.data[offset] = sample[0]; image.data[offset + 1] = sample[1]; image.data[offset + 2] = sample[2]; image.data[offset + 3] = alpha; } maskContext.putImageData(image, 0, 0); terrainContext.clearRect(0, 0, terrain.width, terrain.height); if (maximum >= .16) { terrainContext.imageSmoothingEnabled = true; terrainContext.drawImage(mask, 0, 0, field.columns, field.rows, 0, 0, field.columns * field.step, field.rows * field.step); const levels = vaultTerrain ? [.16, .32, .48, .64, .8, .92] : [.35, .62, .95, 1.35, 1.9, 2.6]; terrainContext.lineCap = 'round'; terrainContext.lineJoin = 'round'; levels.forEach((level, index) => { if (maximum < level) return; this.traceDensityLevel(terrainContext, field, level); terrainContext.strokeStyle = colors.muted; terrainContext.globalAlpha = .14 + index * .035; terrainContext.lineWidth = index === levels.length - 1 ? 1.05 : .72; terrainContext.stroke(); }); terrainContext.globalAlpha = 1; } this.lastTerrainAt = now; this.terrainColorKey = colorKey; }
  ...
  topicCommunityGroups(width, height) {
    ...
    const tuning = this.tuning || MAP_TUNING_DEFAULTS, threshold = this.hasQuery ? .01 : Number(tuning.communityMembership || .42), minimum = Math.max(2, Number(tuning.communityMinSize || 3)), visible = this.nodes.filter(node => node.visibility > .08 && (!this.hasQuery || this.pendingQuery || node.matched) && Number(node.communityMembership || 0) >= threshold), grouped = new Map();
    ...
    const groups = [...grouped].filter(([, values]) => values.length >= minimum).map(([id, values]) => { const weightedHue = values.filter(value => Number.isFinite(value.node.topicHue)), hueX = weightedHue.reduce((sum, value) => sum + Math.cos(value.node.topicHue * Math.PI / 180), 0), hueY = weightedHue.reduce((sum, value) => sum + Math.sin(value.node.topicHue * Math.PI / 180), 0); return { id, values, label: values.find(value => value.node.communityLabel)?.node.communityLabel || values.find(value => value.node.communityFallbackLabel)?.node.communityFallbackLabel || '', confidence: Math.max(...values.map(value => Number(value.node.communityLabelConfidence || 0))), naturalHue: weightedHue.length ? (Math.atan2(hueY, hueX) * 180 / Math.PI + 360) % 360 : stableMapAngle(id) * 180 / Math.PI }; }).filter(group => group.label).sort((first, second) => first.naturalHue - second.naturalHue || String(first.id).localeCompare(String(second.id)));
    ...
    groups.forEach((group, index) => { group.hue = count > 1 ? (offset + index * step) % 360 : group.naturalHue; group.lightness = count > 10 ? 59 + index % 2 * 8 : 63; for (const value of group.values) { value.node.displayTopicHue = group.hue; value.node.displayTopicLightness = group.lightness; } });
    return groups;
  }
  ...
  communityCoreIslands(values, padding) { const tuning = this.tuning || MAP_TUNING_DEFAULTS, memberships = values.map(value => Number(value.node.communityMembership || 0)).sort((a, b) => a - b), floor = Math.max(Number(tuning.communityMembership || .42), memberships[Math.floor(memberships.length * .35)] || 0), core = values.filter(value => Number(value.node.communityMembership || 0) >= floor), remaining = new Set(core), islands = [], reach = padding * 2.45; while (remaining.size) { const first = remaining.values().next().value, island = [first], queue = [first]; remaining.delete(first); while (queue.length) { const current = queue.shift(); for (const candidate of [...remaining]) if (Math.hypot(candidate.x - current.x, candidate.y - current.y) <= reach) { remaining.delete(candidate); island.push(candidate); queue.push(candidate); } } if (island.length >= 2) islands.push(island); } return islands; }
  ...
  paintTopicCommunities(ctx, width, height, colors) {
    if (this.mapGroupingMode !== 'topics') { this.communityLabelsForDraw = []; return; }
    const now = performance.now(), dpr = window.devicePixelRatio || 1, colorKey = `${colors.muted}|${this.options.semanticColors !== false}`, refreshInterval = this.pendingQuery ? 150 : this.ambientDrawing ? 220 : 64, refresh = !this.communityCanvas || this.communityCanvas.width !== Math.ceil(width * dpr) || this.communityCanvas.height !== Math.ceil(height * dpr) || this.communityColorKey !== colorKey || now - Number(this.lastCommunityAt || 0) >= refreshInterval;
    if (refresh) { if (!this.communityCanvas) this.communityCanvas = document.createElement('canvas'); const canvas = this.communityCanvas; canvas.width = Math.ceil(width * dpr); canvas.height = Math.ceil(height * dpr); const communityContext = canvas.getContext('2d'); communityContext.setTransform(dpr, 0, 0, dpr, 0, 0); communityContext.clearRect(0, 0, width, height); const groups = this.topicCommunityGroups(width, height), tuning = this.tuning || MAP_TUNING_DEFAULTS, padding = 17 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))), labels = [];
      for (const group of groups) { const color = this.options.semanticColors !== false ? `hsl(${group.hue} 58% ${group.lightness}%)` : colors.muted, regions = this.hasQuery ? [group.values] : this.communityCoreIslands(group.values, padding); let label = null; for (const region of regions) { const field = this.communityBoundaryField(region, padding, this.hasQuery); communityContext.save(); communityContext.translate(field.left, field.top); this.traceDensityLevel(communityContext, field, field.level); communityContext.strokeStyle = color; communityContext.globalAlpha = this.hasQuery ? .28 : .2; communityContext.lineWidth = 1; communityContext.lineCap = 'round'; communityContext.lineJoin = 'round'; communityContext.stroke(); communityContext.restore(); const weight = region.reduce((sum, value) => sum + Math.max(.01, Number(value.node.communityMembership || 0)), 0), x = region.reduce((sum, value) => sum + value.x * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight, y = region.reduce((sum, value) => sum + value.y * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight, candidate = { x, y, text: group.label, color, confidence: group.confidence, size: region.length }; if (!label || candidate.size > label.size) label = candidate; } if (label) labels.push(label); }
      communityContext.globalAlpha = 1; this.communityLabelsForDraw = labels; this.lastCommunityAt = now; this.communityColorKey = colorKey;
    }
  ...
  drawCommunityLabels(ctx, colors) { const detail = this.semanticDetailLevel(), alpha = this.hasQuery ? .58 : .46 * (1 - detail * .68); for (const label of this.communityLabelsForDraw || []) { const { x, y } = label, text = String(label.text || ''); if (!text) continue; ctx.font = `${this.hasQuery ? 600 : 560} ${this.hasQuery ? 11 : 13}px -apple-system, BlinkMacSystemFont, sans-serif`; ctx.textAlign = 'center'; ctx.lineWidth = 4; ctx.lineJoin = 'round'; ctx.globalAlpha = alpha * .82; ctx.strokeStyle = colors.background; ctx.strokeText(text, x, y + 4); ctx.globalAlpha = alpha; ctx.fillStyle = this.hasQuery ? label.color : colors.muted; ctx.fillText(text, x, y + 4); } ctx.globalAlpha = 1; ctx.textAlign = 'left'; }
  ...
  draw() {
    ...
    for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, landmark = Boolean(this.landmarkIds?.has(node.id)), hierarchy = this.hasQuery ? 1 : landmark ? 1 : .28 + detail * .72, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale * (this.hasQuery || active ? 1 : landmark ? 1.14 : .62 + detail * .38)), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.008, node.visibility * focusAlpha * hierarchy), blur = Math.max(0, Math.min(1, Number(node.blur || 0))), hue = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicHue) ? node.displayTopicHue : node.topicHue, lightness = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicLightness) ? node.displayTopicLightness : 64, semanticColor = node.matched || !this.hasQuery || this.pendingQuery ? this.options.semanticColors !== false && Number.isFinite(hue) ? `hsl(${hue} 54% ${lightness}%)` : colors.normal : colors.faint, alpha = opacity * (1 - blur * .28); if (this.webglRenderer) { if (blur > .04) gpuPoints.push({ x, y, size: (radius + 2.2 * blur) * 2, color: mapColorRgba(this, semanticColor, opacity * blur * .22) }); gpuPoints.push({ x, y, size: radius * 2, color: mapColorRgba(this, semanticColor, alpha) }); } else { ctx.fillStyle = semanticColor; if (blur > .04) { ctx.globalAlpha = opacity * blur * .22; ctx.beginPath(); ctx.arc(x, y, radius + 2.2 * blur, 0, Math.PI * 2); ctx.fill(); } ctx.globalAlpha = alpha; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); } if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery || this.options.preserveBackground) this.hit.push({ node, x, y, radius: radius + 9 }); }
    if (this.webglRenderer) { const roadFrame = this.roadAnimationFrame !== null ? Math.round(performance.now() / 16) : 0, textureKey = `${layer.width}:${layer.height}:${this.lastTerrainAt || 0}:${this.lastCommunityAt || 0}:${this.terrainColorKey || ''}:${this.communityColorKey || ''}:${this.tuning?.showTopography !== false}:${this.showLinks}:${focused || ''}:${roadFrame}`; this.webglRenderer.render(layer, gpuPoints, textureKey, rect.width, rect.height, dpr); }
  ...
  destroy() { document.removeEventListener('visibilitychange', this.visibilityHandler); cancelAnimationFrame(this.roadAnimationFrame); cancelAnimationFrame(this.simulationFrame); cancelAnimationFrame(this.animationFrame); this.webglRenderer?.destroy(); this.resizeObserver?.disconnect(); }
}

class SemanticSearchModal extends SuggestModal {
  constructor(app, plugin, filePath = null) {
    super(app); this.plugin = plugin; this.filePath = filePath; this.debounceTimer = null; this.searchVersion = 0; this.lastResults = []; this.mapResults = []; this.lastQuery = ''; this.visibleLimit = 0; this.canLoadMore = false; this.navigationHandler = null; this.map = null; this.mapVersion = 0; this.lensVersion = 0; this.lens = validSearchLens(plugin.settings.defaultSearchLens); this.lensRelationships = new Map(); this.lensRelationshipEdges = []; this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1)); this.mapGroupingMode = validMapGrouping(plugin.settings.mapGroupingMode);
  ...
  async applyLens() {
    ...
    try {
      ...
      if (lens === 'links') return this.commitLensResults(source, version);
      if (lens === 'context') {
  ...
  setupMap() {
    ...
    this.map = new LivingSemanticMapCanvas(panel, this.app, { title: 'Search map', generations: this.mapGenerations, mapGroupingMode: this.mapGroupingMode, showLinks: this.plugin.settings.showWikilinks, manualLinkInfluence: this.plugin.settings.graphManualLinkInfluence, magicGraph: this.plugin.settings.magicGraphEnabled, semanticColors: this.plugin.settings.graphSemanticColors, tuning: this.plugin.settings.mapTuning, onMapMode: async value => { this.mapGroupingMode = validMapGrouping(value); this.plugin.settings.mapGroupingMode = this.mapGroupingMode; await this.plugin.save(); this.updateMap(); }, onTune: (values, key) => { this.plugin.settings.mapTuning = values; window.clearTimeout(this.mapTuneTimer); this.mapTuneTimer = window.setTimeout(async () => { if (mapTuningNeedsEvidence(key)) this.plugin.search.invalidateGraphEvidence(); else if (mapTuningNeedsLayout(key)) this.plugin.search.clearStarfieldCaches(); await this.plugin.save(); if (mapTuningNeedsLayout(key)) this.updateMap(); else this.map?.draw(); }, 180); }, onGenerations: async value => { this.mapGenerations = value; this.plugin.settings.searchMapGenerations = value; await this.plugin.save(); this.updateMap(); }, onShowLinks: async value => { this.plugin.settings.showWikilinks = value; await this.plugin.save(); }, onManualLinkInfluence: async value => { this.plugin.settings.graphManualLinkInfluence = value; await this.plugin.save(); }, onHover: file => this.hoverResult(file), onSelect: file => this.focusResult(file), onOpen: file => this.openFile(file), onExplore: file => { this.close(); this.plugin.openNeighborhood(file, true); } });
  ...
  async updateMap() {
    if (!this.map || !this.plugin.settings.searchMapEnabled) return; const version = ++this.mapVersion, query = this.lastQuery, results = this.lastResults.slice(0, 80), definition = atlasLens(this.lens); this.map.setTitle(`${SEARCH_LENSES[this.lens].label} map`); this.map.setMapGroupingMode(definition.mapMode);
    if (!query) { const state = this.plugin.atlas.state({ lens: this.lens }), scene = await this.plugin.atlas.scene(state); if (version !== this.mapVersion) return; this.map.setGraph(scene.center, scene.nodes, scene.edges, scene.roads); return; }
    const model = await buildQueryMapModel(this.plugin, query, results, { lens: this.lens, generations: this.mapGenerations, relationships: this.lensRelationships, relationshipEdges: this.lensRelationshipEdges, mapMode: this.mapGroupingMode }); if (version !== this.mapVersion) return;
    this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, model.nodes, model.edges, model.roads);
  }
...
class AtlasNavigatorView extends ItemView {
  ...
  renderContext(context) {
    this.context = context; const state = context.state || this.plugin.atlas.state(), query = String(context.query || state.anchor?.type === 'query' && state.anchor?.value || '').trim(), results = Array.isArray(context.results) ? context.results : [], selected = context.selection || null, signature = JSON.stringify([query, Boolean(context.loading), context.error || '', results.map(result => [result.file, Number(result.score || 0)])]); this.titleEl.textContent = query || state.viewName || 'All Notes'; this.metaEl.textContent = context.loading ? 'Charting results…' : query ? `${results.length} result${results.length === 1 ? '' : 's'} · ${atlasLens(state.lens).label}` : `${state.viewName || 'All Notes'} · ${atlasLens(state.lens).label}`;
...
class NeighborhoodView extends ItemView {
  constructor(leaf, plugin) { super(leaf); this.plugin = plugin; this.filePath = null; this.pinned = false; this.loadVersion = 0; this.lens = validSearchLens(plugin.settings.defaultSearchLens); this.atlasMode = false; this.inheritedState = null; }
  getViewType() { return NEIGHBORHOOD_VIEW; }
  getDisplayText() { return 'Atlas Companion'; }
  getIcon() { return 'compass'; }
  async onOpen() {
    this.contentEl.empty(); this.contentEl.addClass('gib-neighborhood-view'); const toolbar = this.contentEl.createDiv({ cls: 'gib-neighborhood-toolbar' }); const heading = toolbar.createDiv({ cls: 'gib-neighborhood-heading' }); heading.createDiv({ cls: 'gib-neighborhood-kicker', text: 'Atlas Companion' }); this.noteTitle = heading.createDiv({ cls: 'gib-neighborhood-note', text: 'Follow the active note' }); this.lensSelect = toolbar.createEl('select', { cls: 'dropdown gib-neighborhood-lens', attr: { 'aria-label': 'Companion lens', title: 'Relationship lens' } }); for (const [value, lens] of Object.entries(SEARCH_LENSES)) this.lensSelect.createEl('option', { text: lens.label, attr: { value } }); this.lensSelect.value = this.lens; this.lensSelect.addEventListener('change', () => { this.lens = validSearchLens(this.lensSelect.value); if (this.filePath) this.centerOn(this.filePath, this.pinned, this.inheritedState); });
  ...
  followNote(filePath) { this.atlasMode = false; this.inheritedState = null; this.lensSelect.disabled = false; this.centerOn(filePath, false, null); }
  followAtlas(context) { this.atlasMode = true; this.inheritedState = context.state || this.plugin.atlas.state(); this.lens = validSearchLens(this.inheritedState.lens); this.lensSelect.value = this.lens; this.lensSelect.disabled = true; const target = context.hover || context.selection; window.clearTimeout(this.contextTimer); if (!target) { this.empty('Select a note or region in the Atlas'); return; } this.contextTimer = window.setTimeout(() => this.centerOn(target, false, this.inheritedState), context.hover ? 70 : 0); }
  async centerOn(filePath, pin = this.pinned, inheritedState = this.inheritedState) {
    const file = this.app.vault.getAbstractFileByPath(filePath); if (!(file instanceof TFile)) return; this.filePath = file.path; if (pin) this.pinned = true; this.updatePin(); this.noteTitle.textContent = file.basename; const version = ++this.loadVersion, base = inheritedState || this.plugin.atlas.state(), state = this.plugin.atlas.state({ viewId: base.viewId, anchor: { type: 'note', value: file.path }, lens: inheritedState ? base.lens : this.lens, scale: 'neighborhood' });
    try { const scene = await this.plugin.atlas.scene(state); if (version !== this.loadVersion) return; this.map.setMapGroupingMode(atlasLens(state.lens).mapMode); this.map.setGraph(scene.center, scene.nodes, scene.edges, scene.roads); this.map.setIntelligenceStatus(''); this.renderList(scene); }
  ...
  empty(message) { this.loadVersion++; this.filePath = null; this.noteTitle.textContent = message; this.map?.setGraph({ label: 'Companion', hasQuery: false, resultCount: 0 }, [], [], []); if (this.list) { this.list.empty(); this.list.createDiv({ cls: 'gib-companion-empty', text: message }); } if (this.listStatus) this.listStatus.textContent = ''; }
  async openFile(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (file instanceof TFile) await this.app.workspace.getLeaf(false).openFile(file); }
...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.results = []; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.baseScene = null; this.atlasState = plugin.atlas.state(); this.atlasScope = plugin.atlas.resolveScope(this.atlasState); this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1));
  }
  ...
  async onOpen() {
    ...
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      title: 'Atlas', minimalChrome: true, preserveBackground: true, mapGroupingMode: atlasLens(this.atlasState.lens).mapMode, showLinks: this.plugin.settings.showWikilinks, manualLinkInfluence: this.plugin.settings.graphManualLinkInfluence, semanticColors: this.plugin.settings.graphSemanticColors, tuning: this.plugin.settings.mapTuning,
      onHover: file => this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), query: this.query, results: this.results, hover: file }), onSelect: file => this.selectResult(file, true), onOpen: file => this.openFile(file), onViewportScale: value => this.setScaleIndicator(value)
    });
    this.setScaleIndicator(this.map.semanticZoomBand()); this.input.addEventListener('input', () => this.handleQuery(this.input.value)); this.input.addEventListener('keydown', event => { if (event.key !== 'Escape') return; if (this.map.selected) this.selectResult(null); else if (this.input.value) { this.input.value = ''; this.handleQuery(''); } });
    this.graphCacheOff = this.plugin.search.onGraphCache(() => { this.plugin.atlas.clear(); if (this.query.length < 3) this.loadVault(); });
    await this.loadVault(); this.input.disabled = false; this.input.placeholder = 'Search the vault by meaning…'; this.input.focus(); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), query: '', results: [], loading: false, selection: null, hover: null });
  }
  setScaleIndicator(value) { for (const band of this.scaleBands || []) band.toggleClass('is-active', band.dataset.scale === value); }
  currentAtlasState() { return this.query.length >= 3 ? this.plugin.atlas.state({ viewId: this.atlasState.viewId, anchor: { type: 'query', value: this.query }, lens: this.atlasState.lens, scale: this.atlasState.scale }) : this.atlasState; }
  ...
  selectResult(filePath, fromMap = false) { if (!fromMap) this.map.setSelected(filePath || null); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), query: this.query, results: this.results, loading: false, selection: filePath || null, hover: null }); }
  async openFile(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (file instanceof TFile) await this.app.workspace.getLeaf(false).openFile(file); }
  async onClose() { this.loadVersion++; this.searchVersion++; window.clearTimeout(this.searchTimer); window.clearTimeout(this.tuneTimer); this.graphCacheOff?.(); this.map?.destroy(); }
}
...
class SearchSettings extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(this.containerEl).setName('Generated topic labels').setDesc('Use a small local language model to name full-vault topic communities from representative passages. The first use downloads it; labels are cached and statistical labels remain available as a fallback.').addToggle(t => t.setValue(this.plugin.settings.generatedTopicLabels).onChange(async value => { this.plugin.settings.generatedTopicLabels = value; this.plugin.search.clearStarfieldCaches(); await this.plugin.save(); }));
  ...
  async refreshHealth(showNotice = false) {
    ...
    this.healthFields = []; this.field('Phase', stoppedResponding ? 'stopped' : phase.replaceAll('_', ' ')); this.field('Progress', total ? `${done}/${total}` : 'Waiting'); this.field('Indexed', remote?.indexedFiles ?? local.indexedFiles ?? 0); this.field('Chunks', remote?.totalChunks ?? local.totalChunks ?? 0); this.field('Cached highlight phrases', remote?.highlightPhrases ?? local.highlightPhrases ?? 0); this.field('Graph entities', remote?.graphEntities ?? 0); this.field('Graph cache', remote?.graphCacheReady ? 'Ready' : remote?.isIndexing ? 'Waiting for index' : 'Preparing'); this.field('Cached relationships', remote?.cachedRelationships ?? 0); this.field('Cached topic labels', remote?.cachedTopicLabels ?? 0); const modelLabel = MODEL_PROFILES[remote?.modelProfile]?.label || remote?.modelId || 'Loaded'; this.field('Model', remote?.modelLoaded ? `${modelLabel} (${String(remote.modelBackend || 'WASM').toUpperCase()})` : healthy ? 'Loads on demand' : 'Not ready'); this.field('Topic labeler', remote?.topicLabelModelLoaded ? 'Loaded' : this.plugin.settings.generatedTopicLabels ? 'Loads on demand' : 'Disabled'); this.field('Relationship model', remote?.relationModelLoaded ? 'Loaded' : this.plugin.settings.graphRelationshipIntelligence ? 'Loads on demand' : 'Disabled'); this.field('Index size', formatBytes(indexBytes)); if (!this.plugin.isMobile) this.field('Model cache', formatBytes(modelBytes)); this.field('Last success', formatWhen(local.lastSuccessfulIndexAt)); if (activelyWorking && elapsedFrom) this.field('Elapsed', formatElapsed(Date.now() - elapsedFrom)); this.healthGrid.textContent = this.healthFields.join(' · ');
...
module.exports = class GibSearch extends Plugin {
  async onload() {
    ...
    this.settings = Object.assign({}, DEFAULTS, loaded); delete this.settings.mapProjection; this.settings.mapTuning = Object.assign({}, MAP_TUNING_DEFAULTS, loaded.mapTuning || {}); this.settings.atlasViews = Array.isArray(loaded.atlasViews) && loaded.atlasViews.length ? loaded.atlasViews : [JSON.parse(JSON.stringify(DEFAULT_ATLAS_VIEW))]; this.settings.atlasHomeViewId = loaded.atlasHomeViewId || this.settings.atlasViews[0].id || DEFAULT_ATLAS_VIEW.id; this.isMobile = Platform.isMobileApp;
    ...
    this.registerView(GRAPH_VIEW, leaf => new GraphView(leaf, this)); this.registerView(NAVIGATOR_VIEW, leaf => new AtlasNavigatorView(leaf, this)); this.registerView(NEIGHBORHOOD_VIEW, leaf => new NeighborhoodView(leaf, this));
