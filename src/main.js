...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setUserZoom(value, x = null, y = null) { const rect = this.canvas.getBoundingClientRect(), previous = this.userZoom, next = Math.max(.45, Math.min(4.5, Number(value) || 1)); if (!rect.width || !rect.height || Math.abs(next - previous) < .0001) return; const relativeX = (x ?? rect.width / 2) - rect.width / 2, relativeY = (y ?? rect.height / 2) - rect.height / 2, factor = next / previous; this.panX = relativeX - (relativeX - this.panX) * factor; this.panY = relativeY - (relativeY - this.panY) * factor; this.userZoom = next; this.lastTerrainAt = 0; this.roadsDirty = true; this.draw(); }
  zoomBy(factor) { this.setUserZoom(this.userZoom * factor); }
  resetViewport() { this.userZoom = 1; this.panX = 0; this.panY = 0; this.lastTerrainAt = 0; this.roadsDirty = true; this.draw(); }
  ...
  setGraph(center, values, edges = [], roads = []) {
    ...
    this.pendingQuery = false; this.byId = new Map(this.nodes.map(node => [node.id, node])); const validRoadEndpoint = id => this.byId.has(id) || id === center?.id, nextRoads = roads.filter(road => validRoadEndpoint(road.source) && validRoadEndpoint(road.target)), nextKeys = new Set(nextRoads.map(road => road.key || mapEdgeKey(road.source, road.target))), introduced = new Map(); nextRoads.forEach(road => { const key = road.key || mapEdgeKey(road.source, road.target); if (this.roadIntroducedAt.has(key)) introduced.set(key, this.roadIntroducedAt.get(key)); }); this.cancelRoadWork(); this.roadEdges = nextRoads; this.roadIntroducedAt = introduced; this.roadRoutes = new Map([...this.roadRoutes].filter(([key]) => nextKeys.has(key))); this.roadsDirty = true; this.selected = this.byId.has(this.selected) ? this.selected : null; this.hovered = this.byId.has(this.hovered) || this.hovered === '__query__' ? this.hovered : null; const activeCount = this.nodes.filter(node => node.matched).length; this.statusEl.textContent = hasQuery ? `${Number(center.resultCount || 0)} results${activeCount > Number(center.resultCount || 0) ? ` · ${activeCount} shown` : ''}` : `${this.nodes.length} notes`; this.startSimulation(previous.size ? .82 : 1); this.updateDetail();
  }
  ...
  semanticDensityField(width, height) {
    const step = 5, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), points = new Map(visible.map(node => { const [x, y] = this.coordinates(node, width, height); return [node.id, { node, x, y }]; })), zoom = Math.max(.35, this.cameraZoom * this.userZoom), sigma = Math.max(9, 27 * zoom), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y })), gaussian = ratio => this.gaussianLookup[Math.max(0, Math.min(1024, Math.round(ratio / 9 * 1024)))];
  ...
  resampleRoad(points, count) { if (points.length < 2) return points; const cumulative = [0]; for (let index = 1; index < points.length; index++) cumulative.push(cumulative[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); const total = cumulative[cumulative.length - 1] || 1, sampled = []; let cursor = 1; for (let index = 0; index < count; index++) { const distance = total * index / Math.max(1, count - 1); while (cursor < cumulative.length - 1 && cumulative[cursor] < distance) cursor++; const first = points[cursor - 1], second = points[cursor], span = Math.max(.001, cumulative[cursor] - cumulative[cursor - 1]), blend = (distance - cumulative[cursor - 1]) / span; sampled.push({ x: first.x + (second.x - first.x) * blend, y: first.y + (second.y - first.y) * blend }); } return sampled; }
  routeRoad(field, source, target, traffic = null) {
    const { values, columns, rows, step } = field, clampColumn = value => Math.max(0, Math.min(columns - 1, Math.round(value / step))), clampRow = value => Math.max(0, Math.min(rows - 1, Math.round(value / step))), startColumn = clampColumn(source.x), startRow = clampRow(source.y), goalColumn = clampColumn(target.x), goalRow = clampRow(target.y), start = startRow * columns + startColumn, goal = goalRow * columns + goalColumn; if (start === goal) return { points: [source, target], cells: [start] }; const gridDistance = Math.hypot(goalColumn - startColumn, goalRow - startRow), padding = Math.max(8, Math.min(30, Math.ceil(gridDistance * .38))), minColumn = Math.max(0, Math.min(startColumn, goalColumn) - padding), maxColumn = Math.min(columns - 1, Math.max(startColumn, goalColumn) + padding), minRow = Math.max(0, Math.min(startRow, goalRow) - padding), maxRow = Math.min(rows - 1, Math.max(startRow, goalRow) + padding), scores = new Float64Array(values.length), previous = new Int32Array(values.length); scores.fill(Infinity); previous.fill(-1); scores[start] = 0; const heap = [{ index: start, priority: gridDistance }], push = item => { heap.push(item); let child = heap.length - 1; while (child > 0) { const parent = Math.floor((child - 1) / 2); if (heap[parent].priority <= item.priority) break; heap[child] = heap[parent]; child = parent; } heap[child] = item; }, pop = () => { const first = heap[0], last = heap.pop(); if (heap.length && last) { let parent = 0; heap[0] = last; while (true) { const left = parent * 2 + 1, right = left + 1; if (left >= heap.length) break; const child = right < heap.length && heap[right].priority < heap[left].priority ? right : left; if (heap[child].priority >= heap[parent].priority) break; [heap[parent], heap[child]] = [heap[child], heap[parent]]; parent = child; } } return first; }, directions = [[-1, -1, 1.414], [0, -1, 1], [1, -1, 1.414], [-1, 0, 1], [1, 0, 1], [-1, 1, 1.414], [0, 1, 1], [1, 1, 1.414]]; let reached = false, expansions = 0;
    const expansionLimit = Math.min(18000, (maxColumn - minColumn + 1) * (maxRow - minRow + 1)); while (heap.length && expansions++ < expansionLimit) { const current = pop(), index = current.index; if (index === goal) { reached = true; break; } const row = Math.floor(index / columns), column = index - row * columns, currentScore = scores[index]; for (const [dx, dy, move] of directions) { const nextColumn = column + dx, nextRow = row + dy; if (nextColumn < minColumn || nextColumn > maxColumn || nextRow < minRow || nextRow > maxRow) continue; const next = nextRow * columns + nextColumn, endpointDistance = Math.min(Math.hypot(nextColumn - startColumn, nextRow - startRow), Math.hypot(nextColumn - goalColumn, nextRow - goalRow)), gradeWeight = Math.min(1, endpointDistance / 4), grade = Math.abs(values[next] - values[index]), elevation = Math.max(0, values[next] - .28), slopeCost = (grade * 88 + grade * grade * 210) * gradeWeight, corridor = Math.min(.7, Number(traffic?.[next] || 0) * .24), leaving = Number(traffic?.[index] || 0) > .5 && Number(traffic?.[next] || 0) < .2 ? .36 : 0, tentative = currentScore + (move + slopeCost + elevation * .035) * (1 - corridor) + leaving; if (tentative >= scores[next]) continue; scores[next] = tentative; previous[next] = index; const heuristic = Math.hypot(goalColumn - nextColumn, goalRow - nextRow) * .72; push({ index: next, priority: tentative + heuristic }); } }
    const raw = [], cells = []; if (reached) for (let index = goal; index !== -1; index = previous[index]) { const row = Math.floor(index / columns), column = index - row * columns; cells.push(index); raw.push({ x: column * step, y: row * step }); if (index === start) break; } else { const bend = Math.min(44, Math.hypot(target.x - source.x, target.y - source.y) * .22), dx = target.x - source.x, dy = target.y - source.y, length = Math.max(1, Math.hypot(dx, dy)), normalX = -dy / length, normalY = dx / length, steps = Math.max(2, Math.ceil(gridDistance)); for (let index = steps; index >= 0; index--) { const fraction = index / steps, curve = Math.sin(Math.PI * fraction) * bend, x = source.x + dx * fraction + normalX * curve, y = source.y + dy * fraction + normalY * curve, column = clampColumn(x), row = clampRow(y); raw.push({ x, y }); cells.push(row * columns + column); } } raw.reverse(); cells.reverse(); raw[0] = { x: source.x, y: source.y }; raw[raw.length - 1] = { x: target.x, y: target.y }; const simplify = points => { if (points.length < 3) return points; const first = points[0], last = points[points.length - 1], dx = last.x - first.x, dy = last.y - first.y, denominator = Math.max(.001, dx * dx + dy * dy); let farthest = 0, split = 0; for (let index = 1; index < points.length - 1; index++) { const point = points[index], along = Math.max(0, Math.min(1, ((point.x - first.x) * dx + (point.y - first.y) * dy) / denominator)), distance = Math.hypot(point.x - (first.x + dx * along), point.y - (first.y + dy * along)); if (distance > farthest) { farthest = distance; split = index; } } if (farthest <= step * .55) return [first, last]; const left = simplify(points.slice(0, split + 1)), right = simplify(points.slice(split)); return [...left.slice(0, -1), ...right]; }; let smooth = simplify(raw); for (let pass = 0; pass < 2; pass++) { const next = [smooth[0]]; for (let index = 0; index < smooth.length - 1; index++) { const first = smooth[index], second = smooth[index + 1]; next.push({ x: first.x * .75 + second.x * .25, y: first.y * .75 + second.y * .25 }, { x: first.x * .25 + second.x * .75, y: first.y * .25 + second.y * .75 }); } next.push(smooth[smooth.length - 1]); smooth = next; } return { points: this.resampleRoad(smooth, Math.max(16, Math.min(30, Math.ceil(gridDistance / 2.8)))), cells };
  }
  cancelRoadWork() { this.roadWorkToken++; if (this.roadWorkHandle !== null) { if (this.roadWorkUsesIdle && typeof window.cancelIdleCallback === 'function') window.cancelIdleCallback(this.roadWorkHandle); else window.clearTimeout(this.roadWorkHandle); } this.roadWorkHandle = null; }
  scheduleRoadWork(callback) { if (typeof window.requestIdleCallback === 'function') { this.roadWorkUsesIdle = true; this.roadWorkHandle = window.requestIdleCallback(callback, { timeout: 100 }); } else { this.roadWorkUsesIdle = false; this.roadWorkHandle = window.setTimeout(() => callback({ timeRemaining: () => 7, didTimeout: true }), 0); } }
  depositRoadTraffic(field, traffic, cells, amount = 1) { const { columns, rows } = field; for (const cell of cells) { const row = Math.floor(cell / columns), column = cell - row * columns; for (let dy = -2; dy <= 2; dy++) for (let dx = -2; dx <= 2; dx++) { const nextRow = row + dy, nextColumn = column + dx; if (nextRow < 0 || nextRow >= rows || nextColumn < 0 || nextColumn >= columns) continue; const distance = Math.hypot(dx, dy), weight = distance < .5 ? 1 : distance <= 1.5 ? .36 : distance <= 2.25 ? .12 : 0; if (weight) traffic[nextRow * columns + nextColumn] += amount * weight; } } }
  beginRoadRouteRefresh(field, width, height) {
    this.cancelRoadWork(); const token = this.roadWorkToken, degree = new Map(); for (const road of this.roadEdges) { degree.set(road.source, (degree.get(road.source) || 0) + 1); degree.set(road.target, (degree.get(road.target) || 0) + 1); } const queue = this.roadEdges.slice(0, 72).sort((first, second) => (degree.get(second.source) || 0) + (degree.get(second.target) || 0) + Number(second.count || 0) * .25 - (degree.get(first.source) || 0) - (degree.get(first.target) || 0) - Number(first.count || 0) * .25), nextRoutes = new Map(), traffic = new Float32Array(field.values.length), work = deadline => { if (token !== this.roadWorkToken) return; this.roadWorkHandle = null; let processed = 0; while (queue.length && processed < 1 && (processed === 0 || deadline.timeRemaining() > 3)) { const road = queue.shift(), key = road.key || mapEdgeKey(road.source, road.target), source = this.roadEndpoint(road.source, width, height), target = this.roadEndpoint(road.target, width, height); if (source && target) { const routed = this.routeRoad(field, source, target, traffic), points = routed.points, cumulative = [0]; this.depositRoadTraffic(field, traffic, routed.cells, Math.max(1, Number(road.count || 1))); for (let index = 1; index < points.length; index++) cumulative.push(cumulative[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); nextRoutes.set(key, { road, points, cells: routed.cells, cumulative, total: cumulative[cumulative.length - 1] || 1, source, target }); if (!this.roadIntroducedAt.has(key)) this.roadIntroducedAt.set(key, performance.now()); } processed++; } this.roadRoutes = new Map([...this.roadRoutes, ...nextRoutes]); this.draw(); if (queue.length) this.scheduleRoadWork(work); else { this.roadTrafficField = traffic; this.lastRoadAt = performance.now(); } }; this.roadsDirty = false; this.scheduleRoadWork(work);
  }
  adjustedRoadPoints(route, source, target) { return route.points.map((point, index) => { const fraction = route.cumulative[index] / route.total; return { x: point.x + (source.x - route.source.x) * (1 - fraction) + (target.x - route.target.x) * fraction, y: point.y + (source.y - route.source.y) * (1 - fraction) + (target.y - route.target.y) * fraction }; }); }
  sampleRoadHeight(field, x, y) { const { values, columns, rows, step } = field, gridX = Math.max(0, Math.min(columns - 1, x / step)), gridY = Math.max(0, Math.min(rows - 1, y / step)), left = Math.floor(gridX), right = Math.min(columns - 1, left + 1), top = Math.floor(gridY), bottom = Math.min(rows - 1, top + 1), horizontal = gridX - left, vertical = gridY - top, upper = values[top * columns + left] * (1 - horizontal) + values[top * columns + right] * horizontal, lower = values[bottom * columns + left] * (1 - horizontal) + values[bottom * columns + right] * horizontal; return upper * (1 - vertical) + lower * vertical; }
  sampleRoadTraffic(field, x, y) { if (!this.roadTrafficField) return 0; const { columns, rows, step } = field, column = Math.max(0, Math.min(columns - 1, Math.round(x / step))), row = Math.max(0, Math.min(rows - 1, Math.round(y / step))); return Number(this.roadTrafficField[row * columns + column] || 0); }
  rebuildRoadTrafficField(field, width, height) { const traffic = new Float32Array(field.values.length), { columns, rows, step } = field, clampCell = point => { const column = Math.max(0, Math.min(columns - 1, Math.round(point.x / step))), row = Math.max(0, Math.min(rows - 1, Math.round(point.y / step))); return row * columns + column; }; for (const route of this.roadRoutes.values()) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const points = this.adjustedRoadPoints(route, source, target), cells = new Set([clampCell(points[0])]); for (let index = 1; index < points.length; index++) { const first = points[index - 1], second = points[index], distance = Math.hypot(second.x - first.x, second.y - first.y), samples = Math.max(1, Math.ceil(distance / Math.max(2, step * .7))); for (let sample = 1; sample <= samples; sample++) { const blend = sample / samples; cells.add(clampCell({ x: first.x + (second.x - first.x) * blend, y: first.y + (second.y - first.y) * blend })); } } route.liveCells = [...cells]; this.depositRoadTraffic(field, traffic, route.liveCells, Math.max(1, Number(route.road.count || 1))); } this.roadTrafficField = traffic; for (const route of this.roadRoutes.values()) { const cells = route.liveCells || [], own = Math.max(1, Number(route.road.count || 1)), shared = cells.length ? cells.reduce((sum, cell) => sum + Math.max(0, traffic[cell] - own), 0) / cells.length : 0; route.networkWeight = shared; } }
  conformRoadRoute(route, field, source, target) { let points = this.adjustedRoadPoints(route, source, target); const offsets = [-10, -6, -3, 0, 3, 6, 10], passes = this.alpha > .03 || this.dragging ? 1 : 2; for (let pass = 0; pass < passes; pass++) { const next = [points[0]]; for (let index = 1; index < points.length - 1; index++) { const previous = points[index - 1], current = points[index], following = points[index + 1], tangentX = following.x - previous.x, tangentY = following.y - previous.y, length = Math.max(1, Math.hypot(tangentX, tangentY)), normalX = -tangentY / length, normalY = tangentX / length, midpoint = { x: (previous.x + following.x) / 2, y: (previous.y + following.y) / 2 }, origin = { x: current.x * .62 + midpoint.x * .38, y: current.y * .62 + midpoint.y * .38 }, previousHeight = this.sampleRoadHeight(field, previous.x, previous.y), followingHeight = this.sampleRoadHeight(field, following.x, following.y); let best = current, bestCost = Infinity; for (const offset of offsets) { const candidate = { x: origin.x + normalX * offset, y: origin.y + normalY * offset }, height = this.sampleRoadHeight(field, candidate.x, candidate.y), grade = (height - previousHeight) ** 2 + (followingHeight - height) ** 2, bend = ((candidate.x - midpoint.x) ** 2 + (candidate.y - midpoint.y) ** 2) / 400, movement = offset * offset / 400, sharedCorridor = Math.max(0, this.sampleRoadTraffic(field, candidate.x, candidate.y) - Math.max(1, Number(route.road.count || 1))), cost = grade * 1200 + bend * .34 + movement * .07 - Math.min(4, sharedCorridor) * .82; if (cost < bestCost) { best = candidate; bestCost = cost; } } next.push({ x: current.x * .34 + best.x * .66, y: current.y * .34 + best.y * .66 }); } next.push(points[points.length - 1]); points = next; } const cumulative = [0]; for (let index = 1; index < points.length; index++) cumulative.push(cumulative[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); route.points = points; route.cumulative = cumulative; route.total = cumulative[cumulative.length - 1] || 1; route.source = source; route.target = target; route.conformedAt = this.lastTerrainAt; return points; }
  roadPointAt(points, fraction) { const lengths = [0]; for (let index = 1; index < points.length; index++) lengths.push(lengths[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); const target = (lengths[lengths.length - 1] || 1) * fraction; for (let index = 1; index < points.length; index++) if (lengths[index] >= target) { const span = Math.max(.001, lengths[index] - lengths[index - 1]), blend = (target - lengths[index - 1]) / span, first = points[index - 1], second = points[index]; return { x: first.x + (second.x - first.x) * blend, y: first.y + (second.y - first.y) * blend, angle: Math.atan2(second.y - first.y, second.x - first.x) }; } const last = points[points.length - 1], before = points[Math.max(0, points.length - 2)]; return { ...last, angle: Math.atan2(last.y - before.y, last.x - before.x) }; }
  traceRoad(ctx, points, progress) { if (points.length < 2 || progress <= 0) return false; let total = 0; for (let index = 1; index < points.length; index++) total += Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y); const visibleLength = total * progress, visible = [points[0]]; let walked = 0; for (let index = 1; index < points.length; index++) { const first = points[index - 1], second = points[index], length = Math.hypot(second.x - first.x, second.y - first.y); if (walked + length <= visibleLength) visible.push(second); else { const blend = Math.max(0, Math.min(1, (visibleLength - walked) / Math.max(.001, length))); visible.push({ x: first.x + (second.x - first.x) * blend, y: first.y + (second.y - first.y) * blend }); break; } walked += length; } if (visible.length < 2) return false; ctx.beginPath(); ctx.moveTo(visible[0].x, visible[0].y); for (let index = 0; index < visible.length - 1; index++) { const before = visible[Math.max(0, index - 1)], current = visible[index], next = visible[index + 1], after = visible[Math.min(visible.length - 1, index + 2)], controlOne = { x: current.x + (next.x - before.x) / 6, y: current.y + (next.y - before.y) / 6 }, controlTwo = { x: next.x - (after.x - current.x) / 6, y: next.y - (after.y - current.y) / 6 }; ctx.bezierCurveTo(controlOne.x, controlOne.y, controlTwo.x, controlTwo.y, next.x, next.y); } return true; }
  paintRoads(ctx, width, height, colors) {
    if (!this.roadEdges.length || !this.lastTerrainField) return; const now = performance.now(); if (this.roadsDirty && this.roadWorkHandle === null) this.beginRoadRouteRefresh(this.lastTerrainField, width, height); if (this.roadTrafficAt !== this.lastTerrainAt || this.roadTrafficRouteCount !== this.roadRoutes.size) { this.rebuildRoadTrafficField(this.lastTerrainField, width, height); this.roadTrafficAt = this.lastTerrainAt; this.roadTrafficRouteCount = this.roadRoutes.size; } const focused = this.hovered || this.selected, reduced = matchMedia('(prefers-reduced-motion: reduce)').matches, orderedRoutes = [...this.roadRoutes].sort((first, second) => Number(first[1].networkWeight || 0) - Number(second[1].networkWeight || 0)); let animating = false;
    for (const [key, route] of orderedRoutes) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const opacity = Math.min(source.visibility, target.visibility); if (opacity < .035) continue; const points = route.conformedAt === this.lastTerrainAt ? this.adjustedRoadPoints(route, source, target) : this.conformRoadRoute(route, this.lastTerrainField, source, target), introduced = this.roadIntroducedAt.get(key) || now, progress = reduced ? 1 : Math.max(0, Math.min(1, (now - introduced) / 430)); if (progress < 1) animating = true; const active = focused === route.road.source || focused === route.road.target, trunk = Math.min(3.4, Math.sqrt(Math.max(0, Number(route.networkWeight || 0))) * .58); ctx.globalAlpha = opacity * (active ? .82 : .22 + Math.min(.18, trunk * .045)); ctx.strokeStyle = active ? colors.normal : colors.muted; ctx.lineWidth = (active ? 1.5 : .78) + trunk; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; if (this.traceRoad(ctx, points, progress)) ctx.stroke(); const chevron = (fraction, reverse = false) => { if (progress < fraction + .04) return; const point = this.roadPointAt(points, fraction), angle = point.angle + (reverse ? Math.PI : 0), size = active ? 3.8 : 3.1; ctx.lineWidth = active ? 1.4 : .8; ctx.beginPath(); ctx.moveTo(point.x - Math.cos(angle - .62) * size, point.y - Math.sin(angle - .62) * size); ctx.lineTo(point.x, point.y); ctx.lineTo(point.x - Math.cos(angle + .62) * size, point.y - Math.sin(angle + .62) * size); ctx.stroke(); }; if (route.road.forward) chevron(.62); if (route.road.reverse) chevron(.38, true); }
  ...
  destroy() { this.cancelRoadWork(); cancelAnimationFrame(this.roadAnimationFrame); cancelAnimationFrame(this.simulationFrame); cancelAnimationFrame(this.animationFrame); this.resizeObserver?.disconnect(); }
}
...
class SearchSettings extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(this.containerEl).setName('Show manual link roads').setDesc('Route wikilinks between visible notes across the semantic landscape.').addToggle(t => t.setValue(this.plugin.settings.showWikilinks).onChange(async value => { this.plugin.settings.showWikilinks = value; await this.plugin.save(); }));
