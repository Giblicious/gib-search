...
const SEARCH_LENSES = {
  relevance: { label: 'Relevance', description: 'Rank by overall relevance to the search.' },
  concepts: { label: 'Concepts', description: 'Group results into distinct conceptual facets.' },
...
function validSearchLens(value) { return SEARCH_LENSES[value] ? value : 'relevance'; }
...
class SemanticSearchModal extends SuggestModal {
  ...
  getSuggestions(query) {
    if (!query || query.trim().length < 2) { clearTimeout(this.debounceTimer); this.searchVersion++; this.lensVersion++; const changed = Boolean(this.lastQuery); this.lastQuery = ''; this.lastResults = []; this.mapResults = []; this.lensRelationships = new Map(); if (changed) { this.map?.endQuery(); window.setTimeout(() => this.updateMap(), 0); } return []; }
  ...
  async applyLens() {
    const version = ++this.lensVersion, lens = this.lens, query = this.lastQuery, source = this.mapResults.map(result => ({ ...result, lensLabel: '', lensScore: Number(result.score || 0), facet: undefined, argumentGroup: undefined, contextScore: undefined })); this.lensRelationships = new Map(); if (!query || !source.length) return;
    this.lensSelect?.toggleClass('is-working', lens !== 'relevance');
    try {
      if (lens === 'relevance') return this.commitLensResults(source, version);
      const files = source.map(result => result.file);
      if (lens === 'concepts') {
        const facets = await this.plugin.search.conceptFacets(query, files); if (version !== this.lensVersion) return; const strongest = new Map(); for (const result of source) { const facet = facets.get(result.file)?.facet ?? Number.MAX_SAFE_INTEGER; strongest.set(facet, Math.max(strongest.get(facet) || 0, Number(result.score || 0))); } const facetOrder = new Map([...strongest].sort((a, b) => b[1] - a[1]).map(([facet], index) => [facet, index])); for (const result of source) { const info = facets.get(result.file); result.facet = info?.facet; result.lensLabel = info?.label || 'Other'; } source.sort((a, b) => (facetOrder.get(a.facet ?? Number.MAX_SAFE_INTEGER) ?? 999) - (facetOrder.get(b.facet ?? Number.MAX_SAFE_INTEGER) ?? 999) || b.score - a.score); return this.commitLensResults(source, version);
      }
      if (lens === 'context') {
      ...
      const anchor = source[0]; for (const result of source) { result.argumentGroup = result === anchor ? 'anchor' : 'related'; result.lensLabel = result === anchor ? 'Anchor' : 'Related'; } this.commitLensResults(source, version); if (!this.plugin.settings.graphRelationshipIntelligence || source.length < 2) return;
      await new Promise(resolve => window.setTimeout(resolve, 180)); if (version !== this.lensVersion) return; const candidates = source.slice(1, Math.min(source.length, Math.max(this.visibleLimit || 10, 10))), edges = candidates.map(result => ({ source: anchor.file, target: result.file, affinity: Number(result.score || 0), score: Number(result.semanticScore || 0) })), budget = this.plugin.isMobile ? this.plugin.settings.graphRelationshipBudgetMobile : this.plugin.settings.graphRelationshipBudgetDesktop; this.map?.setIntelligenceStatus('Reading arguments…'); const details = await this.plugin.search.graphRelationships(edges, budget); if (version !== this.lensVersion) return; this.lensRelationships = details; const order = { anchor: 0, support: 1, tension: 2, related: 3 }; for (const result of source) { if (result === anchor) continue; const relation = details.get([anchor.file, result.file].sort().join('\0')); result.argumentGroup = relation?.type === 'support' ? 'support' : relation?.type === 'contrast' ? 'tension' : 'related'; result.lensLabel = result.argumentGroup === 'support' ? 'Support' : result.argumentGroup === 'tension' ? 'Tension' : 'Related'; result.relationConfidence = Number(relation?.confidence || 0); } source.sort((a, b) => order[a.argumentGroup] - order[b.argumentGroup] || Number(b.relationConfidence || 0) - Number(a.relationConfidence || 0) || b.score - a.score); this.commitLensResults(source, version); this.map?.setIntelligenceStatus('');
  ...
  async updateMap() {
    ...
    const graphNodes = graph.nodes.map(node => { const result = byFile.get(node.id), generation = generationByFile.get(node.id), semanticRelevance = (Number(node.semanticScore || 0) - semanticLow) / semanticSpread, rankedRelevance = result ? (Number(result.lensScore ?? result.score ?? 0) - rankingLow) / rankingSpread : 0, expandedRelevance = generation && generation.generation > 1 ? Math.max(.22, Math.min(.62, Number(generation.relationScore || 0))) : semanticRelevance; return { ...node, generation: generation?.generation || 1, matched: Boolean(generation), relevance: query ? (result ? .08 + rankedRelevance * .92 : expandedRelevance) : semanticRelevance, fileScale: fileScale.get(node.id) ?? .35, facet: result?.facet, argumentGroup: result?.argumentGroup, contextScore: result?.contextScore }; }), layoutOptions = { magic: this.plugin.settings.magicGraphEnabled, lens: this.lens }, initialLayout = await this.plugin.search.multiRelationalLayout(query, graphNodes, this.lens === 'arguments' ? this.lensRelationships : new Map(), layoutOptions); if (version !== this.mapVersion) return; for (const node of graphNodes) { const target = initialLayout.get(node.id); if (target) { node.layoutX = target.x; node.layoutY = target.y; } }
