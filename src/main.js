...
const MAP_TUNING_CONTROLS = [
  ...
  { section: 'Terrain', key: 'terrainSpread', label: 'Terrain spread', min: .5, max: 2, step: .05 },
...
function validMapGrouping(value) { return value === 'topics' ? 'topics' : 'similarity'; }
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setupMapModeControl() { this.mapModeControl = this.headingEl.createDiv({ cls: 'gib-search-map-mode', attr: { 'aria-label': 'Map grouping mode' } }); this.headingEl.insertBefore(this.mapModeControl, this.statusEl); this.mapModeButtons = [['similarity', 'Similarity'], ['topics', 'Topics']].map(([value, label]) => { const button = this.mapModeControl.createEl('button', { text: label, attr: { type: 'button', title: value === 'topics' ? 'Group notes by dominant passage topics' : 'Arrange notes by continuous semantic similarity' } }); button.addEventListener('mousedown', event => event.preventDefault()); button.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); this.setMapGroupingMode(value); this.options.onMapMode(value); }); return button; }); this.setMapGroupingMode(this.mapGroupingMode); }
  setMapGroupingMode(value) { this.mapGroupingMode = validMapGrouping(value); this.mapModeButtons?.forEach((button, index) => { const active = ['similarity', 'topics'][index] === this.mapGroupingMode; button.toggleClass('is-active', active); button.setAttribute('aria-pressed', String(active)); }); }
  setupTuningPanel() {
  ...
  setAnalysisPending(value) { this.analysisPending = Boolean(value); if (!this.analysisPending) { this.lastTerrainAt = 0; this.draw(); } }
  ...
  semanticDensityField(width, height) {
    const tuning = this.tuning || MAP_TUNING_DEFAULTS, step = 5, columns = Math.max(2, Math.ceil(width / step) + 1), rows = Math.max(2, Math.ceil(height / step) + 1), values = new Float32Array(columns * rows), heightWeights = new Float32Array(values.length), heightTotals = new Float32Array(values.length), visible = this.nodes.filter(node => node.visibility > .04 && (!this.hasQuery || this.pendingQuery || node.matched)), canonicalTerrain = this.hasQuery && !this.pendingQuery, points = new Map(visible.map(node => { const position = canonicalTerrain ? { x: node.layoutX, y: node.layoutY } : node, [x, y] = this.coordinates(position, width, height); return [node.id, { node, x, y }]; })), vaultTerrain = !this.hasQuery && !this.pendingQuery, zoom = Math.max(.35, this.cameraZoom * this.userZoom), vaultDensityScale = this.hasQuery ? 1 : Math.max(.48, Math.min(1, Math.sqrt(48 / Math.max(1, visible.length)))), sigma = Math.max(this.hasQuery ? 9 : 11, 27 * zoom * vaultDensityScale) * (vaultTerrain ? Number(tuning.terrainSpread || 1) : 1), anchors = [...points.values()].map(point => ({ x: point.x, y: point.y })), gaussian = ratio => this.gaussianLookup[Math.max(0, Math.min(1024, Math.round(ratio / 9 * 1024)))];
    ...
    if (this.hasQuery && this.queryPresence > .04) { const centerPosition = canonicalTerrain ? { x: this.queryNode.layoutX, y: this.queryNode.layoutY } : this.queryNode, [centerX, centerY] = this.coordinates(centerPosition, width, height), center = { x: centerX, y: centerY }, directResults = [...points.values()].filter(({ node }) => node.matched && node.generation === 1).sort((first, second) => second.node.relevance - first.node.relevance).slice(0, 5); for (const target of directResults) addRidge(center, target, sigma * .46, this.queryPresence * target.node.visibility * (.07 + Math.max(0, Math.min(1, target.node.relevance)) * .11)); const supportMaximum = Math.max(0, ...values); addMass(centerX, centerY, sigma * .56, (supportMaximum * 1.18 + .42) * this.queryPresence); anchors.push(center); }
  ...
  destroy() { cancelAnimationFrame(this.roadAnimationFrame); cancelAnimationFrame(this.simulationFrame); cancelAnimationFrame(this.animationFrame); this.resizeObserver?.disconnect(); }
}
...
class SemanticSearchModal extends SuggestModal {
  ...
  getSuggestions(query) {
    if (!query || query.trim().length < 2) { clearTimeout(this.debounceTimer); this.searchVersion++; this.lensVersion++; const changed = Boolean(this.lastQuery); this.lastQuery = ''; this.lastResults = []; this.mapResults = []; this.lensRelationships = new Map(); this.lensRelationshipEdges = []; this.map?.setAnalysisPending(false); if (changed) { this.map?.endQuery(); window.setTimeout(() => this.updateMap(), 0); } return []; }
    const trimmed = query.trim();
    if (trimmed !== this.lastQuery) { const entering = !this.lastQuery; this.lastQuery = trimmed; this.map?.setAnalysisPending(true); if (entering) this.map?.beginQuery(trimmed); this.visibleLimit = activeTweaks(this.plugin).topK; this.triggerSearch(trimmed); }
  ...
  triggerSearch(query, immediate = false) {
    ...
    this.debounceTimer = setTimeout(async () => {
      ...
      } catch (error) { if (version === this.searchVersion) this.map?.setAnalysisPending(false); if (error?.name !== 'AbortError') this.plugin.reportOnce(error.message); }
    }, immediate ? 0 : 75);
  }
  ...
  async updateMap() {
    ...
    this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, model.nodes, model.edges, model.roads); this.map.setAnalysisPending(false);
  }
...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.results = []; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.mapGroupingMode = validMapGrouping(plugin.settings.mapGroupingMode); this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1));
  }
  ...
  async onOpen() {
    ...
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      ...
      onTune: (values, key) => { this.plugin.settings.mapTuning = values; window.clearTimeout(this.tuneTimer); this.tuneTimer = window.setTimeout(async () => { if (['commonnessSuppression', 'passageCoverage', 'passageDiversity', 'communitySensitivity', 'communityMinSize', 'communityLabelSensitivity', 'reset'].includes(key)) { this.plugin.search.contentProfileCache = null; this.plugin.search.starfieldCache = null; } await this.plugin.save(); this.query.length >= 3 ? this.runSearch(this.query, true) : this.loadVault(); }, 180); },
  ...
  async loadVault() {
    ...
    try {
      const files = this.indexableFiles(), paths = files.map(file => file.path), scales = vaultFileScales(this.app), graph = await this.plugin.search.semanticStarfield('', paths); if (version !== this.loadVersion || this.query.length >= 3) return;
  ...
  handleQuery(value) {
    ...
    if (query.length < 3) { this.searchVersion++; this.results = []; this.map.setAnalysisPending(false); this.resultsPanel.hide(); this.reopenButton.hide(); this.contentEl.addClass('is-results-collapsed'); if (this.baseNodes.length) { this.map.setTitle(`${this.mapGroupingMode === 'topics' ? 'Topics' : 'Similarity'} · vault`); this.map.setGraph({ label: 'Search', hasQuery: false, resultCount: 0 }, this.baseNodes, this.baseEdges, this.baseRoads); } else this.loadVault(); return; }
    this.map.setAnalysisPending(true); this.map.beginQuery(query, true); this.map.setTitle(`Searching “${query}”…`); this.setResultsCollapsed(false); this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: 'Searching…' }); this.resultStatus.textContent = ''; this.searchTimer = window.setTimeout(() => this.runSearch(query), 75);
  }
  async runSearch(query, immediate = false) {
    if (String(query).trim().length < 3) return; this.map.setAnalysisPending(true); if (!immediate) this.map.beginQuery(query, true); const version = ++this.searchVersion, tweaks = activeTweaks(this.plugin), limit = Math.max(30, tweaks.topK);
    try {
      ...
      this.results = results; this.renderResults(); this.map.setTitle(`${results.length} results · ${this.mapGroupingMode === 'topics' ? 'Topics' : 'Similarity'}`); this.map.setGraph({ label: query, hasQuery: true, resultCount: results.length }, merged, model.edges, model.roads); this.map.setAnalysisPending(false);
    } catch (error) { if (version === this.searchVersion) { this.map.setAnalysisPending(false); this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: error.message }); this.map.setTitle(error.message); } }
  }
  renderResults() {
  ...
  focusResult(file) { const item = [...this.resultList.querySelectorAll('.gib-graph-result')].find(value => value.dataset.gibFile === file); if (item) { this.setResultsCollapsed(false); item.scrollIntoView({ block: 'nearest' }); } }
  async openFile(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (file instanceof TFile) await this.app.workspace.getLeaf(false).openFile(file); }
