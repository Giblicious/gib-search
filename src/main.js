...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  routeRoad(field, source, target) {
    ...
    while (heap.length && expansions++ < 5200) { const current = pop(), index = current.index; if (index === goal) { reached = true; break; } const row = Math.floor(index / columns), column = index - row * columns, currentScore = scores[index]; for (const [dx, dy, move] of directions) { const nextColumn = column + dx, nextRow = row + dy; if (nextColumn < minColumn || nextColumn > maxColumn || nextRow < minRow || nextRow > maxRow) continue; const next = nextRow * columns + nextColumn, endpointDistance = Math.min(Math.hypot(nextColumn - startColumn, nextRow - startRow), Math.hypot(nextColumn - goalColumn, nextRow - goalRow)), gradeWeight = Math.min(1, endpointDistance / 4), grade = Math.abs(values[next] - values[index]), elevation = Math.max(0, values[next] - .28), tentative = currentScore + move + grade * 17 * gradeWeight + elevation * .07; if (tentative >= scores[next]) continue; scores[next] = tentative; previous[next] = index; const heuristic = Math.hypot(goalColumn - nextColumn, goalRow - nextRow) * .9; push({ index: next, priority: tentative + heuristic }); } }
    if (!reached) return [source, target]; const raw = []; for (let index = goal; index !== -1; index = previous[index]) { const row = Math.floor(index / columns), column = index - row * columns; raw.push({ x: column * step, y: row * step }); if (index === start) break; } raw.reverse(); raw[0] = { x: source.x, y: source.y }; raw[raw.length - 1] = { x: target.x, y: target.y }; let smooth = raw; for (let pass = 0; pass < 2; pass++) { const next = [smooth[0]]; for (let index = 0; index < smooth.length - 1; index++) { const first = smooth[index], second = smooth[index + 1]; next.push({ x: first.x * .75 + second.x * .25, y: first.y * .75 + second.y * .25 }, { x: first.x * .25 + second.x * .75, y: first.y * .25 + second.y * .75 }); } next.push(smooth[smooth.length - 1]); smooth = next; } return smooth;
  }
  refreshRoadRoutes(field, width, height) {
    const routes = new Map(); for (const road of this.roadEdges.slice(0, 72)) { const key = road.key || mapEdgeKey(road.source, road.target), source = this.roadEndpoint(road.source, width, height), target = this.roadEndpoint(road.target, width, height); if (!source || !target) continue; const points = this.routeRoad(field, source, target), cumulative = [0]; for (let index = 1; index < points.length; index++) cumulative.push(cumulative[index - 1] + Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y)); routes.set(key, { road, points, cumulative, total: cumulative[cumulative.length - 1] || 1, source, target }); } this.roadRoutes = routes; this.roadsDirty = false; this.lastRoadAt = performance.now();
  }
  ...
  paintRoads(ctx, width, height, colors) {
    if (!this.roadEdges.length || !this.lastTerrainField) return; const now = performance.now(), moving = this.alpha > .025 || this.dragging || this.panning, drift = this.roadRouteDrift(width, height), shouldRefresh = this.roadsDirty || (moving ? drift > 18 && now - this.lastRoadAt >= 220 : drift > 3); if (shouldRefresh) this.refreshRoadRoutes(this.lastTerrainField, width, height); const focused = this.hovered || this.selected, reduced = matchMedia('(prefers-reduced-motion: reduce)').matches; let animating = false;
    for (const [key, route] of this.roadRoutes) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const opacity = Math.min(source.visibility, target.visibility); if (opacity < .035) continue; const points = this.adjustedRoadPoints(route, source, target), introduced = this.roadIntroducedAt.get(key) || now, progress = reduced ? 1 : Math.max(0, Math.min(1, (now - introduced) / 430)); if (progress < 1) animating = true; const active = focused === route.road.source || focused === route.road.target, total = points.slice(1).reduce((sum, point, index) => sum + Math.hypot(point.x - points[index].x, point.y - points[index].y), 0), visibleLength = total * progress; ctx.globalAlpha = opacity * (active ? .78 : .28); ctx.strokeStyle = active ? colors.normal : colors.muted; ctx.lineWidth = active ? 1.45 : .85; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.beginPath(); ctx.moveTo(points[0].x, points[0].y); let walked = 0; for (let index = 1; index < points.length; index++) { const first = points[index - 1], second = points[index], length = Math.hypot(second.x - first.x, second.y - first.y); if (walked + length <= visibleLength) ctx.lineTo(second.x, second.y); else { const blend = Math.max(0, Math.min(1, (visibleLength - walked) / Math.max(.001, length))); ctx.lineTo(first.x + (second.x - first.x) * blend, first.y + (second.y - first.y) * blend); break; } walked += length; } ctx.stroke(); const chevron = (fraction, reverse = false) => { if (progress < fraction + .04) return; const point = this.roadPointAt(points, fraction), angle = point.angle + (reverse ? Math.PI : 0), size = active ? 3.8 : 3.1; ctx.beginPath(); ctx.moveTo(point.x - Math.cos(angle - .62) * size, point.y - Math.sin(angle - .62) * size); ctx.lineTo(point.x, point.y); ctx.lineTo(point.x - Math.cos(angle + .62) * size, point.y - Math.sin(angle + .62) * size); ctx.stroke(); }; if (route.road.forward) chevron(.62); if (route.road.reverse) chevron(.38, true); }
    ctx.globalAlpha = 1; if (animating) this.startSimulation(.16);
  }
