...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  roadRouteDrift(width, height) { let drift = 0; for (const route of this.roadRoutes.values()) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; drift = Math.max(drift, Math.hypot(source.x - route.source.x, source.y - route.source.y), Math.hypot(target.x - route.target.x, target.y - route.target.y)); } return drift; }
  ...
  traceRoad(ctx, points, progress) { if (points.length < 2 || progress <= 0) return false; let total = 0; for (let index = 1; index < points.length; index++) total += Math.hypot(points[index].x - points[index - 1].x, points[index].y - points[index - 1].y); const visibleLength = total * progress; ctx.beginPath(); ctx.moveTo(points[0].x, points[0].y); let walked = 0; for (let index = 1; index < points.length; index++) { const first = points[index - 1], second = points[index], length = Math.hypot(second.x - first.x, second.y - first.y); if (walked + length <= visibleLength) ctx.lineTo(second.x, second.y); else { const blend = Math.max(0, Math.min(1, (visibleLength - walked) / Math.max(.001, length))); ctx.lineTo(first.x + (second.x - first.x) * blend, first.y + (second.y - first.y) * blend); break; } walked += length; } return true; }
  paintRoads(ctx, width, height, colors) {
    if (!this.roadEdges.length || !this.lastTerrainField) return; const now = performance.now(), moving = this.alpha > .025 || this.dragging || this.panning, drift = this.roadRouteDrift(width, height); if (this.roadsDirty && this.roadWorkHandle === null) this.beginRoadRouteRefresh(this.lastTerrainField, width, height); else if (!moving && this.roadWorkHandle === null && drift > 10 && now - this.lastRoadAt >= 500) this.beginRoadRouteRefresh(this.lastTerrainField, width, height); const focused = this.hovered || this.selected, reduced = matchMedia('(prefers-reduced-motion: reduce)').matches; let animating = false;
    for (const [key, route] of this.roadRoutes) { const source = this.roadEndpoint(route.road.source, width, height), target = this.roadEndpoint(route.road.target, width, height); if (!source || !target) continue; const opacity = Math.min(source.visibility, target.visibility); if (opacity < .035) continue; const points = this.adjustedRoadPoints(route, source, target), introduced = this.roadIntroducedAt.get(key) || now, progress = reduced ? 1 : Math.max(0, Math.min(1, (now - introduced) / 430)); if (progress < 1) animating = true; const active = focused === route.road.source || focused === route.road.target; ctx.globalAlpha = opacity * (active ? .78 : .28); ctx.strokeStyle = active ? colors.normal : colors.muted; ctx.lineWidth = active ? 1.45 : .85; ctx.lineCap = 'round'; ctx.lineJoin = 'round'; if (this.traceRoad(ctx, points, progress)) ctx.stroke(); const chevron = (fraction, reverse = false) => { if (progress < fraction + .04) return; const point = this.roadPointAt(points, fraction), angle = point.angle + (reverse ? Math.PI : 0), size = active ? 3.8 : 3.1; ctx.beginPath(); ctx.moveTo(point.x - Math.cos(angle - .62) * size, point.y - Math.sin(angle - .62) * size); ctx.lineTo(point.x, point.y); ctx.lineTo(point.x - Math.cos(angle + .62) * size, point.y - Math.sin(angle + .62) * size); ctx.stroke(); }; if (route.road.forward) chevron(.62); if (route.road.reverse) chevron(.38, true); }
