...
function buildSemanticEnvelopes(query, nodes, edges, size = 190) {
  const bodies = [query, ...nodes]; if (!bodies.length) return null; const padding = .105, extent = Math.min(1.12, Math.max(.36, ...bodies.map(body => Math.max(Math.abs(body.x), Math.abs(body.y)) + padding + .045))), bands = [];
  for (let band = 0; band < 6; band++) {
    const threshold = band / 6, points = [query, ...nodes.filter(node => node.relevance >= threshold)], nearest = points.map((point, index) => Math.min(...points.filter((_, other) => other !== index).map(other => Math.hypot(point.x - other.x, point.y - other.y)))), typicalSpacing = median(nearest.filter(Number.isFinite)), wrapReach = Math.max(padding * 2.35, Math.min(.58, typicalSpacing * 2.15 || padding * 2.35));
    const triangles = triangulateTerrain(points).filter(triangle => { const lengths = [Math.hypot(triangle[0].x - triangle[1].x, triangle[0].y - triangle[1].y), Math.hypot(triangle[1].x - triangle[2].x, triangle[1].y - triangle[2].y), Math.hypot(triangle[2].x - triangle[0].x, triangle[2].y - triangle[0].y)]; return Math.max(...lengths) <= wrapReach; }), field = new Float32Array(size * size);
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  startSimulation(alpha = .7) { this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = time => { this.simulationFrame = null; if (!matchMedia('(prefers-reduced-motion: reduce)').matches) this.physicsStep(this.alpha); this.alpha *= .985; if (!this.terrain || time - this.lastTerrainAt > 105) { this.terrain = buildSemanticEnvelopes(this.queryNode, this.nodes, this.activeEdges, this.alpha < .08 ? 190 : 105); this.lastTerrainAt = time; } this.draw(); if (this.alpha > .012 || this.dragging) this.simulationFrame = requestAnimationFrame(tick); else { this.terrain = buildSemanticEnvelopes(this.queryNode, this.nodes, this.activeEdges, 220); this.draw(); } }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  drawContours(ctx, width, height, colors) {
    if (!this.terrain) return; const { size, bands, extent } = this.terrain, scale = Math.max(20, Math.min(width, height) / 2 - 68), toCanvas = (column, row) => [width / 2 + ((column / (size - 1) * 2 - 1) * extent) * scale, height / 2 + ((row / (size - 1) * 2 - 1) * extent) * scale]; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    for (let band = 0; band < bands.length; band++) { const outer = band === 0, paths = stitchContourSegments(contourSegments(bands[band], size)); ctx.beginPath(); for (const path of paths) drawSmoothContour(ctx, path, toCanvas); ctx.globalAlpha = outer ? .18 : .23 + band * .021; ctx.strokeStyle = outer ? colors.muted : colors.normal; ctx.lineWidth = outer ? .72 : .78 + band * .045; ctx.stroke(); } ctx.globalAlpha = 1;
  }
  draw() {
