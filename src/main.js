...
async function buildQueryMapModel(plugin, query, results, options = {}) {
  ...
  const layout = await plugin.search.multiRelationalLayout(query, nodes, lens === 'arguments' ? relationships : new Map(), { magic: plugin.settings.magicGraphEnabled, lens, mapMode: validMapGrouping(options.mapMode) });
...
class GraphView extends ItemView {
  ...
  async runSearch(query, immediate = false) {
    ...
    try {
      ...
      const results = groupSearchResults(raw, query, limit), facets = await this.plugin.search.conceptFacets(query, results.map(result => result.file)); for (const result of results) { const facet = facets.get(result.file); result.facet = facet?.facet; result.conceptAffinities = facet?.affinities; } const model = await buildQueryMapModel(this.plugin, query, results, { lens: 'relevance', generations: this.mapGenerations, mapMode: this.mapGroupingMode, fileScales: new Map(this.baseNodes.map(node => [node.id, node.fileScale])) }); if (version !== this.searchVersion || query !== this.query) return; const queryById = new Map(model.nodes.map(node => [node.id, node])), merged = this.baseNodes.map(base => queryById.get(base.id) || { ...base, matched: false, relevance: 0 });
