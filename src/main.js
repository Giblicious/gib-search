const { Plugin, PluginSettingTab, Setting, SuggestModal, ItemView, Notice, TFile, MarkdownRenderer, setIcon, Platform } = require('obsidian');
...
class LivingSemanticMapCanvas extends SemanticMapCanvas {
  ...
  setUserZoom(value, x = null, y = null) { const rect = this.canvas.getBoundingClientRect(), previous = this.userZoom, next = Math.max(.45, Math.min(4.5, Number(value) || 1)); if (!rect.width || !rect.height || Math.abs(next - previous) < .0001) return; const relativeX = (x ?? rect.width / 2) - rect.width / 2, relativeY = (y ?? rect.height / 2) - rect.height / 2, factor = next / previous; this.panX = relativeX - (relativeX - this.panX) * factor; this.panY = relativeY - (relativeY - this.panY) * factor; this.userZoom = next; this.lastTerrainAt = 0; this.lastCommunityAt = 0; this.draw(); }
  zoomBy(factor) { this.setUserZoom(this.userZoom * factor); }
  resetViewport() { this.userZoom = 1; this.panX = 0; this.panY = 0; this.lastTerrainAt = 0; this.lastCommunityAt = 0; this.draw(); }
  ...
  startSimulation(alpha = .7) { const startingLinkLayout = this.mapGroupingMode === 'links' && alpha >= .38; if (startingLinkLayout) { this.linkStableFrames = 0; this.linkSimulationTicks = 0; this.linkIdleAnchored = false; } this.alpha = Math.max(this.alpha, alpha); if (this.simulationFrame) return; const tick = now => { this.simulationFrame = null; if (!this.canvas.isConnected) return; const reducedMotion = matchMedia('(prefers-reduced-motion: reduce)').matches, activePhysics = this.alpha > .012 || this.dragging || this.pendingQuery; if (!reducedMotion) { if (activePhysics) this.physicsStep(Math.max(this.alpha, .025)); else this.idlePhysicsStep(now); } else { this.queryPresence = this.targetQueryPresence; this.cameraZoom = this.hasQuery ? 1.35 : 1; this.queryNode.x = this.queryNode.layoutX; this.queryNode.y = this.queryNode.layoutY; for (const node of this.nodes) { node.relevance = node.targetRelevance; node.visibility = node.targetVisibility; node.blur = node.targetBlur; node.accent = node.targetAccent; node.x = node.layoutX; node.y = node.layoutY; } } this.alpha = Math.max(0, this.alpha * (this.mapGroupingMode === 'links' ? .992 : .984) - .00012); this.ambientDrawing = !activePhysics; this.draw(); this.ambientDrawing = false; if (!reducedMotion && !document.hidden) this.simulationFrame = requestAnimationFrame(tick); }; this.simulationFrame = requestAnimationFrame(tick); }
  ...
  baseCoordinates(node, width, height) { const point = node.isQuery ? { x: Number(node.x), y: Number(node.y) } : this.ambientPosition(node.x, node.y, false, node.id), scale = Math.max(20, Math.min(width, height) / 2 - 68) * this.cameraZoom * this.userZoom; return [width / 2 + (point.x - this.cameraX) * scale + this.panX, height / 2 + (point.y - this.cameraY) * scale + this.panY]; }
  ...
  paintTopicCommunities(ctx, width, height, colors) {
    ...
    if (refresh) { if (!this.communityCanvas) this.communityCanvas = document.createElement('canvas'); const canvas = this.communityCanvas; canvas.width = Math.ceil(width * dpr); canvas.height = Math.ceil(height * dpr); const communityContext = canvas.getContext('2d'); communityContext.setTransform(dpr, 0, 0, dpr, 0, 0); communityContext.clearRect(0, 0, width, height); const groups = this.topicCommunityGroups(width, height), tuning = this.tuning || MAP_TUNING_DEFAULTS, padding = 17 * Math.max(.5, Math.min(2, Number(tuning.boundaryPadding || 1))), labels = [];
      for (const group of groups) { const color = this.options.semanticColors !== false ? `hsl(${group.hue} 58% ${group.lightness}%)` : colors.muted, regions = this.hasQuery ? [group.values] : this.communityCoreIslands(group.values, padding); for (const region of regions) { const field = this.communityBoundaryField(region, padding, this.hasQuery); communityContext.save(); communityContext.translate(field.left, field.top); this.traceDensityLevel(communityContext, field, field.level); communityContext.strokeStyle = color; communityContext.globalAlpha = .34; communityContext.lineWidth = 1.1; communityContext.lineCap = 'round'; communityContext.lineJoin = 'round'; communityContext.stroke(); communityContext.restore(); const weight = region.reduce((sum, value) => sum + Math.max(.01, Number(value.node.communityMembership || 0)), 0), x = region.reduce((sum, value) => sum + value.x * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight, y = region.reduce((sum, value) => sum + value.y * Math.max(.01, Number(value.node.communityMembership || 0)), 0) / weight; labels.push({ x, y, text: group.label, color, confidence: group.confidence }); } }
  ...
  drawCommunityLabels(ctx, colors) { for (const label of this.communityLabelsForDraw || []) { const { x, y } = label; ctx.font = '600 9px -apple-system, BlinkMacSystemFont, sans-serif'; const text = String(label.text).toUpperCase(), width = ctx.measureText(text).width; ctx.globalAlpha = .88; ctx.fillStyle = colors.background; ctx.fillRect(x - width / 2 - 5, y - 8, width + 10, 15); ctx.globalAlpha = .72; ctx.fillStyle = label.color; ctx.textAlign = 'center'; ctx.fillText(text, x, y + 3); } ctx.globalAlpha = 1; ctx.textAlign = 'left'; }
  ...
  draw() {
    ...
    const labels = [], gpuPoints = [], ordered = this.hasQuery && !this.pendingQuery ? [...this.nodes.filter(node => !node.matched), ...this.nodes.filter(node => node.matched)] : this.nodes; for (const node of ordered) { if (node.visibility < .012) continue; const [x, y] = this.coordinates(node, rect.width, rect.height), active = node.id === focused, relationship = focused && !queryFocused ? this.relationships.get(mapEdgeKey(node.id, focused)) : null, related = Number(relationship?.overall || 0), generationScale = node.generation === 1 ? 1 : node.generation === 2 ? .86 : .74, radius = Math.max(1.1, (2 + node.fileScale * 4.4 + (active ? 1.5 : 0)) * generationScale), focusAlpha = !focused ? 1 : active ? 1 : queryFocused ? .38 + node.relevance * .62 : .18 + related * .76, opacity = Math.max(.01, node.visibility * focusAlpha), blur = Math.max(0, Math.min(1, Number(node.blur || 0))), hue = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicHue) ? node.displayTopicHue : node.topicHue, lightness = this.mapGroupingMode === 'topics' && Number.isFinite(node.displayTopicLightness) ? node.displayTopicLightness : 64, semanticColor = node.matched || !this.hasQuery || this.pendingQuery ? this.options.semanticColors !== false && Number.isFinite(hue) ? `hsl(${hue} 58% ${lightness}%)` : colors.normal : colors.faint, alpha = opacity * (1 - blur * .28); if (this.webglRenderer) { if (blur > .04) gpuPoints.push({ x, y, size: (radius + 2.2 * blur) * 2, color: mapColorRgba(this, semanticColor, opacity * blur * .22) }); gpuPoints.push({ x, y, size: radius * 2, color: mapColorRgba(this, semanticColor, alpha) }); } else { ctx.fillStyle = semanticColor; if (blur > .04) { ctx.globalAlpha = opacity * blur * .22; ctx.beginPath(); ctx.arc(x, y, radius + 2.2 * blur, 0, Math.PI * 2); ctx.fill(); } ctx.globalAlpha = alpha; ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill(); } if (labelNodes.has(node.id) && (node.matched || !this.hasQuery || this.pendingQuery)) labels.push({ node, x, y, radius, active, opacity }); if (node.matched || !this.hasQuery || this.pendingQuery || this.options.preserveBackground) this.hit.push({ node, x, y, radius: radius + 9 }); }
...
class GraphView extends ItemView {
  constructor(leaf, plugin) {
    super(leaf); this.plugin = plugin; this.query = ''; this.searchVersion = 0; this.loadVersion = 0; this.previewVersion = 0; this.previewFilePath = null; this.results = []; this.baseNodes = []; this.baseEdges = []; this.baseRoads = []; this.baseScene = null; this.atlasState = plugin.atlas.state(); this.atlasScope = plugin.atlas.resolveScope(this.atlasState); this.mapGenerations = Math.max(1, Math.min(3, Number(plugin.settings.searchMapGenerations) || 1));
  }
  ...
  async onOpen() {
    ...
    this.resultsPanel = this.contentEl.createDiv({ cls: 'gib-graph-results-panel' }); const resultHeader = this.resultsPanel.createDiv({ cls: 'gib-graph-results-header' }); this.resultTitle = resultHeader.createSpan({ text: 'Results' }); this.resultStatus = resultHeader.createSpan({ cls: 'gib-graph-results-status' }); this.resultList = this.resultsPanel.createDiv({ cls: 'gib-graph-results-list' }); this.resultsPanel.hide();
    this.previewPanel = this.contentEl.createDiv({ cls: 'gib-graph-preview-panel' }); const previewHeader = this.previewPanel.createDiv({ cls: 'gib-graph-preview-header' }), previewHeading = previewHeader.createDiv({ cls: 'gib-graph-preview-heading' }), previewIcon = previewHeading.createSpan({ cls: 'gib-graph-preview-icon' }); setIcon(previewIcon, 'sticky-note'); const previewNames = previewHeading.createDiv({ cls: 'gib-graph-preview-names' }); this.previewTitle = previewNames.createDiv({ cls: 'gib-graph-preview-title' }); this.previewPath = previewNames.createDiv({ cls: 'gib-graph-preview-path' }); this.previewBody = this.previewPanel.createDiv({ cls: 'gib-graph-preview-body markdown-rendered' }); this.previewPanel.hide();
    this.map = new LivingSemanticMapCanvas(mapHost, this.app, {
      ...
      onHover: file => { this.hoverResult(file); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), hover: file }); }, onSelect: file => { if (file) this.showPreview(file); else this.closePreview(); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), selection: file, hover: null }); }, onOpen: file => this.openFile(file)
    });
    this.input.addEventListener('input', () => this.handleQuery(this.input.value)); this.input.addEventListener('keydown', event => { if (event.key !== 'Escape') return; if (this.previewFilePath) this.closePreview(); else if (this.input.value) { this.input.value = ''; this.handleQuery(''); } });
    this.graphCacheOff = this.plugin.search.onGraphCache(() => { this.plugin.atlas.clear(); if (this.query.length < 3) this.loadVault(); });
    await this.loadVault(); this.input.disabled = false; this.input.placeholder = 'Search the vault by meaning…'; this.input.focus(); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), selection: null, hover: null });
  }
  ...
  handleQuery(value) {
    const query = String(value || '').trim(), version = ++this.searchVersion; this.query = query; window.clearTimeout(this.searchTimer); this.plugin.search.cancelLiveSearch?.(); this.plugin.atlas.publishContext({ surface: 'atlas', state: this.currentAtlasState(), selection: null, hover: null });
    if (query.length < 3) { this.results = []; this.resultsPanel.hide(); this.closePreview(); if (this.baseScene) this.map.setGraph(this.baseScene.center, this.baseScene.nodes, this.baseScene.edges, this.baseScene.roads); else this.loadVault(); return; }
    this.map.beginQuery(query, true); this.resultsPanel.show(); if (!this.results.length) { this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: 'Searching…' }); this.resultStatus.textContent = ''; } this.searchTimer = window.setTimeout(() => this.runSearch(query, false, version), 130);
  }
  async runSearch(query, immediate = false, requestedVersion = null) {
    ...
    } catch (error) { if (version === this.searchVersion) { this.resultList.empty(); this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: error.message }); this.map.setTitle(error.message); } }
  }
  renderResults() {
    this.resultList.empty(); this.resultStatus.textContent = `${this.results.length}`; if (!this.results.length) { this.resultList.createDiv({ cls: 'gib-graph-results-empty', text: 'No results' }); return; }
    for (const result of this.results) { const item = this.resultList.createDiv({ cls: 'gib-graph-result', attr: { tabindex: '0' } }); item.dataset.gibFile = result.file; item.addEventListener('pointerenter', () => this.map.setHover(result.file)); item.addEventListener('pointerleave', () => this.map.setHover(null)); item.addEventListener('click', () => this.openFile(result.file)); const pathParts = result.file.replace(/\.md$/i, '').split('/'), fileName = pathParts.pop() || result.file; const meta = item.createDiv({ cls: 'gib-semantic-result-meta' }), folder = meta.createDiv({ cls: 'gib-semantic-result-folder' }); (pathParts.length ? pathParts : ['Vault']).forEach((part, index) => { if (index) folder.createSpan({ cls: 'gib-semantic-result-folder-separator', text: '/' }); folder.createSpan({ text: part }); }); const header = item.createDiv({ cls: 'gib-semantic-result-header' }), icon = header.createSpan({ cls: 'gib-semantic-result-icon' }); setIcon(icon, 'sticky-note'); const title = header.createSpan({ cls: 'gib-semantic-result-file' }); renderHighlighted(title, fileName, this.query, result.filenameHighlights); header.createSpan({ cls: 'gib-semantic-result-score', text: `${Math.round(Number(result.score || 0) * 100)}%` }); const snippet = result.snippets?.[0]; if (snippet) { const block = item.createDiv({ cls: 'gib-semantic-snippet' }); if (snippet.heading) { const heading = block.createDiv({ cls: 'gib-semantic-result-heading' }); renderHighlighted(heading, snippet.heading, this.query, snippet.headingHighlights); } const preview = block.createDiv({ cls: 'gib-semantic-result-preview' }); renderHighlighted(preview, snippet.text, this.query, snippet.semanticHighlights); } }
    this.resultsPanel.show();
  }
  hoverResult(file) { for (const item of this.resultList.querySelectorAll('.is-map-hovered')) item.removeClass('is-map-hovered'); if (!file) return; const item = [...this.resultList.querySelectorAll('.gib-graph-result')].find(value => value.dataset.gibFile === file); item?.addClass('is-map-hovered'); item?.scrollIntoView({ block: 'nearest' }); }
  focusResult(file) { const item = [...this.resultList.querySelectorAll('.gib-graph-result')].find(value => value.dataset.gibFile === file); item?.scrollIntoView({ block: 'nearest' }); }
  async showPreview(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (!(file instanceof TFile)) return; const version = ++this.previewVersion; this.previewFilePath = file.path; this.previewTitle.textContent = file.basename; this.previewPath.textContent = file.parent?.path && file.parent.path !== '/' ? file.parent.path : 'Vault'; this.previewBody.empty(); this.previewBody.createDiv({ cls: 'gib-graph-preview-loading', text: 'Loading note…' }); this.previewPanel.show(); this.contentEl.addClass('is-preview-open'); window.setTimeout(() => this.map?.draw(), 0); try { const source = await this.app.vault.cachedRead(file); if (version !== this.previewVersion || this.previewFilePath !== file.path) return; this.previewBody.empty(); await MarkdownRenderer.render(this.app, source, this.previewBody, file.path, this.plugin); } catch (error) { if (version === this.previewVersion) { this.previewBody.empty(); this.previewBody.createDiv({ cls: 'gib-graph-preview-loading', text: error.message }); } } }
  closePreview() { this.previewVersion++; this.previewFilePath = null; this.previewPanel.hide(); this.contentEl.removeClass('is-preview-open'); window.setTimeout(() => this.map?.draw(), 0); }
  async openFile(filePath) { const file = this.app.vault.getAbstractFileByPath(filePath); if (file instanceof TFile) await this.app.workspace.getLeaf(false).openFile(file); }
